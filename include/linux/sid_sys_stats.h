...
extern int get_sid_cputime(struct sid_sys_stats *stat_array, size_t n);

#ifdef CONFIG_PID_SYS_STATS
extern int get_sid_cputime_emon(int type, struct sid_sys_stats *sid_array,
			struct sid_sys_stats *pid_array, size_t n);
#else
extern int get_sid_cputime_emon(int type,
...
#else
static inline int get_sid_cputime(struct sid_sys_stats *stat_array, size_t n)
{ return 0; }
#ifdef CONFIG_PID_SYS_STATS
static inline int get_sid_cputime_emon(int type, struct sid_sys_stats *sid_array,
			struct sid_sys_stats *pid_array, size_t n)
{ return 0; }
#else
static inline int get_sid_cputime_emon(int type, struct sid_sys_stats *stat_array)
{ return 0; }
#endif
