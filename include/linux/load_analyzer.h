...
/*** FUNCTION OPTION ***/
#define CPU_LOAD_HISTORY_NUM_MAX	1000
#define CPU_LOAD_HISTORY_NUM_MIN	150

#define CPU_TASK_HISTORY_NUM_MAX	15000
#define CPU_TASK_HISTORY_NUM_MIN	5000
...
#define CONFIG_SLP_PERFORMANCE_MONITOR	1
//#endif
...
#define CONFIG_KSWAPD_WAKEUP_DEBUGS	1
//#endif
...
#define FRAMERATE_DETECT_PERIOD 5000;
#endif

#if defined(CONFIG_SLP_BUSY_LEVEL)
#define TARGET_BUSY_STATE 9
#define FRAME_DROP_THRESHOLD 30
#define PRESS_RELEASE_THRESHOLD 100
#define TIME_CHECK_DELAY 1000
#define BUSY_LEVEL_TRIGGER_CPU_LOAD 0
#define BUSY_LEVEL_TRIGGER_IOWAIT 1
#define BUSY_LEVEL_TRIGGER_ALLOCSTALL 2
#define BUSY_LEVEL_TRIGGER_FRAME_DROP 3
#define BUSY_LEVEL_TRIGGER_MANUAL -1
#define BUSY_LEVEL_NUMBER_TRIGGER 4
extern unsigned long busy_level_time_trigger[BUSY_LEVEL_NUMBER_TRIGGER];
extern int load_analyzer_init_done;
#endif

#define MAX_CPU_STATE 20

#if 0
#if defined(CONFIG_SLP_KERNEL_ENG) || defined (CONFIG_TIZEN_SEC_KERNEL_ENG)
#define CONFIG_CHECK_TIME_IN_STATE	1
#endif
...
struct time_in_state_busy_level_diff {
	int last_time_in_state;
	int diff;
	int cpu_freq;
	int time_diff_busy_occur;
};
...
extern int performance_monitor_en;
...
/******** +CONFIG_SLP_PERFORMANCE_MONITOR+  ********/

#if defined (CONFIG_SLP_PERFORMANCE_MONITOR)
void performance_monitor_manager(int cnt);
unsigned int show_performance_monitor_read_sub(int cnt, int show_cnt
					, char *buf, unsigned int buf_size, int ret);
#endif
/******** -CONFIG_SLP_PERFORMANCE_MONITOR- ********/
...
int input_dev_info_performance_num;
unsigned int input_dev_info_saved_num;
struct input_dev_info_tag input_dev_info_performance[MAX_INPUT_DEVICES];
...
int check_load_level(unsigned int performance_cnt);
extern int cpu_busy_level_enable;
extern unsigned long high_cpu_load_boot_completed;
#define EXPECTED_CPU_BOOT_TIME	(70 * HZ)
#endif

/******** +CONFIG_CHECK_TIME_IN_STATE+ ********/
#if defined (CONFIG_CHECK_TIME_IN_STATE)
#define TIME_IN_STATE_INTERVAL	5000 /* set time interval to get time in state is 5s */
void la_get_time_in_state(unsigned int state_cnt, unsigned int frequency,  unsigned long long last_time,char *name);
#endif
