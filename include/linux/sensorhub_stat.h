...
#define SHUB_INST_LIB_DATA_WRISTUP_MODE_0 (SHUB_LIB_MAX + 1)
#define SHUB_INST_LIB_DATA_WRISTUP_MODE_MAX (SHUB_INST_LIB_DATA_WRISTUP_MODE_0 + WU_MODE_MAX)
#define SHUB_INST_MAX (SHUB_INST_LIB_DATA_WRISTUP_MODE_MAX + 1)
...
struct sensorhub_wu_stat {
	unsigned int wu_mode;
	unsigned int wakeup;
	ktime_t subtotal_time; /* total time this wrist up mode has been active in wear on state */
	ktime_t total_time; /* total time this wrist up mode has been active */
};

struct sensorhub_wo_stat {
	ktime_t wear_on_time;
	ktime_t wear_off_time;
};
...
static inline int sensorhub_stat_rcv_instruction(unsigned char inst,
	unsigned char type, unsigned char buf, size_t count) { return 0; }
static inline int sensorhub_stat_get_wakeup
				(int type, struct sensorhub_stat_info *sh_stat) { return 0; }
static inline int sensorhub_stat_get_gps
				(struct sensorhub_gps_stat *sh_gps) { return 0; }
static inline int sensorhub_stat_get_gps_emon
...
static inline int sensorhub_stat_get_wu_info
				(struct sensorhub_wu_stat *sh_wu, size_t size) { return 0; }
static inline int sensorhub_stat_get_wo_info(
