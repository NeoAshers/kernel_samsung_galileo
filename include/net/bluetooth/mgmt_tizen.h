...
/* PHYS */
#define MGMT_OP_LE_READ_PHYS		(TIZEN_OP_CODE_BASE + 0x1C)
struct mgmt_cp_le_read_phys {
	bdaddr_t bdaddr;
} __packed;
#define MGMT_LE_SET_READ_PHYS_SIZE	6

#define MGMT_OP_LE_SET_DEFAULT_PHYS		(TIZEN_OP_CODE_BASE + 0x1D)
struct mgmt_cp_le_set_default_phys {
	__u8	all_phy;
	__u8	tx_phys;
	__u8	rx_phys;
} __packed;
#define MGMT_OP_LE_SET_DEFAULT_PHYS_SIZE	3

#define MGMT_OP_LE_SET_PHYS		(TIZEN_OP_CODE_BASE + 0x1E)
struct mgmt_cp_le_set_phys {
	bdaddr_t bdaddr;
	__u8	tx_phys;
	__u8	rx_phys;
	__u16	phy_opt;
} __packed;
#define MGMT_LE_SET_PHY_SIZE	10

struct mgmt_rp_le_set_phys {
	__u8 status;
} __packed;
#define MGMT_LE_SET_PHYS_RSP_SIZE	 1

#define MGMT_OP_GET_LE_BATCHING_BUFFER	(TIZEN_OP_CODE_BASE + 0x1F)
#define MGMT_GET_LE_BATCHING_BUFFER_SIZE	0

struct mgmt_rp_get_le_batching_buffer {
	__u8 status;
	__u32 buffer;
} __packed;
#define MGMT_GET_LE_BATCHING_BUFFER_RSP_SIZE	5
...
#define MGMT_SET_LE_BATCHING_PARAM_SIZE	10

struct mgmt_rp_set_le_batching_param {
	__u8 status;
} __packed;
#define MGMT_SET_LE_BATCHING_PARAM_RSP_SIZE	1
...
#define MGMT_SET_LE_BATCHING_ENABLE_SIZE	7

struct mgmt_rp_set_le_batching_enable {
	__u8 status;
} __packed;
#define MGMT_SET_LE_BATCHING_ENABLE_RSP_SIZE	1

#define MGMT_OP_DELETE_LE_BATCHING_INST	(TIZEN_OP_CODE_BASE + 0x22)
struct mgmt_cp_delete_le_batching_inst {
	bdaddr_t bdaddr;
} __packed;
#define MGMT_DELETE_LE_BATCHING_INST_SIZE	6
...
#define MGMT_EV_DBFW_PLUS	 (TIZEN_EV_BASE + 0x0e)
struct mgmt_ev_dbfw_plus {
	__u8	event_code;
	__u8	data[0];
} __packed;
...
#define MGMT_EV_LE_PHY_UPDATED		(TIZEN_EV_BASE + 0x10)
struct mgmt_ev_le_phy_updated {
	bdaddr_t addr;
	__u8	status;
	__u8	tx_phys;
	__u8	rx_phys;
...
#define MGMT_EV_LE_READ_PHY		(TIZEN_EV_BASE + 0x11)
struct mgmt_rp_le_read_phys {
	bdaddr_t bdaddr;
	__u8	status;
	__u8	tx_phys;
	__u8	rx_phys;
...
#define MGMT_EV_ALF	 (TIZEN_EV_BASE + 0x12)
struct mgmt_ev_alf {
	__le16	handle;
	__u8	link_status;
	__le16	packet_count;
