...
enum {
	...
	DBFW_PLUS_LINK_LOSS_PAGE_SLOTS_EVENT_CODE,
	DBFW_PLUS_LINK_LOSS_LMP_TRACE_EVENT_CODE,
	DBFW_PLUS_A2DP_INFO_EVENT_CODE = 0x20,
	DBFW_PLUS_HFP_INFO_EVENT_CODE = 0x31,
	DBFW_PLUS_HFP_SCO_PACKET_TYPE_INFO_EVENT_CODE,
	DBFW_PLUS_RSSI_NOC_EVENT_CODE = 0x40,
};
#endif
...
#define LE_BATCHING_SUBCODE_GET_BUFFER	0x01
#define LE_BATCHING_SUBCODE_SET_PARAM	0x02
#define LE_BATCHING_SUBCODE_SET_ENABLE	0x03
#define LE_BATCHING_SUBCODE_DEL_INSTANCE	0x04

#define HCI_OP_LE_BATCHING	0xfe47
struct hci_cp_get_le_batching_buffer {
	__u8	subcode;
} __packed;

struct hci_rp_get_le_batching_buffer {
	__u8	status;
	__u8	subcode;
	__u16   buf_size;
} __packed;

struct hci_cp_set_le_batching_param {
	__u8	subcode;
	__le16	handle;
	__u16   cid;
	__u16	pkt_th;
...
struct hci_rp_le_batching {
	__u8	status;
	__u8	subcode;
} __packed;

struct hci_cp_set_le_batching_enable {
	__u8	subcode;
	__le16	handle;
	__u16   cid;
	__u8	enable;
...
struct hci_rp_set_le_batching_enable {
	__u8	status;
} __packed;

struct hci_cp_delete_le_batching_inst {
	__u8	subcode;
	__le16	handle;
	__u16   cid;
} __packed;
...
/* DBFW+ VSC */
#define HCI_OP_DBFW_PLUS	0xFDF1
...
struct hci_cp_dbfw_plus_a2dp_debug {
	__u8	subcommand;
	__u8	enable;
	__le16	handle;
	__le16	delay_threshold;
} __packed;

struct hci_cp_dbfw_plus_rssi_by_noc {
	__u8	subcommand;
	__u8	enable;
} __packed;

#if defined(CONFIG_NOBLESSE)
#define HCI_OP_EWP	0xFE20
struct hci_cp_enable_ewp {
	__u8	subcommand;
	__u8	params[16];
} __packed;
...
#define SEC_BRCM_ALF_EVENT 0xB0
#endif
