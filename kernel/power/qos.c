...
#ifdef CONFIG_PM_QOS_STAT
static struct pm_qos_request removed_req[PM_QOS_NUM_CLASSES];
#endif
			 ...
			 enum pm_qos_req_action action, int value, void *notify_param)
{
	...
	switch (action) {
	case PM_QOS_REMOVE_REQ:
		req->remove_req_count++;

		if (req->active) {
			now = ktime_get();
			duration = ktime_sub(now, req->last_time);
			req->total_time = ktime_add(req->total_time, duration);
			req->last_time = now;

			pr_debug("%s:%d:%s:%d: %02d: %lld: %d:%d:%d: %lld\n",
				__func__, action, req->func, req->line,
				req->pm_qos_class, req->qos_req_count,
				prev_value, new_value, curr_value,
				ktime_to_ms(duration));
		}
		break;
	case PM_QOS_ADD_REQ:
		req->add_req_count++;
		req->last_time = ktime_get();

		if (new_value == 0) {
			pr_debug("%s:%d:%s:%d: %02d: %lld: %d:%d:%d\n",
				__func__, action, req->func, req->line,
				req->pm_qos_class, req->qos_req_count,
				prev_value, new_value, curr_value);
			break;
...
#ifdef CONFIG_PM_QOS_STAT
	req->add_req_count = 0;
	req->update_req_count = 0;
	req->remove_req_count = 0;
	req->qos_req_count = 0;
	req->active = 0;

	pr_info("%s:%s:%d: %02d: %d\n",
		__func__, req->func, req->line, req->pm_qos_class, value);
#endif
...
#ifdef CONFIG_PM_QOS_STAT
	removed_req[req->pm_qos_class].add_req_count += req->add_req_count;
	removed_req[req->pm_qos_class].update_req_count += req->update_req_count;
	removed_req[req->pm_qos_class].remove_req_count += req->remove_req_count;
	removed_req[req->pm_qos_class].qos_req_count += req->qos_req_count;
	removed_req[req->pm_qos_class].total_time =
		ktime_add(removed_req[req->pm_qos_class].total_time, req->total_time);
	removed_req[req->pm_qos_class].last_time = req->last_time;

	pr_info("%s:%s:%d: %02d: %ld:%lld:%ld:%lld:%lld: %ld:%lld:%ld:%lld:%lld\n",
		__func__, req->func, req->line,
		req->pm_qos_class,
		req->add_req_count, req->update_req_count,
		req->remove_req_count, req->qos_req_count,
		ktime_to_ms(req->total_time),
		removed_req[req->pm_qos_class].add_req_count,
		removed_req[req->pm_qos_class].update_req_count,
		removed_req[req->pm_qos_class].remove_req_count,
		removed_req[req->pm_qos_class].qos_req_count,
		ktime_to_ms(removed_req[req->pm_qos_class].total_time));
#endif
...
#ifdef CONFIG_PM_QOS_STAT
static void pm_qos_debug_show_one(struct seq_file *s, struct pm_qos_object *qos, int pm_qos_class)
{
	struct plist_node *p;
	unsigned long flags;
	...
	plist_for_each(p, &qos->constraints->list)
		seq_printf(s, " 	 %pk(%s:%d:%ld:%lld:%ld:%lld:%lld): %d\n",
				...
				(container_of(p, struct pm_qos_request, node))->add_req_count,
				(container_of(p, struct pm_qos_request, node))->update_req_count,
				(container_of(p, struct pm_qos_request, node))->remove_req_count,
				(container_of(p, struct pm_qos_request, node))->qos_req_count,
				ktime_to_ms((container_of(p, struct pm_qos_request, node))->total_time),
				p->prio);

	seq_printf(s, "   removed requests:\n");
	seq_printf(s, " 	 (%ld:%lld:%ld:%lld:%lld)\n",
			removed_req[pm_qos_class].add_req_count,
			removed_req[pm_qos_class].update_req_count,
			removed_req[pm_qos_class].remove_req_count,
			removed_req[pm_qos_class].qos_req_count,
			ktime_to_ms(removed_req[pm_qos_class].total_time));
...
static int pm_qos_debug_show(struct seq_file *s, void *d)
{
	...
	for (i = 1; i < PM_QOS_NUM_CLASSES; i++)
		pm_qos_debug_show_one(s, pm_qos_array[i], i);

	return 0;
}

static int pm_qos_debug_all_open(struct inode *inode, struct file *file)
{
	return single_open(file, pm_qos_debug_show, inode->i_private);
}

const static struct file_operations pm_qos_debug_all_fops = {
	.open		= pm_qos_debug_all_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
#ifdef CONFIG_PM_QOS_STAT
	debugfs_create_file("all", S_IRUGO, d, NULL, &pm_qos_debug_all_fops);
#endif
	return ret;
}
