...
int ext4_register_sysfs(struct super_block *sb)
{
	...
	if (strnlen(sbi->s_es->s_volume_name, 16) == strlen("tizen-user") &&
		strncmp(sbi->s_es->s_volume_name, "tizen-user", strlen("tizen-user")) == 0) {
		err = sysfs_create_link(&ext4_kset.kobj, &sbi->s_kobj, "userdata");

		if (err)
			printk(KERN_ERR "Can not create sysfs link for userdata(%d)", err);
	}
