				   ...
				   struct pt_regs *regs)
{
	...
	if (addr < USER_DS && is_permission_fault(esr, regs)) {
		...
		if (regs->orig_addr_limit == KERNEL_DS) {
			pr_alert("Accessing user space memory(%lx) with fs=KERNEL_DS\n", addr);
#ifdef CONFIG_SEC_DEBUG_EXTRA_INFO
		...
		if (is_el1_instruction_abort(esr)) {
			pr_alert("Attempting to execute userspace memory(%lx)\n", addr);
#ifdef CONFIG_SEC_DEBUG_EXTRA_INFO
			sec_debug_set_extra_info_fault(PAGE_FAULT, addr, regs);
			sec_debug_set_extra_info_esr(esr);
#endif
		...
		if (!search_exception_tables(regs->pc)) {
			pr_alert("Accessing user space memory(%lx) outside uaccess.h routines\n", addr);
#ifdef CONFIG_SEC_DEBUG_EXTRA_INFO
			sec_debug_set_extra_info_fault(PAGE_FAULT, addr, regs);
			sec_debug_set_extra_info_esr(esr);
#endif
