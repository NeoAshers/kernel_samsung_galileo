...
#if 0 // NEED_TO_REMOVE_CODE_FOR_NOBLESSE
...
//#define I2C_M_RD                 1 /* for i2c Read */ //Already defined in /include/linux/i2c.h
...
static DEVICE_ATTR(register_read, S_IWUSR | S_IWGRP | S_IRUGO,
		sx9360_register_read_show, sx9360_register_read_store);
		...
		unsigned long action, void *data)
{
	...
	if (pdata->init_done == ON) {
		...
		case ATTACHED_DEV_JIG_USB_ON_MUIC:
			pr_info("[SX9360]: %s skip cable = %d, attach = %d\n",
				__func__, usb_typec_info.cable_type, usb_typec_info.attach);
			break;
		default:
			pr_info("[SX9360]: %s accept cable = %d, attach = %d\n",
				__func__, usb_typec_info.cable_type, usb_typec_info.attach);
