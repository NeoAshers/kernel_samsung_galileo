...
#ifdef CONFIG_RENAISSANCE
#define DISABLE_VDD_CONTROL		/* USE FULL TIME POWER VDD02 */
#endif
...
#ifdef DISABLE_VDD_CONTROL
	prs->pdata->power_on = lps22_default_power_on;
	prs->pdata->power_off = lps22_default_power_off;
	dev_info(prs->dev, "Using default power_on/off ldo(DISABLE_VDD_CONTROL) \n");
#else
	const char *vdd_str;
