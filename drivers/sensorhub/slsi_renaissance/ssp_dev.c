...
#if 0
	data->ecg_con_det = 0;
	data->ecg_con_value = 1; /*Init with 1*/
	if(of_get_named_gpio(np, "ecg-con-det", 0))
	{
		data->ecg_con_det = of_get_named_gpio(np, "ecg-con-det", 0);
		data->ecg_con_value = gpio_get_value(data->ecg_con_det);

		pr_info("[SSP] ecg detect pin: %d value: %d\n", data->ecg_con_det, data->ecg_con_value);
	}
#endif
...
#ifdef CONFIG_SENSORS_SSP_CHUB
//	init_waitqueue_head(&data->comm_wait);
//	init_waitqueue_head(&data->pending_wait);
