...
static void get_ecg_fac_sensordata(char *pchRcvDataFrame, int *iDataIdx,
	struct sensor_value *sensorsdata)
{
	memcpy(sensorsdata, pchRcvDataFrame + *iDataIdx, 4*HRM_ECG_FAC_DATA_SIZE);
	*iDataIdx += 4*HRM_ECG_FAC_DATA_SIZE;
}

static void get_ecg_raw_sensordata(char *pchRcvDataFrame, int *iDataIdx,
	struct sensor_value *sensorsdata)
{
	memcpy(sensorsdata, pchRcvDataFrame + *iDataIdx, 4*HRM_ECG_LIB_DATA_SIZE);
	*iDataIdx += 4*HRM_ECG_LIB_DATA_SIZE;
}
#endif
...
static void get_hrm_raw_fac_sensordata(char *pchRcvDataFrame, int *iDataIdx,
	struct sensor_value *sensorsdata)
{
	memcpy(sensorsdata, pchRcvDataFrame + *iDataIdx, 4*HRM_EOL_DATA_SIZE);
	*iDataIdx += 4*HRM_EOL_DATA_SIZE;
}
		...
		case MSG2AP_INST_BYPASS_DATA:
			...
			do {
				...
				if (data->skipEventReport == false) {
					if(iSensorData == HRM_ECG_LIB_SENSOR)
					{
						report_sensor_data(data, SENSOR_TYPE_ECG_IIO-1, &sensorsdata);
					}
					else
					{
						data->report_sensor_data[iSensorData](data, &sensorsdata);
...
void initialize_function_pointer(struct ssp_data *data)
{
	...
	data->get_sensor_data[ACCELEROMETER_SENSOR_32G] = get_3axis_sensordata;
	data->report_sensor_data[ACCELEROMETER_SENSOR_32G] = report_acc32g_data;
#endif
	...
	data->get_sensor_data[HRM_ECG_FAC_SENSOR] = get_ecg_fac_sensordata;
	data->report_sensor_data[HRM_ECG_FAC_SENSOR] = report_ecg_fac_data;
#endif

#ifdef CONFIG_SENSORS_ECG_LIB
	data->get_sensor_data[HRM_ECG_LIB_SENSOR] = get_ecg_raw_sensordata;
	data->report_sensor_data[HRM_ECG_LIB_SENSOR] = report_ecg_raw_data;
#endif
...
#endif
#ifndef CONFIG_SENSORS_SSP_CHUB
	...
	data->ssp_big_task[BIG_TYPE_WOM] = ssp_send_big_library_task;
#endif
}
