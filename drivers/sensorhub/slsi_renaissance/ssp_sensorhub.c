...
void ssp_read_big_library_task(struct work_struct *work)
{
	...
	while (residue > 0) {
		...
		msg->free_buffer = 0;
#if 0
		ret = ssp_ipc_sync(big->data, msg, 1000);
#else
		...
		memcpy((hub_data->big_events.library_data + pos), rbuf, rbuf_length);
...
void ssp_send_big_library_task(struct work_struct *work)
{
  ...
  while (residue > 0) {
    ...
    msg->free_buffer = 0;
#if 0
    ret = ssp_ipc_sync(big->data, msg, timeout);
#else
		ret = ssp_send_command(big->data, msg->cmd, msg->options, 1000,
													msg->buffer, msg->length, &rbuf, &rbuf_length, msg->data);
#endif
    ...
    memcpy((hub_data->big_send_events.library_data + pos), rbuf, rbuf_length);
