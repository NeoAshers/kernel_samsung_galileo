...
void ssp_dump_task(struct work_struct *work)
{
	...
	while (residue > 0) {
		...
		msg->free_buffer = 0;
#if 0
		iRetTrans = ssp_ipc_sync(big->data, msg, 1000);
#else
		iRetTrans = ssp_send_command(big->data, msg->cmd, msg->options, 1000,
	...
	kfree(big);
#endif
	pr_err("[SSP]: %s Chub is not support dump log done\n", __func__);
}
...
void ssp_temp_task(struct work_struct *work) {
	...
	while (residue > 0) {
		...
		msg->free_buffer = 0;
#if 0
		iRet = ssp_ipc_sync(big->data, msg, 1000);
#else
		iRet = ssp_send_command(big->data, msg->cmd, msg->options, 1000,
														msg->buffer, msg->length, &rbuf, &rbuf_length, msg->data);
#endif
...
#ifdef SSP_LIBSTATE_DEBUG
	/*First 8 byte of debug message is libstate*/
	data->uLibraryState = *(u64 *)(&pchRcvDataFrame[cur]);
	ssp_dbg("[SSP]: %s : iLength: %d, LibraryState: %llx\n", __func__, iLength, data->uLibraryState);
	ssp_dbg("[SSP]: MSG From MCU - %s\n", &pchRcvDataFrame[(*pDataIdx)+8]);
#else
...
//	ssp_enable(data, false);
	...
	if(ret) {
		pr_err("[SSP] %s, Chub reset fail", __func__);
	}
...
#endif
		break;
	case HRM_ECG_FAC_SENSOR:
		ssp_dbg("[SSP] %u : %d, %d, %d, %d, %d, %d, %d, %d, %d, %d(%ums, %dms)\n", uSensor,
		data->buf[uSensor].ecg_lead_off, data->buf[uSensor].ecg_ext_noise,
		data->buf[uSensor].ecg_amp, data->buf[uSensor].ecg_freq,
		data->buf[uSensor].ecg_trim, data->buf[uSensor].reserved1,
		data->buf[uSensor].reserved2, data->buf[uSensor].reserved3,
		data->buf[uSensor].reserved4, data->buf[uSensor].reserved5,
		get_msdelay(data->adDelayBuf[uSensor]),
		data->batchLatencyBuf[uSensor]);
		break;
...
static void debug_work_func(struct work_struct *work)
{
	...
	ssp_dbg("[SSP]: %s(%u) - Sensor state: 0x%llx, RC: %u, CC: %u, DC: %u, TC: %u\n",
