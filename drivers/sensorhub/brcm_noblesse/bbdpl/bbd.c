...
struct bbd_device {
	...
	unsigned long patch_tbit;	/* FW type bit */
};
...
static unsigned char bbd_wom_patch[] = {
#include "mainhub_18050910.h"
};
#else
static unsigned char bbd_wom_patch[] = {
		"dummy",
...
static void bbd_report_stat(struct work_struct *work)
{
	...
	p += sprintf(p, " host_req=%d mcu_resp=%d mcu_req:%d",
		stat1hz.host_req, stat1hz.mcu_resp, stat1hz.mcu_req);
	p += sprintf(p, " irq_en:%d irq_cnt:%d",
		stat1hz.irq_enabled, stat1hz.irq_count);
...
ssize_t bbd_control(const char *buf, ssize_t len)
{
...
#ifdef CONFIG_SENSORHUB_SPU
	if(strstr(buf, BBD_CTRL_PASSTHRU_ON)){
		pr_info("[SSP_SPU]: spu_flag :%d\n",get_spu_flag());
		//open_spu();
	}
	if(strstr(buf, BBD_CTRL_PASSTHRU_OFF)){
		close_spu();
	}
#endif

	if (strstr(buf, ESW_CTRL_READY)) {
		pr_info("[BBD_SSP]: current FW type : 0x%lx\n",
			bbd.patch_tbit);
    ...
    /* We assuem that LHD always sets NOTREADY before,
		 * it try to re-download FW again.
		 */
		if (bbd.ssp_cb && bbd.ssp_cb->get_wom_state) {
			if (bbd.ssp_cb->get_wom_state(bbd.ssp_priv, &wom_mode))
				pr_err("[BBD_SSP]:%s():failed to get WOM_MODE\n",
					__func__);

			if (wom_mode) {
				set_bit(WOM_BBD_PATCH_RAM, &bbd.patch_tbit);
				pr_warn("[BBD_SSP]: WOM_FW will be used!!\n");
			} else
				clear_bit(WOM_BBD_PATCH_RAM, &bbd.patch_tbit);
		}
		...
		ssi_dbg_rng = false;
#endif
	} else if (strstr(buf, RPC_ALLOW_SSP)) {
		rpc_denay_ssp = false;
	} else if (strstr(buf, RPC_DENAY_SSP)) {
		rpc_denay_ssp = true;
	...
	if (strstr(buf, BBD_CTRL_LHD_STOP)) {
		saveMiniDump(buf);
	}
...
ssize_t bbd_patch_read( struct file *filp, char __user *buf, size_t size, loff_t *ppos)
{
	...
	if (bbd->ssp_cb && bbd->ssp_cb->get_wom_state) {
		ret = bbd->ssp_cb->get_wom_state(bbd->ssp_priv, &wom_mode);
		if (ret)
			pr_err("[BBD_SSP]:%s():failed to get WOM_MODE(%d)\n",
				__func__, ret);
	}

	if (test_bit(WOM_BBD_PATCH_RAM, &bbd->patch_tbit)) {
		curr_bbd_patch = bbd_wom_patch;
		bbd_patch_sz = sizeof(bbd_wom_patch);
	} else {
		if (test_bit(LEGACY_BBD_PATCH_RAM, &bbd->patch_tbit)) {
			curr_bbd_patch = legacy_bbd_patch;
			bbd_patch_sz = sizeof(legacy_bbd_patch);
		}
#ifdef CONFIG_SENSORHUB_SPU
		else if(get_spu_flag()){
			curr_bbd_patch = get_spu_buffer();
			bbd_patch_sz = get_spu_size();
		}
#endif
		else {
...
int bbd_init(struct device *dev, bool legacy_patch)
{
	...
	if (legacy_patch) {
		set_bit(LEGACY_BBD_PATCH_RAM, &bbd.patch_tbit);
		pr_warn("[BBD_SSP]: legacy bbd patch-ram used!!\n");
	} else
		set_bit(NORMAL_BBD_PATCH_RAM, &bbd.patch_tbit);
