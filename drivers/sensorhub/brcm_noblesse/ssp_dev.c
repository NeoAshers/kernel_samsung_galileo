...
{
	struct sensor_info sensorinfo[SENSOR_TYPE_ECG_IIO] = {
...
static void initialize_variable_iio(struct ssp_data *data)
{
	...
	data->delay[SENSOR_TYPE_ECG_IIO-1].sampling_period = DEFUALT_POLLING_DELAY;
	data->delay[SENSOR_TYPE_ECG_IIO-1].max_report_latency = 0;
...
static void initialize_variable(struct ssp_data *data)
{
	...
	data->uSubIrqCnt = 0;
#endif
	data->iHubState = EVT2FW_HUB_STATE_NOT_READY;
	data->ulHubStateBit = 0;
	...
	data->motor_flag = FF_MEMLESS_EVENT_STOP;
#endif
...
int initialize_mcu(struct ssp_data *data)
{
	...
	set_sensor_state(data->uSensorState);
	...
	check_spu_firmware_match(data->uCurFirmRev);
#endif
...
static bbd_callbacks ssp_bbd_callbacks = {
	...
	.on_mcu_ready       = callback_bbd_on_mcu_ready,
      ...
      .get_wom_state    = callback_bbd_get_wom_state,
#endif
};

#ifdef SSP_LDO25_RESET
static int sensorhub_get_regulator(struct device *dev, struct ssp_data *data)
{
	char *str_regulator_3p0v = NULL;

	data->regulator_3p0v = NULL;
	pr_info("%s : start\n", __func__);

	if(!of_property_read_string(dev->of_node, "regulator_3p0v", (const char **)&str_regulator_3p0v)) {
		pr_info("%s : regulator_get\n", __func__);
		data->regulator_3p0v = regulator_get(dev, str_regulator_3p0v);
		if (IS_ERR(data->regulator_3p0v)) {
			pr_err("%s : SSP_LDO25 3.0V get failed\n", __func__);
			regulator_put(data->regulator_3p0v);
			data->regulator_3p0v = NULL;
		}
	}
	else
	{
		pr_info("%s : regulator_get fail\n", __func__);
	}
	...
	struct ssp_data *data)
{
	...
	sensorhub_get_regulator(dev, data);
#endif
...
#ifdef SSP_LDO25_RESET
	if (data->regulator_3p0v != 0) {
		pr_info("[SSP]: %s - regulator enable1\n", __func__);
		iRet = regulator_enable(data->regulator_3p0v);
		if (iRet < 0)
			pr_err("[SSP]: %s - SSP_LDO25 enable failed\n", __func__);
...
static int sensor_motor_notifier_callback(struct notifier_block *self,
    unsigned long event, void *data)
{
  ...
  effect = (struct ff_effect *)data;
	...
	case FF_MEMLESS_EVENT_STOP:
		...
		if(ssp_data_info->motor_flag != FF_MEMLESS_EVENT_STOP)
		{
			ssp_data_info->motor_flag = FF_MEMLESS_EVENT_STOP;
			ssp_data_info->motor_duration = 0;
			queue_work(ssp_data_info->ssp_motor_wq, &ssp_data_info->work_ssp_motor);
		}
		break;
	default:
...
static struct notifier_block sensor_motor_notifier = {
	.notifier_call = sensor_motor_notifier_callback,
};
...
static int ssp_probe(struct spi_device *spi)
{
	...
	iRet = initialize_indio_dev(&(spi->dev), data);
	if (iRet < 0) {
	...
	iRet = ff_memless_register_client(&sensor_motor_notifier);
	pr_info("[SSP]: sensor_motor_callback result : %d\n", iRet);
...
#ifdef CONFIG_SENSORHUB_SPU
	schedule_delayed_work(&data->spu_work, msecs_to_jiffies(5000));
#endif
...
static int ssp_suspend(struct device *dev)
{
	...
	data->uLastResumeStatePM = MSG2SSP_AP_STATUS_SUSPEND;
	disable_debug_timer(data);
#if SSP_STATUS_MONITOR
	cancel_delayed_work_sync(&data->polling_work);
#endif
...
static int ssp_resume(struct device *dev)
{
	...
	data->uLastResumeStatePM = MSG2SSP_AP_STATUS_RESUME;

#if 1 /* Enable resume cmd */
