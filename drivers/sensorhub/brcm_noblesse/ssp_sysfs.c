	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	return size;
#else
    pr_err("[SSP]: %s: wom_cmd not supported\n",__func__);
    return -EINVAL;
#endif
}
...
static ssize_t show_ecg_fac_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct ssp_data *data = dev_get_drvdata(dev);

	return snprintf(buf, PAGE_SIZE, "%lld\n",
		data->adDelayBuf[HRM_ECG_FAC_SENSOR]);
}

static ssize_t set_ecg_fac_delay(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	int64_t dNewDelay;
	struct ssp_data *data = dev_get_drvdata(dev);
	...
	change_sensor_delay(data, HRM_ECG_FAC_SENSOR, dNewDelay);
...
static ssize_t cpu_profile_info_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct ssp_data *sdev = dev_get_drvdata(dev);
    struct brcm_cpu_Profile_info info;

	get_cpu_profile_info(sdev, &info);

	return snprintf(buf, PAGE_SIZE, "%llu %llu %llu %llu %llu %llu\n",info.cpu_26mhz_ms, info.cpu_52mhz_ms, info.cpu_turbo_ms,
            info.idle_ms, info.sleep_ms, info.total_ms);
}

static ssize_t mini_dump_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	/*struct ssp_data *sdev = dev_get_drvdata(dev);*/
	char miniDump[MINI_DUMP_MAX_LEN+2] = {0,};
	u64 SensorState = 0;
	u64 LibraryState = 0;

	getMiniDump(miniDump, &SensorState, &LibraryState);

	return snprintf(buf, PAGE_SIZE, "%llx %llx %s\n",SensorState, LibraryState, miniDump);
}
...
#endif
#ifdef CONFIG_SENSORS_ECG_FAC
static DEVICE_ATTR(ecg_fac_poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	show_ecg_fac_delay, set_ecg_fac_delay);
#endif
#ifdef CONFIG_SENSORS_ECG_LIB
...
static DEVICE_ATTR(cpu_profile_info, S_IRUGO, cpu_profile_info_show, NULL);
static DEVICE_ATTR(mini_dump, S_IRUGO, mini_dump_show, NULL);
...
#ifdef CONFIG_SENSORS_SSP_ACCELEROMETER_32GSENSOR
static struct device_attribute dev_attr_input_accel32g_poll_delay
	= __ATTR(poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	show_acc32g_delay, set_acc32g_delay);
#endif
...
static struct device_attribute dev_attr_input_temp_humi_poll_delay
	= __ATTR(poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	show_temp_humi_delay, set_temp_humi_delay);
static struct device_attribute dev_attr_input_ecg_fac_poll_delay
	= __ATTR(poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	show_ecg_fac_delay, set_ecg_fac_delay);
static struct device_attribute dev_attr_input_ecg_lib_poll_delay
	= __ATTR(poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	show_ecg_lib_delay, set_ecg_lib_delay);
...
static void remove_input_poll_delay_sysfs(struct ssp_data *data)
{
	...
	if (data->ecg_fac_input_dev)
		device_remove_file(&data->ecg_fac_input_dev->dev,
			&dev_attr_input_ecg_fac_poll_delay);
	if (data->ecg_lib_input_dev)
		device_remove_file(&data->ecg_lib_input_dev->dev,
			&dev_attr_input_ecg_lib_poll_delay);
	...
	if (data->acc32g_input_dev)
		device_remove_file(&data->acc32g_input_dev->dev,
			&dev_attr_input_accel32g_poll_delay);
}
...
static int initialize_input_poll_delay_sysfs(struct ssp_data *data)
{
	...
	if (data->acc32g_input_dev) {
		iRet = device_create_file(&data->acc32g_input_dev->dev,
					&dev_attr_input_accel32g_poll_delay);
		if (iRet < 0)
			goto err_create_poll_delay;
	}
	...
	if (data->ecg_fac_input_dev) {
		iRet = device_create_file(&data->ecg_fac_input_dev->dev,
			&dev_attr_input_ecg_fac_poll_delay);
		if (iRet < 0)
			goto err_create_poll_delay;
	}
	...
	if (data->ecg_lib_input_dev) {
		iRet = device_create_file(&data->ecg_lib_input_dev->dev,
			&dev_attr_input_ecg_lib_poll_delay);
		if (iRet < 0)
			goto err_create_poll_delay;
	}
