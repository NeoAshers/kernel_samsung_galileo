...
#define CALIBRATION_FILE_PATH	"/csa/sensor/accel_cal_data1"
...
int lsm6dsl_accel_open_calibration1(struct ssp_data *data)
{
	...
	ssp_dbg("[SSP]: accel calibration1 read_buf %d, %d, %d %d\n",
	...
	ssp_dbg("[SSP]: open accel calibration1 %d, %d, %d\n",
...
int lsm6dsl_set_accel_cal1(struct ssp_data *data)
{
	...
	if (!(data->uSensorState & (1 << ACCELEROMETER_SENSOR_32G))) {
...
static int enable_accel_for_cal(struct ssp_data *data)
{
	...
	if (atomic64_read(&data->aSensorEnable) & (1 << ACCELEROMETER_SENSOR_32G)) {
...
static int accel_do_calibrate1(struct ssp_data *data, int iEnable)
{
	...
	ssp_dbg("[SSP]: do accel calibrate1 %d, %d, %d\n",
		data->accelcal.x, data->accelcal.y, data->accelcal.z);
	...
	if (IS_ERR(cal_filp)) {
		pr_err("[SSP]: %s - Can't open calibration1 file\n", __func__);
		set_fs(old_fs);
		iRet = PTR_ERR(cal_filp);
		return iRet;
	}
	...
	ssp_dbg("[SSP]:accel calibrate1 write_buf %d, %d, %d %d\n",
		send_buf[0], send_buf[1], send_buf[2], iRet);

	if (iRet != 3 * sizeof(int)) {
		pr_err("[SSP]: %s - Can't write the accelcal1 to file\n",
			__func__);
		iRet = -EIO;
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sysfs_streq(buf, "1"))
		...
		msg->cmd = ACCELEROMETER_FACTORY_32G;
		msg->length = 1;
...
static ssize_t accel_hw_selftest_show1(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	unsigned char chTempBuf[14] = { 2, 0, };
	...
	shift_ratio[0] = (u16)((chTempBuf[2] << 8) + chTempBuf[1]);
	shift_ratio[1] = (u16)((chTempBuf[4] << 8) + chTempBuf[3]);
	shift_ratio[2] = (u16)((chTempBuf[6] << 8) + chTempBuf[5]);
	shift_ratio[3] = (u16)((chTempBuf[8] << 8) + chTempBuf[7]);
	shift_ratio[4] = (u16)((chTempBuf[10] << 8) + chTempBuf[9]);
	shift_ratio[5] = (u16)((chTempBuf[12] << 8) + chTempBuf[11]);
	result = chTempBuf[13];
