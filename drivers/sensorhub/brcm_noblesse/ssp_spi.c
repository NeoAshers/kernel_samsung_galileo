...
typedef struct mini_dump_data {
	u64 uSensorState;
	u64 uLibraryState;
	char data[MINI_DUMP_MAX_LEN+2];
}mini_dump_data_t;

static mini_dump_data_t miniDump;
static u64 gCurrentSensorState;
static u64 gCurrentLibraryState;
...
int ssp_send_cmd(struct ssp_data *data, char command, int arg)
{
	...
	ssp_dbg("[SSP]: %s - command 0x%x %d , resume state[%x] FW:%x PM:%x\n", __func__, command, arg,
			data->uLastResumeState, data->uLastResumeStateFW, data->uLastResumeStatePM);
...
unsigned int get_ssr_stuck_info(struct ssp_data *data) {
	...
	if (iRet != SUCCESS){
		...
		data->uSsrStuckInfo[0] = buf[0];
		data->uSsrStuckInfo[1] = buf[1];
		data->uSsrStuckInfo[2] = buf[2];
		data->uSsrStuckInfo[3] = buf[3];
...
unsigned int get_cpu_profile_info(struct ssp_data *data, struct brcm_cpu_Profile_info* info) {
	...
	msg->cmd = MSG2SSP_CPU_PROFILE_INFO;
	msg->length = sizeof(struct brcm_cpu_Profile_info);
	msg->options = AP2HUB_READ;
	msg->buffer = (char*) info;
	msg->free_buffer = 0;
	...
	if (iRet != SUCCESS){
		pr_err("[SSP]: %s - transfer fail %d\n", __func__, iRet);
		memset(info, 0x0, sizeof(struct brcm_cpu_Profile_info));
	}
	else{
		pr_err("[SSP]: %s (%lu)- %llu %llu %llu %llu %llu %llu\n", __func__, sizeof(struct brcm_cpu_Profile_info), info->cpu_26mhz_ms, info->cpu_52mhz_ms, info->cpu_turbo_ms,
...
{
	gCurrentSensorState = uSensorState;
}

#ifdef SSP_LIBSTATE_DEBUG
void set_library_state(u64 uLibraryState)
{
	gCurrentLibraryState = uLibraryState;
}
#endif

void saveMiniDump(const char *buf)
{
	memcpy(miniDump.data, buf, MINI_DUMP_MAX_LEN);
	miniDump.uSensorState = gCurrentSensorState;
	miniDump.uLibraryState = gCurrentLibraryState;

	pr_info("[SSP_BIG_S]: SensorState(0x%llx) LibraryState(0x%llx) \n", miniDump.uSensorState, miniDump.uLibraryState);
	pr_info("[SSP_BIG_S]: %s \n", miniDump.data);
}

void getMiniDump(char *buf, u64 *SensorState, u64 *LibraryState)
{
	pr_info("[SSP_BIG]: SensorState(0x%llx) LibraryState(0x%llx) \n", miniDump.uSensorState, miniDump.uLibraryState);
	pr_info("[SSP_BIG]: %s \n", miniDump.data);

	memcpy(buf, miniDump.data, MINI_DUMP_MAX_LEN);
	*SensorState = miniDump.uSensorState;
	*LibraryState = miniDump.uLibraryState;

	// reset mini dump var
	memset((void *)&miniDump, 0x0, sizeof(mini_dump_data_t));
}
