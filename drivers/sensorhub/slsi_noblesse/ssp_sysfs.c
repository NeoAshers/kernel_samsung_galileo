...
static ssize_t sensorhub_dump_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "[SSP] sensorhub_dump_show is temporarily not available ");
#if 0
	int iRet=0;
...
#if 1//def CONFIG_SENSORS_SSP_BBD
...
static DEVICE_ATTR(feature_list, 0664, feature_list_show, NULL);
