...
int lsm6dsl_gyro_open_calibration(struct ssp_data *data)
{
	...
	ssp_dbg("[SSP]: gyro calibration read_buf %d, %d, %d %d\n",
		read_buf[0], read_buf[1], read_buf[2], iRet);

	data->gyrocal.x = (s16)read_buf[0];
	data->gyrocal.y = (s16)read_buf[1];
	data->gyrocal.z = (s16)read_buf[2];
...
int save_gyro_caldata(struct ssp_data *data, s16 *iCalData)
{
	...
	send_buf[0] = (int)data->gyrocal.x;
	send_buf[1] = (int)data->gyrocal.y;
	send_buf[2] = (int)data->gyrocal.z;

	iRet = vfs_write(cal_filp, (char *)send_buf, 3 * sizeof(int), &cal_filp->f_pos);

	ssp_dbg("[SSP]:gyro calibrate write_buf %d, %d, %d %d\n",
	...
	struct device_attribute *attr, char *buf)
{
	...
	msg->free_buffer = 0;
#if 0
	iRet = ssp_ipc_sync(data, msg, 15000);
#else
	iRet = ssp_send_command(data, msg->cmd, msg->options, 15000,
													msg->buffer, msg->length, &buffer, &buffer_length, msg->data);
#endif
