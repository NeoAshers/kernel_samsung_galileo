    ...
    switch(param_data->state){
        case NAD_PARAM_WRITE:
            ...
            NAD_PRINT(
                    ...
                    sec_nad_env.nad_dram_test_count);
        ...
        const char *buf, size_t count)
{
    ...
    if (!strncmp(buf, "nad_acat", 8)) {
        ...
        /* case 2 : DRAM test */
        if (kstrtol(nad_cmd[2], 10, &dram_test_count) < 0)
            return -EINVAL;

        if (dram_test_count > 0) {
            ...
            sec_nad_env.nad_dram_test_count = dram_test_count;
        } else {
...
static ssize_t store_dram_test_count(struct device *dev,
        struct device_attribute *attr,
        const char *buf, size_t count)
{
    int ret = -1;
    ...
    if ( kstrtol(buf, 10, &dram_test_count) < 0)
        return -EINVAL;
...
static ssize_t show_dram_test_count(struct device *dev,
        struct device_attribute *attr,
        char *buf)
{
    NAD_PRINT("%s\n", __func__);

    return sprintf(buf, "%d", sec_nad_env.nad_dram_test_count);
}

static DEVICE_ATTR(dram_test_count, S_IRUGO | S_IWUSR, show_dram_test_count, store_dram_test_count);
...
static int __init sec_nad_init(void)
{
    ...
    ret = device_create_file(sec_nad, &dev_attr_dram_test_count);
    if(ret) {
        pr_err("%s: Failed to create device file\n", __func__);
        goto err_create_nad_sysfs;
    }
