...
#include "../../sensorhub/slsi_renaissance/ssp_platform.h"
#endif
...
#ifdef CONFIG_SENSORHUB_IPC_DEBUG
...
{
	char buf[6];
	char log_str[BIG_DATA_SIZE] ={0,};
	int log_size;
	int i;

	pr_err("[SSP]: %s - %s (%d)\n", __func__, func_name, length);
...
void chub_dbg_printk_gpr(struct contexthub_ipc_info *ipc)
{
	int i;
	...
	if (p_dbg_dump) {
		...
		pr_info("========================================\n");
		pr_info("CHUB CPU register dump\n");
			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	if (!ret)
		index += snprintf(buf, PAGE_SIZE, "chub alive\n");
	else
		index += snprintf(buf, PAGE_SIZE, "chub isn't alive\n");
			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < sizeof(chub_utc_name) / SIZE_UTC_NAME; i++)
		if (chub_utc_name[i][0])
			index +=
			    snprintf(buf + index, PAGE_SIZE, "%d %s\n", i,
				    chub_utc_name[i]);
...
#define TEST_PACKET_SIZE_MAX (128)
static ssize_t chub_ipc_store(struct device *dev,
			      struct device_attribute *attr,
			      const char *buf, size_t count)
{
	struct contexthub_ipc_info *ipc = dev_get_drvdata(dev);
	char input[TEST_PACKET_SIZE_MAX];
	char output[TEST_PACKET_SIZE_MAX];
	int ret;

	memset(input, 0, TEST_PACKET_SIZE_MAX);
	memset(output, 0, TEST_PACKET_SIZE_MAX);

	if (count <= TEST_PACKET_SIZE_MAX) {
		memset(input, 0, TEST_PACKET_SIZE_MAX);
		memcpy(input, buf, count);
	} else {
		dev_err(ipc->dev, "%s: ipc size(%d) is bigger than max(%d)\n",
			__func__, (int)count, (int)TEST_PACKET_SIZE_MAX);
		return -EINVAL;
	}
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	if (p_dbg_dump) {
		...
		pbuf +=
		    snprintf(pbuf, PAGE_SIZE, "========================================\n");
		pbuf += snprintf(pbuf, PAGE_SIZE,"CHUB CPU register dump\n");

		for (i = 0; i <= 15; i++)
			pbuf +=
			    snprintf(pbuf, PAGE_SIZE, "R%02d        : %08x\n", i,
				    p_dbg_dump->gpr[i]);

		pbuf +=
		    snprintf(pbuf, PAGE_SIZE, "PC         : %08x\n",
			    p_dbg_dump->gpr[GPR_PC_INDEX]);
		pbuf +=
		    snprintf(pbuf, PAGE_SIZE, "========================================\n");
	}
