...
void ipc_set_chub_bootmode(u16 bootmode)
{
	...
	if (bootmode == BOOTMODE_PWRGATING)
		map->pd_cnt++;
	else if (bootmode == 0)
		map->pu_cnt++;
	/* update sleep count */
...
void *ipc_get_chub_map(void)
{
	...
	if (sizeof(struct chub_bootargs) > MAP_INFO_MAX_SIZE) {
		CSP_PRINTF_ERROR
		    ...
		    (u32)sizeof(struct chub_bootargs), (u32)MAP_INFO_MAX_SIZE);
		return 0;
	}
	...
	CSP_PRINTF_INFO
		("%s: contexthub shared mem information: share base(%p %d)\n\t a2c(%p %d)\n\t c2a(%p %d)\n",
		NAME_PREFIX,
		ipc_addr[IPC_REG_SHARED].base, ipc_addr[IPC_REG_SHARED].offset,
...
/* 
	WA from S.LSI boojin.kim 2019.06.05
	Chub clk is not good enough to read IPC data from the AP without delay.
	It shound be removed before SRA.
*/
	msleep(1);
...
void ipc_init(void)
{
	...
	if (!ipc_map){
		/* WID:3216362 After having been compared to NULL value at chub_ipc.c:402,
		pointer 'ipc_map' is dereferenced at chub_ipc.c:406. */
