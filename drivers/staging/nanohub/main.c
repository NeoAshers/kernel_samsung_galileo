				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "WAKEUP: %d INT1: %d INT2: %d\n",
			atomic_read(&pdata->wakeup_chub),
			atomic_read(&pdata->irq1_apInt), -1);
#else
	return scnprintf(buf, PAGE_SIZE, "WAKEUP: %d INT1: %d INT2: %d\n",
				    ...
				    const char *buf, size_t count)
{
	...
	contexthub_ipc_write_event(data, MAILBOX_EVT_ERASE_SHARED);
...
{
#ifdef CONFIG_NANOHUB_MAILBOX
	return contexthub_poweron(data);
#else
	const struct nanohub_platform_data *pdata = data->pdata;
			     ...
			     size_t length, loff_t *offset)
{
	...
	if (atomic_read(&data->pdata->chub_status) != CHUB_ST_RUN) {
		dev_warn(data->io[ID_NANOHUB_SENSOR].dev, "%s fails. nanohub isn't running\n", __func__);
		return -EINVAL;
	}
#endif
	...
	ret = request_wakeup_timeout(data, WAKEUP_TIMEOUT_MS);
	if (ret) {
		DEBUG_PRINT(KERN_ERR, "WT timeout:%d\n", ret);
		return ret;
	}
