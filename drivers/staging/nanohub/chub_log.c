...
void log_printf(const char *format, ...)
{
	...
	if (print_info) {
		...
		/* Use of vulnerable function 'vsprintf'. This function is unsafe, use vsnprintf instead. */
		size = vsnprintf(tmp_buf, 512, format, args);
		va_end(args);
