...
static int therm_history_is_charging(void)
{
	...
	if (!psy) {
		pr_warn("%s: fail to get battery power supply\n", __func__);
		return 0;
	}

	if (power_supply_get_property(psy, POWER_SUPPLY_PROP_STATUS, &value)) {
		pr_warn("%s: fail to get battery property(%d)\n", __func__,
...
void sec_therm_history_update(int device_num, int temp)
{
	...
	for(group_num = 0; group_num < MAX_SEC_THERM_GROUP_NUM; group_num++)
	{
		...
		if (therm_history_is_charging()) {
			if (group_num == SEC_THERM_HISTORY_BOOT_DISCHARGE)
				continue;
		} else {
			if (group_num == SEC_THERM_HISTORY_BOOT_CHARGE)
				continue;
		}
...
static int therm_history_show(struct seq_file *m, void *unused)
{
	...
	for (i = 0; i < MAX_SEC_THERM_DEVICE_NUM; i++) {
		if (!history_table[i]) {
			...
			ret += snprintf(buf + ret, sizeof(buf),	"/%3d/%3d/%3d/%7d/%5d",
				history_table[i][SEC_THERM_HISTORY_BOOT].min,
				history_table[i][SEC_THERM_HISTORY_BOOT].max,
				history_table[i][SEC_THERM_HISTORY_BOOT].sum /
				history_table[i][SEC_THERM_HISTORY_BOOT].cnt,
				history_table[i][SEC_THERM_HISTORY_BOOT].sum,
				history_table[i][SEC_THERM_HISTORY_BOOT].cnt);
		}
	}
	...
	seq_printf(m, "[sec_therm_history_charge]\n");
	for (i = 0; i < MAX_SEC_THERM_DEVICE_NUM; i++)
		ret += snprintf(buf + ret, sizeof(buf),
				"/_____ %12s ______", sec_therm_dev_name[i]); /* len : 25 */
	ret += snprintf(buf + ret, sizeof(buf), "\n");
	...
	for (i = 0; i < MAX_SEC_THERM_DEVICE_NUM; i++) {
		if (!history_table[i]) {
			...
			ret += snprintf(buf + ret, sizeof(buf),	"/%3d/%3d/%3d/%7d/%5d",
				history_table[i][SEC_THERM_HISTORY_BOOT_CHARGE].min,
				history_table[i][SEC_THERM_HISTORY_BOOT_CHARGE].max,
				history_table[i][SEC_THERM_HISTORY_BOOT_CHARGE].sum /
				history_table[i][SEC_THERM_HISTORY_BOOT_CHARGE].cnt,
				history_table[i][SEC_THERM_HISTORY_BOOT_CHARGE].sum,
				history_table[i][SEC_THERM_HISTORY_BOOT_CHARGE].cnt);
		}
	}
	...
	seq_printf(m, "[sec_therm_history_discharge]\n");
	for (i = 0; i < MAX_SEC_THERM_DEVICE_NUM; i++)
		ret += snprintf(buf + ret, sizeof(buf),
				"/_____ %12s ______", sec_therm_dev_name[i]); /* len : 25 */
	ret += snprintf(buf + ret, sizeof(buf), "\n");
	...
	for (i = 0; i < MAX_SEC_THERM_DEVICE_NUM; i++) {
		if (!history_table[i]) {
			...
			ret += snprintf(buf + ret, sizeof(buf),	"/%3d/%3d/%3d/%7d/%5d",
				history_table[i][SEC_THERM_HISTORY_BOOT_DISCHARGE].min,
				history_table[i][SEC_THERM_HISTORY_BOOT_DISCHARGE].max,
				history_table[i][SEC_THERM_HISTORY_BOOT_DISCHARGE].sum /
				history_table[i][SEC_THERM_HISTORY_BOOT_DISCHARGE].cnt,
				history_table[i][SEC_THERM_HISTORY_BOOT_DISCHARGE].sum,
				history_table[i][SEC_THERM_HISTORY_BOOT_DISCHARGE].cnt);
		}
	}
