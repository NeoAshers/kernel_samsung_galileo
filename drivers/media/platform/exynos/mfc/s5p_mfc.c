...
static int s5p_mfc_open(struct file *file)
{
	...
	mfc_info_dev("dev->num_inst increased: %d\n", dev->num_inst);
	...
	strncpy(ctx->process_name_owner, current->comm, sizeof(ctx->process_name_owner) - 1);
	ctx->process_name_owner[sizeof(ctx->process_name_owner) - 1] = '\0';
	ctx->process_id_owner = task_pid_nr(current);

	mfc_info_ctx("MFC open completed [%d:%d] dev = %p, ctx = %p, version = %d, process: %d (%s)\n",
			dev->num_drm_inst, dev->num_inst, dev, ctx, MFC_DRIVER_INFO,
			ctx->process_id_owner, ctx->process_name_owner);
...
err_ctx_alloc:
	dev->num_inst--;
	mfc_info_dev("err_ct_alloc: dev->num_inst decreased: %d\n", dev->num_inst);
...
static int s5p_mfc_release(struct file *file)
{
	...
	mfc_info_ctx("MFC driver release is called [%d:%d], is_drm(%d), process: %d (%s)\n",
			dev->num_drm_inst, dev->num_inst,
			ctx->is_drm, ctx->process_id_owner, ctx->process_name_owner);
	...
	if (ctx->is_drm)
		dev->num_drm_inst--;
	dev->num_inst--;
	mfc_info_dev("dev->num_inst decreased: %d\n", dev->num_inst);
		...
		unsigned long addr, int id, void *param)
{
	...
	/* If ID[1:0] is 0x0 in SYSMMU fault info, it is MFC page fault */
	if (IS_MFCV1021(dev) &&
			((MFC_MMU_READL(MFC_MMU_FAULT_TRANS_INFO) &
			 MFC_MMU_FAULT_TRANS_INFO_AXID_MASK) != 0x0)) {
...
static int s5p_mfc_suspend(struct device *dev)
{
	...
	mfc_info_dev ("number of mfc device instances: %d\n", m_dev->num_inst);
	if (m_dev->num_inst == 0)
		return 0;
