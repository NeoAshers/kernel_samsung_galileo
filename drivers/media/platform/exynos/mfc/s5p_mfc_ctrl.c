...
int s5p_mfc_sleep(struct s5p_mfc_dev *dev)
{
	...
	// log for processes owning a device context
	for (i = 0; i < MFC_NUM_CONTEXTS; i++) {
		if (dev->ctx[i]) {
			mfc_info_dev("opened device #%2d: owned by process: %d (%s)\n",
				i, dev->ctx[i]->process_id_owner, dev->ctx[i]->process_name_owner);
			count_ctx++;
		}
	}
	mfc_info_dev("Total number of opened devices: %d", count_ctx);
