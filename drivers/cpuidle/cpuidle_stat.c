...
static u64 emon_cpus_busy_stat;
static u64 emon_cp_busy_stat;
static u64 emon_jig_block_stat;
static char *etc_blocker[] = {
	"cpu", "cp", "jig"
};

int cpuidle_stats_get_stats(struct emon_cpuidle_stat *cpuidle_stat)
{
	...
	now = ktime_get();
	cpuidle_stat->total_stat_time = now;
		...
		for_each_possible_cpu(cpu) {
			...
			if (cpu < 2) {
				cpuidle_stat->cpuidle[cpu].usage[i].entry_count =
					stat->usage[i].entry_count;
				cpuidle_stat->cpuidle[cpu].usage[i].early_wakeup_count =
					stat->usage[i].early_wakeup_count;
				cpuidle_stat->cpuidle[cpu].usage[i].time =stat->usage[i].time;

				online_time = stat->hp.online_time;
				if (stat->hp.up) {
					duration = ktime_sub(now, stat->hp.last_onlie_time);
					online_time = ktime_add(online_time, duration);
				}
				cpuidle_stat->cpuidle[cpu].hp.online_time = online_time;
			}
		}
	}
	...
	for (i = 0; i < 1; i++) {
		...
		cpuidle_stat->lpm.usage.entry_count = stat->usage[i].entry_count;
		cpuidle_stat->lpm.usage.early_wakeup_count =
			stat->usage[i].early_wakeup_count;
		cpuidle_stat->lpm.usage.time = stat->usage[i].time;
	}

	return 0;
}

int cpuidle_stats_get_stats_emon(int type,
					struct emon_cpuidle_stat *cpuidle_stat)
{
	...
	online_time = ktime_sub(now, cpuidle_stat_start_time);
	per_cpu(idle_stat, 0).hp.online_time = online_time;
	...
	for_each_possible_cpu(cpu) {
		...
		if (cpu < 2) {
			for (i = 0; i < state_count; i++) {
				...
				cpuidle_stat->cpuidle[cpu].usage[i].early_wakeup_count =
					stat->usage[i].early_wakeup_count -
					emon->usage[i].early_wakeup_count;
				cpuidle_stat->cpuidle[cpu].usage[i].time =
			...
			online_time =
				ktime_sub(stat->hp.online_time, emon->hp.online_time);
			if (stat->hp.up)
				duration = ktime_sub(now, stat->hp.last_onlie_time);
			else
				duration = ktime_set(0,0);
			online_time = ktime_add(online_time, duration);
			cpuidle_stat->cpuidle[cpu].hp.online_time = online_time;

			if (type != ENERGY_MON_TYPE_DUMP) {
				pr_info("%s: %d: %lld %lld\n", __func__, cpu,
					ktime_to_ms(emon->hp.online_time),
					ktime_to_ms(cpuidle_stat->cpuidle[cpu].hp.online_time));
				emon->hp.online_time = ktime_add(stat->hp.online_time, duration);
				emon->hp.last_onlie_time = now;
			}
		}
	}
		...
		struct emon_lpm_blocker *blocker_array,  size_t n)
{
	...
	emon_count = cpus_busy_stat - emon_cpus_busy_stat;
	if (emon_count > 0 && emon_count > blocker_array[n-1].count) {
		blocker_array[n-1].name = etc_blocker[0];
		blocker_array[n-1].count = emon_count;
		sort(&blocker_array[0],
			n,
			sizeof(struct emon_lpm_blocker),
	...
	emon_count = cp_busy_stat - emon_cp_busy_stat;
	if (emon_count > 0 && emon_count > blocker_array[n-1].count) {
		blocker_array[n-1].name = etc_blocker[1];
		blocker_array[n-1].count = emon_count;
		sort(&blocker_array[0],
			n,
			sizeof(struct emon_lpm_blocker),
	...
	emon_count = jig_block_stat - emon_jig_block_stat;
	if (emon_count > 0 && emon_count > blocker_array[n-1].count) {
		blocker_array[n-1].name = etc_blocker[2];
		blocker_array[n-1].count = emon_count;
		sort(&blocker_array[0],
			n,
			sizeof(struct emon_lpm_blocker),
	...
	if (type != ENERGY_MON_TYPE_DUMP) {
		emon_cpus_busy_stat = cpus_busy_stat;
		emon_cp_busy_stat = cp_busy_stat;
		emon_jig_block_stat = jig_block_stat;
	}
				     ...
				     char *buf)
{
	...
	s64 temp_time_ms;
	s64 online_time_ms;
	ktime_t cpuidle_stat_total_time;
	ktime_t duration, online_time;
	ktime_t now = ktime_get();
	...
	cpuidle_stat_total_time = ktime_sub(now, cpuidle_stat_start_time);
	cpuidle_stat_total_time_ms = ktime_to_ms(cpuidle_stat_total_time);
	...
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
		"[total idle ratio]\n");
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
		"#cpu  #time(ms)    #ratio #online(ms)\n");
	per_cpu(idle_stat, 0).hp.online_time = cpuidle_stat_total_time;

	for_each_possible_cpu(cpu) {
		info = &per_cpu(idle_stat, cpu);
		temp_time_ms = ktime_to_ms(sum_idle_time(info));
		online_time = info->hp.online_time;
		if (info->hp.up) {
			duration = ktime_sub(now, info->hp.last_onlie_time);
			online_time = ktime_add(online_time, duration);
		}
		online_time_ms = ktime_to_ms(online_time);
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"cpu%d  %-12lld %3u%%   %-12lld\n",
			cpu,
			temp_time_ms,
			calculate_percent(temp_time_ms, online_time_ms),
			online_time_ms);
	}
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "\n");
	...
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
		"[LPM] - Low Power Mode\n");
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
		"#mode    #entry       #early       #time(ms)     #ratio\n");
...
static int cpuidle_stat_hp_callback(struct notifier_block *nfb,
					unsigned long action, void *hcpu)
{
	...
	if (info) {
		pr_debug("%s: (%d:0x%04x)\n", __func__, cpu, (int)action);
		now = ktime_get();
		switch (action) {
		case CPU_ONLINE:
		case CPU_ONLINE_FROZEN:
			info->hp.up = true;
			info->hp.last_onlie_time = now;
			info->hp.online_count++;
			break;
		case CPU_DEAD:
		case CPU_DEAD_FROZEN:
			if (info->hp.up) {
				info->hp.up = false;
				duration = ktime_sub(now, info->hp.last_onlie_time);
				info->hp.online_time = ktime_add(info->hp.online_time, duration);
				info->hp.last_onlie_time = now;
				pr_debug("%s: cpu%d %lld %lld %u\n",
					__func__, cpu, ktime_to_ms(info->hp.online_time),
					ktime_to_ms(duration), info->hp.online_count);
			}
			break;
		}
	}
...
static struct notifier_block __refdata cpuidle_stat_hp_notifier = {
	.notifier_call = cpuidle_stat_hp_callback,
	.priority = INT_MIN,
};
 ...
 *********************************************************************/
static void __init cpuidle_stat_info_init(
					struct cpuidle_stat_info *info, int state_count)
{
...
void __init cpuidle_stat_register(struct cpuidle_driver *drv)
{
	...
	for_each_possible_cpu(i) {
		cpuidle_stat_info_init(&per_cpu(idle_stat, i), idle_state_count);
		/* force up cpu except the boot cpu */
		if (i)
			per_cpu(idle_stat, i).hp.up = true;
#ifdef CONFIG_ENERGY_MONITOR
		cpuidle_stat_info_init(&per_cpu(emon_idle_stat, i), idle_state_count);
#endif
	}
...
static int __init cpuidle_stat_init(void)
{
	...
	dev = device_create(class, NULL, 0, NULL, "exynos");
	...
	register_hotcpu_notifier(&cpuidle_stat_hp_notifier);
