...
static void dw_mci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_POWER_UP:
#ifdef CONFIG_MMC_DW_EMMC_POWERCTRL
		if (host->dev->of_node) {
			int ctrl_id;
...
static irqreturn_t dw_mci_interrupt(int irq, void *dev_id)
{
	...
	u32 status, pending;
	int i;
	struct mmc_request *mrq = host->cur_slot->mrq;
...
#if defined(CONFIG_BCM43012) || defined(CONFIG_BCM43013)
...
#endif /* CONFIG_BCM43012 || CONFIG_BCM43013 */
...
#ifdef CONFIG_MMC_DW_EMMC_POWERCTRL
	if ((!(of_property_read_u32(np, "emmc-pwr-suspend", &pdata->emmc_pwr.mmc_pwr_ctrl))) &&
			(pdata->emmc_pwr.mmc_pwr_ctrl == DT_OP_ENABLE)) {

		if (of_property_read_u32(np, "emmc-dis-charge-time", &pdata->emmc_pwr.dis_charge))
			pdata->emmc_pwr.dis_charge = 30;

		/* enable once to prevent power off use_cnt 0 regulators */
		pdata->emmc_pwr.vemmc = devm_regulator_get_optional(dev, "vemmc");
		if (!IS_ERR(pdata->emmc_pwr.vemmc)) {
			ret = regulator_enable(pdata->emmc_pwr.vemmc);
			if (ret) {
				dev_err(host->dev, "vemmc regulators enable failed\n");
		...
		pdata->emmc_pwr.vqemmc = devm_regulator_get_optional(dev, "vqemmc");
		if (!IS_ERR(pdata->emmc_pwr.vqemmc)) {
			ret = regulator_enable(pdata->emmc_pwr.vqemmc);
			if (ret) {
				dev_err(host->dev, "vqemmc regulators enable failed\n");
		...
		dev_info(dev, "vemmc, vqemmc registration completed\n");
	}
#endif
#if defined(CONFIG_BCM43012) || defined(CONFIG_BCM43013)
...
int dw_mci_suspend(struct dw_mci *host)
{
	...
	if (host->use_dma && host->dma_ops->exit)
		host->dma_ops->exit(host);
#ifdef CONFIG_MMC_DW_EMMC_POWERCTRL
	if (host->pdata->emmc_pwr.mmc_pwr_ctrl)
