...
#define MHZ (1000 * 1000)
	...
	defined(CONFIG_RENAISSANCE) || defined(CONFIG_NOBLESSE)
struct device *mmc_dev_for_wlan = NULL;
EXPORT_SYMBOL(mmc_dev_for_wlan);
#endif /* CONFIG_GALILEO || CONFIG_PULSE || CONFIG_RENAISSANCE || CONFIG_NOBLESSE */
...
#if defined(CONFIG_GALILEO) || defined(CONFIG_PULSE) || \
	defined(CONFIG_RENAISSANCE) || defined(CONFIG_NOBLESSE)
		/* In case of GALILEO, dwmmc2 is SDIO for Broadcom Wi-Fi chipset */
		if (id == 2)
		...
		struct device_attribute *attr, char *buf)
{
	...
	len = snprintf(buf, PAGE_SIZE,
		...
		/* Because Exynos9110 doesn't support Command Queue, we set CQ halt error & CQ Enable error count to 0 */
out:
	return len;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (card) {
		...
		/* SUMMARY */
		sprintf(buf, "\"REV\":\"%#x%x%x%x\",\"SIZE\":\"%s\"\n",
		...
		return sprintf(buf, "\"REV\":\"NoCard\",\"SIZE\":\"NoCard\"\n");
	}
}
