...
#if 0 /* Exynos9110 doesn't support Command Queue Support CONFIG_MMC_CQ_HCI */
...
static void mmc_rpmb_switch_error_logging(struct mmc_card *card, u32 status)
{
	int index = 0;
	struct mmc_card_error_log *err_log = card->err_log;
				      ...
				      struct mmc_blk_data *md)
{
	...
	if (mmc_card_mmc(card)) {
		...
		ret = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
				 ...
				 card->ext_csd.part_time);
		if (ret) {
			mmc_rpmb_switch_error_logging(card, RPMB_SWITCH_ERR);
			if (md->part_type == EXT_CSD_PART_CONFIG_ACC_RPMB)
	...
	struct mmc_blk_request *brq, int *ecc_err, int *gen_err)
{
	...
	if (err) {
		...
		if (mmc_detect_card_removed(card->host))
			return ERR_NOMEDIUM;
		pr_err("%s: abort: suspended:%u, opcode:%u, req_op:%u\n",
			req->rq_disk->disk_name, mmc_card_suspended(card),
			brq->cmd.opcode, (u32)req_op(req));
		return ERR_ABORT;
	}
