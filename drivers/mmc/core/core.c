...
#define MMC_INT_MIN_FREQ	(598000)	/* kHz  */
#define MMC_INT_QOS_LOCK_TIME	(100)		/* ms   */
#define MMC_INT_SIZE_THRESHOLD	(1*1024*1024)	/* byte */

static struct pm_qos_request mmc_qos_request;
...
static int mmc_start_request(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if (mrq->data) {
		...
		if(mrq->data->blocks * mrq->data->blksz >= MMC_INT_SIZE_THRESHOLD) {
			pm_qos_update_request_timeout(&mmc_qos_request,
					MMC_INT_MIN_FREQ,
					MMC_INT_QOS_LOCK_TIME * USEC_PER_MSEC);
		}
	}
...
static int __init mmc_init(void)
{
	...
	pm_qos_add_request(&mmc_qos_request, PM_QOS_CLUSTER0_FREQ_MIN, 0);
...
{
	pm_qos_remove_request(&mmc_qos_request);
	sdio_unregister_bus();
	mmc_unregister_host_class();
	mmc_unregister_bus();
}
