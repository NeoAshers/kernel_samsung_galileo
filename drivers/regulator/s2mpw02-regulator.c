...
int s2m_ldo15_set_mode(unsigned int val)
{
	...
	ret = s2mpw02_update_reg(static_info->i2c, S2MPW02_PMIC_REG_L15CTRL, val, 0xC0);
	if (ret)
		return ret;
					...
					unsigned sel)
{
	int ret;
	struct s2mpw02_info *s2mpw02 = rdev_get_drvdata(rdev);
	u8 out_reg1 = 0, out_reg2 = 0;

	if (!strcmp("BUCK1", rdev->desc->name)) {
		ret = s2mpw02_write_reg(s2mpw02->i2c, rdev->desc->vsel_reg, sel);
		if (ret < 0)
			goto out;
	} else {
		if (!strcmp("BUCK2", rdev->desc->name)) {
			out_reg1 = S2MPW02_PMIC_REG_B2OUT1;
			out_reg2 = S2MPW02_PMIC_REG_B2OUT2;
		} else if (!strcmp("BUCK3", rdev->desc->name)) {
			out_reg1 = S2MPW02_PMIC_REG_B3OUT1;
			out_reg2 = S2MPW02_PMIC_REG_B3OUT2;
		} else if (!strcmp("BUCK4", rdev->desc->name)) {
			out_reg1 = S2MPW02_PMIC_REG_B4OUT1;
			out_reg2 = S2MPW02_PMIC_REG_B4OUT2;
		}

		ret = s2mpw02_write_reg(s2mpw02->i2c, out_reg1, sel);
		if (ret < 0) {
			pr_err("%s: OUT1 reg write failed\n", rdev->desc->name);
			goto out;
		}
		ret = s2mpw02_write_reg(s2mpw02->i2c, out_reg2, sel);
		if (ret < 0) {
			pr_err("%s: OUT2 reg write failed\n", rdev->desc->name);
			goto out;
		}
...
u8 s2mpw02_get_pmic_revision(void)
{
    return static_info->iodev->pmic_rev;
}
EXPORT_SYMBOL_GPL(s2mpw02_get_pmic_revision);
