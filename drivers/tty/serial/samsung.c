...
#define BT_LOG_BUFFER_SIZE (0x200000) /* Allocate 2MB of buffer */
#define BT_TX_RX_BUFFER_SIZE 256
...
#define BLUETOOTH_UART_PORT_LINE CONFIG_BT_UART_PORT
#else
#define BLUETOOTH_UART_PORT_LINE 1
#endif

/* This patch fix UART suspend & resume problem */
#if 1
#define PATCH_UDIVSLOT_CALCULATION
#define PATCH_FIRST_DIV_NEXT_AFC
#define PATCH_FIRST_FIFO_RESET_NEXT_CHANNEL_ENABLE
#define PATCH_WAIT_TX_FIFO_DONE_WHEN_SICD_ENTER
#define PATCH_MASK_INTERRUPT_BEFORE_CHANNEL_ENABLE
#define PATCH_MOVE_SERIAL_RESETPORT_POSITION
#endif /* kwoo.kang@samsung.com */

#if 1
#define PATCH_RESET_BOTH_FIFO
#endif /* donghyeon.kim@samsung.com */

#if 1
//#define TRACE_UART_UDIVSLOT_FIND
#define TRACE_UART_SUSPEND_RESUME
//#define TRACE_UART_SICD_NOTIFIER
//#define TRACE_UART_TX_CIRC_QUEUE
#endif /* logging */
...
#ifdef TRACE_UART_SUSPEND_RESUME
static ssize_t
uart_pm_dbg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;

	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"UART PM Debug Mode Configuration.\n");
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"0 : Change to Normal mode.\n");
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
...
static ssize_t
uart_pm_dbg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int input_cmd = 0, ret;
	struct s3c24xx_uart_port *ourport;
	...
	list_for_each_entry(ourport, &drvdata_list, node) {
		...
		switch(input_cmd) {
		case 0:
			dev_notice(port->dev, "[%s(%d)] Change UART PM to Normal Mode\n", __FUNCTION__, __LINE__);
			ourport->pm_dbg_mode = 0;
			break;
		case 1:
			dev_notice(port->dev, "[%s(%d)] Change UART PM to Debug Mode\n", __FUNCTION__, __LINE__);
			ourport->pm_dbg_mode = UART_DBG_MODE;
			break;
		default:
...
static DEVICE_ATTR(uart_pm_dbg, 0640, uart_pm_dbg_show, uart_pm_dbg_store);
#endif /* logging */

#if defined(CONFIG_BT_DQA_SUPPORT) /* && defined(CONFIG_TIZEN_WIP) */
static ssize_t
uart_err_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret=0;
	struct s3c24xx_uart_port *ourport;
	...
	list_for_each_entry(ourport, &drvdata_list, node) {
		...
		//overrun, parity, frame, break
#if 0
		ret = sprintf(buf, "%u %u %u %u\n%s\n", port->icount.overrun, 0, port->icount.frame, port->icount.brk, "<overrun, parity, frame, break>");
#else
		ret = sprintf(buf, "\"UART_OVERRUN\":\"%u\",\"UART_PARITY\":\"%u\",\"UART_FRAME\":\"%u\",\"UART_BREAK\":\"%u\"\n", port->icount.overrun, port->icount.parity, port->icount.frame, port->icount.brk);
#endif
	}
	return ret;
}

static DEVICE_ATTR(uart_err, 0444, uart_err_show, NULL);
#endif /* */
...
s3c24xx_serial_rx_chars(int irq, void *dev_id)
{
...
#ifdef BT_UART_TRACE
	unsigned char ch_str[BT_TX_RX_BUFFER_SIZE] = {0, };
	int ch_str_cnt = 0;
#endif
	spin_lock_irqsave(&port->lock, flags);
	...
	while (max_count-- > 0) {
...
#if 1 /* defined(CONFIG_TIZEN_WIP) */
		if (unlikely(uerstat & S3C2443_UERSTAT_PARITY)) {
			port->icount.parity++;
			dev_notice_ratelimited(port->dev, "[%s(%d)] UART Parity Error!(%u times)\n", __FUNCTION__, __LINE__, port->icount.parity);
		}
#endif
		...
		if (unlikely(uerstat & S3C2410_UERSTAT_ANY)) {
			...
			if (uerstat & S3C2410_UERSTAT_BREAK) {
				...
				port->icount.brk++;
#if 1 /* defined(CONFIG_TIZEN_WIP) */
				dev_notice_ratelimited(port->dev, "[%s(%d)] UART Break Error!(%u times)\n", __FUNCTION__, __LINE__, port->icount.brk);
#endif
...
#if 1 /* defined(CONFIG_TIZEN_WIP) */
				dev_notice_ratelimited(port->dev, "[%s(%d)] UART Frame Error!(%u times)\n", __FUNCTION__, __LINE__, port->icount.frame);
#else
...
#if 1 /* defined(CONFIG_TIZEN_WIP) */
				dev_notice_ratelimited(port->dev, "[%s(%d)] UART Overrun Error!(%u times)\n", __FUNCTION__, __LINE__, port->icount.overrun);
#else
 ...
 ignore_char:
	...
	spin_unlock_irqrestore(&port->lock, flags);
	/* Deliver gathered multi bytes to upper layer for enhancing Rx */
 ...
 out:
	...
	if (ourport->port.line == BLUETOOTH_UART_PORT_LINE) {
		...
		uart_copy_local_buf(2, &ourport->local_buf, uart_log_buf, sizeof(uart_log_buf));
	}
#endif
	return IRQ_HANDLED;
}
...
#ifdef TRACE_UART_TX_CIRC_QUEUE
	int is_uart_circ_empty;
	int is_uart_tx_stopped;
#endif /* logging */
...
#ifdef TRACE_UART_TX_CIRC_QUEUE
	if ((ourport->dbg_mode & UART_DBG_MODE) && (ourport->port.line == BLUETOOTH_UART_PORT_LINE)) {
		/* check available fifo space */
		if(count == 0)
			dev_notice(port->dev, "[%s(%d)] fifo is already full!\n", __FUNCTION__, __LINE__);
	}
#endif /* logging */
...
#ifdef TRACE_UART_TX_CIRC_QUEUE
	if ((is_uart_circ_empty = uart_circ_empty(xmit)) || (is_uart_tx_stopped = uart_tx_stopped(port))) {
		if ((ourport->dbg_mode & UART_DBG_MODE) && (ourport->port.line == BLUETOOTH_UART_PORT_LINE)) {
			dev_notice(port->dev, "[%s(%d)] uart_circ_empty:%d, uart_tx_stopped:%d, tx_enabled:%d\n", __FUNCTION__, __LINE__, is_uart_circ_empty, is_uart_tx_stopped, tx_enabled(port));
		}
#else
...
#ifdef BT_UART_TRACE
		if (ourport->port.line == BLUETOOTH_UART_PORT_LINE)
			ch_str[ch_str_cnt++] = (unsigned char)xmit->buf[xmit->tail];
#endif
...
#ifdef TRACE_UART_SUSPEND_RESUME
		if ((ourport->pm_dbg_mode & UART_DBG_MODE) && (ourport->port.line == BLUETOOTH_UART_PORT_LINE)) {
			dev_notice(port->dev, "[%s: %s(%d)] UART_PORT_SUSPEND(in_band_wakeup: %d)\n", __FILE__, __FUNCTION__, __LINE__, ourport->in_band_wakeup);
		}
#endif /* logging */
		break;
...
#endif
#ifdef PATCH_MOVE_SERIAL_RESETPORT_POSITION
...
#ifdef TRACE_UART_SUSPEND_RESUME
		if ((ourport->pm_dbg_mode & UART_DBG_MODE) && (ourport->port.line == BLUETOOTH_UART_PORT_LINE)) {
			dev_notice(port->dev, "[%s: %s(%d)] UART_PORT_RESUME(in_band_wakeup: %d)\n", __FILE__, __FUNCTION__, __LINE__, ourport->in_band_wakeup);
		}
#endif /* logging */
		break;
	default:
				       ...
				       struct ktermios *old)
{
	...
	unsigned int ulcon;
#ifdef PATCH_FIRST_DIV_NEXT_AFC
	unsigned int umcon;
#endif
	unsigned int udivslot = 0;
#ifdef PATCH_UDIVSLOT_CALCULATION
...
#ifdef TRACE_UART_UDIVSLOT_FIND
			dev_notice(port->dev, "[%s: %s(%d)] Find udivslot of the lowest error rate\n", __FILE__, __FUNCTION__, __LINE__);
#endif
		}
#endif /* kwoo.kang@samsung.com */
	...
	if (ourport->port.line == BLUETOOTH_UART_PORT_LINE) {
		...
		uart_copy_local_buf(3, &ourport->local_buf, uart_log_buf, sizeof(uart_log_buf));
	}
#endif
	spin_unlock_irqrestore(&port->lock, flags);
}
...
#ifdef PATCH_MASK_INTERRUPT_BEFORE_CHANNEL_ENABLE
	/* To prevent IRQ storm, Need to clear pending interrupt before enabling the channel */
	wr_regl(port, S3C64XX_UINTP, 0xf);

	/* To prevent IRQ storm, Need to mask interrupt before enabling the channel */
	/* To prevent unexpected Interrupt before enabling the channel */
	wr_regl(port, S3C64XX_UINTM, 0xf);
#endif /* kwoo.kang@samsung.com */

#ifndef PATCH_FIRST_FIFO_RESET_NEXT_CHANNEL_ENABLE
	/* Move from here */
	wr_regl(port, S3C2410_UCON,  ucon | cfg->ucon); /* enable channel */
#endif /* kwoo.kang@samsung.com */

#ifdef PATCH_RESET_BOTH_FIFO
	/* reset both fifos */
...
#ifdef PATCH_FIRST_FIFO_RESET_NEXT_CHANNEL_ENABLE
	/* Move to here */
	wr_regl(port, S3C2410_UCON,  ucon | cfg->ucon); /* enable channel */
#endif /* kwoo.kang@samsung.com */
}
...
#ifdef TRACE_UART_SICD_NOTIFIER
			if ((ourport->dbg_mode & UART_DBG_MODE) && (ourport->port.line == BLUETOOTH_UART_PORT_LINE)) {
				dev_notice(port->dev, "[%s: %s(%d)] %s\n", __FILE__, __FUNCTION__, __LINE__,	(cmd == SICD_ENTER)?"SICD_ENTER":
																						(cmd == SICD_AUD_ENTER)?"SICD_AUD_ENTER":"Unknown");
			}
#endif /* logging */
		}
#ifdef PATCH_WAIT_TX_FIFO_DONE_WHEN_SICD_ENTER
		s3c24xx_serial_fifo_wait();
#endif /* kwoo.kang@samsung.com */
		break;
...
#ifdef TRACE_UART_SICD_NOTIFIER
			if ((ourport->dbg_mode & UART_DBG_MODE) && (ourport->port.line == BLUETOOTH_UART_PORT_LINE)) {
				dev_notice(port->dev, "[%s: %s(%d)] %s\n", __FILE__, __FUNCTION__, __LINE__,	(cmd == SICD_EXIT)?"SICD_EXIT":
																						(cmd == SICD_AUD_EXIT)?"SICD_AUD_EXIT":"Unknown");
			}
#endif /* logging */
		}
		break;
...
#ifdef TRACE_UART_SUSPEND_RESUME
	ret = device_create_file(&pdev->dev, &dev_attr_uart_pm_dbg);
	if (ret < 0)
		dev_err(&pdev->dev, "failed to create sysfs file.\n");
#endif /* logging */

#if defined(CONFIG_BT_DQA_SUPPORT) /* && defined(CONFIG_TIZEN_WIP) */
	ret = device_create_file(&pdev->dev, &dev_attr_uart_err);
	if (ret < 0)
		dev_err(&pdev->dev, "failed to create sysfs file.\n");
#endif /* */
...
#if 1 /* defined(CONFIG_TIZEN_WIP) */
#if 0 /* def CONFIG_TIZEN_SEC_KERNEL_ENG */
	ourport->pm_dbg_mode = UART_DBG_MODE;
#else
	ourport->pm_dbg_mode = 0;
#endif /* CONFIG_TIZEN_SEC_KERNEL_ENG */
#endif
...
static int s3c24xx_serial_remove(struct platform_device *dev)
{
	...
	device_remove_file(&dev->dev, &dev_attr_uart_dbg);

#ifdef TRACE_UART_SUSPEND_RESUME
		device_remove_file(&dev->dev, &dev_attr_uart_pm_dbg);
#endif /* logging */

#if defined(CONFIG_BT_DQA_SUPPORT) /* && defined(CONFIG_TIZEN_WIP) */
		device_remove_file(&dev->dev, &dev_attr_uart_err);
#endif /* */
...
#ifdef TRACE_UART_SUSPEND_RESUME
			if ((ourport->pm_dbg_mode & UART_DBG_MODE) && (ourport->port.line == BLUETOOTH_UART_PORT_LINE)) {
				dev_notice(dev, "[%s: %s(%d)] resume_noirq\n", __FILE__, __FUNCTION__, __LINE__);
			}
#endif /* logging */
		}
	}
