...
const struct ztm620_info *g_ztm620_info;
...
static void run_jitter_acceleration_read(void *device_data);
...
static int ztm620_set_optional_mode(struct ztm620_info *info, bool force)
{
	...
	if (ret) {
		ret = ztm620_write_reg(info->client, ZTM620_WHEEL_VIB_SETTING, info->vib_mode);
		if (ret != I2C_SUCCESS) {
			dev_err(&info->client->dev,
...
#endif
	{TSP_CMD("run_jitter_acc_read", run_jitter_acceleration_read),},
...
static void get_chip_vendor(void *device_data)
{
	...
	scnprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", ZTM620_VENDOR_NAME);
	set_cmd_result(finfo, finfo->cmd_buff,
			strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
	finfo->cmd_state = OK;
...
static void get_chip_name(void *device_data)
{
	...
	if (info->chip_code == ZTM620_IC_CHIP_CODE)
		scnprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", ZTM620_CHIP_NAME);
	else if (info->chip_code == ZTM620_IC_CHIP_CODE)
		scnprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", ztm620_CHIP_NAME);
	else if (info->chip_code == ZT7538_IC_CHIP_CODE)
		scnprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", ZT7538_CHIP_NAME);
	else
		scnprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", ZTM620_VENDOR_NAME);
	set_cmd_result(finfo, finfo->cmd_buff,
			strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
...
static void run_jitter_acceleration_read(void *device_data)
{
	...
	ret = ztm620_set_touchmode(info, TOUCH_JITTER_ACCELERATION_MODE);
	if (ret) {
		dev_err(&client->dev, "%s:Failed to set TOUCH_JITTER_ACCELERATION_MODE\n", __func__);
		finfo->cmd_state = FAIL;
		scnprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "NG");
		goto out;
	}
	...
	for (j = 0; j < (info->cap_info.y_node_num*2); j++) {
		scnprintf(tmp, 16, "%d", raw_data->acc_test_data[j]);
		strncat(finfo->cmd_buff, tmp, strlen(tmp));
		if (j < (info->cap_info.y_node_num*2) - 1)
			strncat(finfo->cmd_buff, " ", strlen(" "));
	}
				  ...
				  const char *buf, size_t size)
{
	...
	ret = ztm620_set_optional_mode(info, true);
	if (ret) {
		dev_err(&client->dev, "%s:failed ztm620_set_optional_mode\n", __func__);
		return -EIO;
	}
...
static ssize_t ztm620_store_read_reg(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t size)
{
	struct ztm620_info *info = dev_get_drvdata(dev);
...
static ssize_t ztm620_show_read_reg(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct ztm620_info *info = dev_get_drvdata(dev);
	...
	ret = ztm620_read_data(info->client, (u16)read_reg_addr, (u8 *)&r_buff[0], read_reg_size);
	if (ret < 0) {
		dev_err(&client->dev, "%s:failed read params[%d]\n", __func__, ret);
		return -EIO;
	}
...
static ssize_t ztm620_store_write_reg(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t size)
{
	struct ztm620_info *info = dev_get_drvdata(dev);
	...
	ret = ztm620_write_reg(info->client, (u16)buff[0], (u16)buff[2]);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev,
			"%s:Failed to write ZTM620_REGISTER_WRITE(%x)(%x)\n",
			__func__, buff[0], buff[1]);
		return -EIO;
	}
...
static ssize_t ztm620_show_hard_covery_cnt(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm620_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static ssize_t ztm620_show_gesture_time(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm620_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static ssize_t ztm620_show_normal_time(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm620_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static DEVICE_ATTR(gesture_time, S_IRUGO, ztm620_show_gesture_time, NULL);
static DEVICE_ATTR(normal_time, S_IRUGO, ztm620_show_normal_time, NULL);
...
static DEVICE_ATTR(recovery_count, S_IRUGO, ztm620_show_hard_covery_cnt, NULL);
...
static DEVICE_ATTR(read_reg, S_IRUGO | S_IWUSR | S_IWGRP,\
		ztm620_show_read_reg, ztm620_store_read_reg);
static DEVICE_ATTR(write_reg, S_IWUSR | S_IWGRP,\
		NULL, ztm620_store_write_reg);
#endif
#if SUPPORTED_TOUCH_WHEEL
...
#if SUPPORTED_TOUCH_WHEEL
static int ztm620_bezel_open(struct input_dev *dev)
{
	struct ztm620_info *info = input_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static void ztm620_bezel_close(struct input_dev *dev)
{
	struct ztm620_info *info = input_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
				  ...
				  const char *buf, size_t size)
{
	...
	zinitix_bit_clr(info->vib_mode, BIT_WHEEL_VIB_MODE_0);
	zinitix_bit_clr(info->vib_mode, BIT_WHEEL_VIB_MODE_1);

	if (value == VIB_LIGHT) {
		zinitix_bit_set(info->vib_mode, BIT_WHEEL_VIB_MODE_0);
	} else if (value == VIB_MEDIUM) {
		zinitix_bit_set(info->vib_mode, BIT_WHEEL_VIB_MODE_1);
	} else if (value == VIB_STRONG) {
		zinitix_bit_set(info->vib_mode, BIT_WHEEL_VIB_MODE_0);
		zinitix_bit_set(info->vib_mode, BIT_WHEEL_VIB_MODE_1);
	}

	dev_info(&client->dev, "%s:vib mode[%d][0x%04x]\n", __func__, value, info->vib_mode);
			...
			struct device_attribute *attr, char *buf)
{
	...
	ret = ztm620_read_data(info->client, ZTM620_WHEEL_VIB_SETTING, (u8 *)&status, 2);
	if (ret < 0) {
...
static int ztm620_setup_bezel_input(struct ztm620_info *info)
{
	...
	ddata->input_dev->open = ztm620_bezel_open;
	ddata->input_dev->close = ztm620_bezel_close;
