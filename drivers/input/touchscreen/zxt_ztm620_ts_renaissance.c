...
static s32 ztm620_read_data(struct i2c_client *client, u16 reg, u8 *values, u16 length)
{
	...
	for (retry = 0; retry < I2C_RETRY_TIMES; retry++) {
		if (i2c_dev_pm->is_suspended) {
			dev_err(&client->dev, "%s:i2c_adapter is in suspend[%d]\n",
		...
		if (ret < 0) {
			dev_err(&client->dev, "%s:failed send. reg=0x%04x, ret:%d, try:%d/%d\n",
					__func__, reg, ret, retry, I2C_RETRY_TIMES);
		...
		if (ret < 0) {
			dev_err(&client->dev, "%s:failed recv. reg=0x%04x, ret:%d, try:%d/%d\n",
					__func__, reg, ret, retry, I2C_RETRY_TIMES);
			usleep_range(DELAY_FOR_TRANSCATION, DELAY_FOR_TRANSCATION);
			info->comm_err_cnt++;
			continue;
