...
static int ztm620_set_optional_mode(struct ztm620_info *info, bool force)
{
	...
	dev_info(&info->client->dev, "%s:optional_mode[0x%04x]\n", __func__, info->optional_mode);

#if SUPPORTED_TOUCH_WHEEL
	ret = zinitix_bit_test(info->optional_mode, DEF_OPTIONAL_MODE_WHEEL_ON_BIT);
	if (ret) {
		ret = ztm620_write_reg(info->client, ZTM620_WHEEL_VIB_SETTING, bezel_info->vib_mode);
		if (ret != I2C_SUCCESS) {
			dev_err(&info->client->dev,
	...
	ret = ztm620_write_reg(info->client, ZTM630_WHEEL_INTERVAL_SETTING, bezel_info->interval_mode);
	if (ret != I2C_SUCCESS) {
		dev_err(&info->client->dev,
...
static bool ztm620_mini_init_touch(struct ztm620_info *info)
{
	...
	ret = ztm620_write_reg(client, ZTM620_COORD_ORIENTATION, (TOUCH_V_FLIP | TOUCH_H_FLIP));
	if (ret != I2C_SUCCESS) {
...
static int ztm620_resume(struct device *dev)
{
	...
	if (!gpio_get_value(info->pdata->gpio_int))
		dev_info(&client->dev, "%s:irq_gpio state [LOW] recovery_cnt[%d]\n",
			__func__, info->recovery_cnt);
...
static void get_fw_ver_bin(void *device_data)
{
	...
	scnprintf(finfo->cmd_buff, length + 1, "%s", "ZI");
	scnprintf(finfo->cmd_buff + length, sizeof(finfo->cmd_buff) - length, "%06X", version);
	set_cmd_result(finfo, finfo->cmd_buff,
			strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
	finfo->cmd_state = OK;
					...
					char *buf)
{
	...
	for ( i = 0; i < read_reg_size; i++) {
		...
		ret += sprintf(s_buff, "0x%02x", r_buff[i]);
		strcat(buf, s_buff);
...
static ssize_t ztm620_show_fw_ver_ic(struct device *dev, struct device_attribute *devattr, char *buf)
{
	...
	scnprintf(buffer + length, sizeof(buffer) - length, "%06X", version);
...
static ssize_t ztm620_show_fw_ver_bin(struct device *dev, struct device_attribute *devattr, char *buf)
{
	...
	length = sizeof(vendor_id);
	scnprintf(buffer, length + 1, "%s", "ZI");
	scnprintf(buffer + length, sizeof(buffer) - length, "%06X", version);
			...
			struct device_attribute *attr, char *buf)
{
	...
	ret = ztm620_read_data(info->client, ZTM630_WHEEL_INTERVAL_SETTING, (u8 *)&status, 2);
	if (ret < 0) {
