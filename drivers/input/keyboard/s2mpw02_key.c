/*
 * Driver for Power keys on s2mpw02 IC by PWRON rising, falling interrupts.
 *
 * drivers/input/keyboard/s2mpw02_keys.c
 * S2MPW02 Keyboard Driver
 *
				...
				struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < ddata->pdata->nbuttons; i++) {
		struct power_button_data *bdata = &ddata->button_data[i];
		struct power_keys_button *button = bdata->button;
...
static struct power_keys_platform_data *power_keys_get_devtree_pdata(
		struct s2mpw02_dev *mfd_dev)
{
	/* to do */
	#define S2MPW02_SUPPORT_KEY_NUM	(1)
	...
	key_node = of_find_node_by_name(p_node, "s2mpw02-keys");
	if (!key_node) {
		dev_err(dev, "could not find s2mpw02-keys sub-node\n");
		error = -ENOENT;
		goto err_out;
	}

	nbuttons = of_get_child_count(key_node);
	if (nbuttons > S2MPW02_SUPPORT_KEY_NUM)
		dev_warn(dev, "s2mpw02-keys support only one button.\n");
...
power_keys_get_devtree_pdata(struct s2mpw02_dev *mfd_dev)
{
...
static int power_keys_probe(struct platform_device *pdev)
{
	...
	struct s2mpw02_platform_data *mpdata = s2mpw02->pdata;
	...
	if (!pdata) {
		pdata = power_keys_get_devtree_pdata(s2mpw02);
		if (IS_ERR(pdata))
	...
	input->phys	= "s2mpw02-keys/input0";
...
MODULE_DESCRIPTION("Keyboard driver for s2mpw02");
MODULE_ALIAS("platform:s2mpw02 Power key");
