...
const struct sec_input_bridge *g_bridge;
...
static void input_bridge_check_panic(char *onekey_env_str)
{
	...
	if (!strncmp(onekey_env_str, PANIC_STRING, strlen(PANIC_STRING)))
		panic("Crash Key. atomic:[%d]", in_atomic());
}
...
static void input_bridge_onekey_dwork(struct work_struct *work)
{
	struct sec_input_bridge *sec_bridge =
		container_of(work, struct sec_input_bridge, onekey_dwork.work);
	struct sec_input_bridge_platform_data *pdata = sec_bridge->pdata;
	int i;

	cancel_delayed_work(&sec_bridge->onekey_dwork);
			       ...
			       unsigned int code, int value)
{
	...
	if (!sec_bridge->onekey_cnt_start) {
		...
		schedule_delayed_work(&sec_bridge->onekey_dwork,
				msecs_to_jiffies(pdata->onekey_timeout));
	} else {
			       ...
			       unsigned int code, int value)
{
	...
	if ((mmap->mkey_map[sec_bridge->check_index[KEY_LOGDUMP]].code == code)&&
		(mmap->mkey_map[sec_bridge->check_index[KEY_LOGDUMP]].value == value)) {
		sec_bridge->check_index[KEY_LOGDUMP]++;
		if ((sec_bridge->check_index[KEY_LOGDUMP]) >= mmap->num_mkey) {
			...
			struct sec_input_bridge_platform_data *pdata)
{
	...
	unsigned int *value;
	const char *out_prop;
	int i, j, rc, map_size, type_size, value_size;
	u32 id = 0;
	...
	for (i = 0; i < pdata->num_map; i++) {
		...
		if (rc < 0) {
			dev_err(dev, "%s: failed to get %d map_id string.[%s]\n", __func__, i, out_prop);
			goto error;
		...
		if (rc) {
			dev_err(dev, "%s: failed to get id[%s]\n", __func__, out_prop);
			goto error;
		}
		...
		rc = of_property_read_string_index(np, "input_bridge,map_value", i, &out_prop);
		if (rc < 0) {
			value_size = prop->length / sizeof(unsigned int);
			value = devm_kzalloc(dev, sizeof(unsigned int)*value_size, GFP_KERNEL);
			if (!value) {
				dev_err(dev, "%s: Failed to allocate key memory.\n", __func__);
				rc = -ENOMEM;
				goto error;
			}
			dev_err(dev, "%s: failed to get %d map_value string.[%s]\n", __func__, i, out_prop);
		} else {
			prop = of_find_property(np, out_prop, NULL);
			if (prop) {
				value_size = prop->length / sizeof(unsigned int);
				...
				rc = of_property_read_u32_array(np, out_prop, value, value_size);
				if (rc && (rc != -EINVAL)) {
					dev_dbg(dev, "%s: Unable to read %s\n", __func__, out_prop);
				}
			}
		}
		...
		for (j = 0; j < map_size; j++) {
			...
			if (value) {
				pdata->mmap[id].mkey_map[j].value = value[j];
			}
		}
	...
	for (i = 0; i < INPUT_MAX; i++) {
		...
		for(j = 0; j < pdata->mmap[i].num_mkey; j++) {
			...
			dev_info(dev,
				"%s: pdata->mmap[%d].mkey_map[%d].value=[%d]\n",
				__func__, i, j, pdata->mmap[i].mkey_map[j].value);
		}
...
#endif
	if (pdata->mmap[ONEKEY_DEBUG].enable) {
		INIT_DELAYED_WORK(&bridge->onekey_dwork, input_bridge_onekey_dwork);
	}
...
static int sec_input_bridge_remove(struct platform_device *dev)
{
	...
	cancel_delayed_work_sync(&bridge->onekey_dwork);
