...
static struct devfreq *la_stats_devfreq_mif;
static struct devfreq *la_stats_devfreq_int;
#endif
...
static int devfreq_update_status(struct devfreq *devfreq, unsigned long freq)
{
	...
	static unsigned long long time_state_mif = 0;
	static unsigned long long time_state_int = 0;
	int i;
#endif
...
#if defined(CONFIG_CHECK_TIME_IN_STATE)
	if ((jiffies_to_msecs(cur_time -time_state_mif ) >= TIME_IN_STATE_INTERVAL) ||
		(jiffies_to_msecs(cur_time -time_state_int ) >= TIME_IN_STATE_INTERVAL)) {
		for (i = 0; i < devfreq->profile->max_state; i++) {
			if (strstr(pdev->name, "mif") != NULL) {
				la_get_time_in_state(i, devfreq->profile->freq_table[i], (unsigned long long)
					jiffies_to_msecs(devfreq->time_in_state[i]),"LA_MIF");
				la_stats_devfreq_mif = devfreq;
				time_state_mif = cur_time;
			} else if (strstr(pdev->name, "int") != NULL) {
				la_get_time_in_state(i, devfreq->profile->freq_table[i], (unsigned long long)
					jiffies_to_msecs(devfreq->time_in_state[i]),"LA_INT");
				la_stats_devfreq_int = devfreq;
				time_state_int = cur_time;
			}
		}
	}
#endif
...
#if defined(CONFIG_CHECK_TIME_IN_STATE)
void la_devfreq_update_status(char *name, unsigned long freq) {

	if ((la_stats_devfreq_mif == NULL) || (la_stats_devfreq_int == NULL))
		return;

	if (strstr(name,"MIF")!=NULL) {
		devfreq_update_status(la_stats_devfreq_mif,freq);
	} else if (strstr(name,"INT")!=NULL) {
		devfreq_update_status(la_stats_devfreq_int,freq);
	}
}
EXPORT_SYMBOL(la_devfreq_update_status);
#endif
/**
