...
static void watchdog_timer_handler(unsigned long data)
{
	...
	mod_timer(&watchdog_timer, (jiffies + 10*HZ));
}

static int watchdog_pm_notifier(struct notifier_block *this,
		unsigned long event, void *ptr)
{
	struct s3c2410_wdt *wdt = container_of(this, struct s3c2410_wdt, pm_nb);

	switch (event) {
	case PM_POST_SUSPEND:
		if (g_kicktimer_state && !timer_pending(&watchdog_timer)) {
			s3c2410wdt_keepalive(&wdt->wdt_device);
			mod_timer(&watchdog_timer, (jiffies + 10*HZ));
			dev_info(wdt->dev, "Restart watchdog keepalive!\n");
		}
		break;
	case PM_FS_SYNC_PREPARE:
		if (g_kicktimer_state) {
			s3c2410wdt_keepalive(&wdt->wdt_device);
			del_timer_sync(&watchdog_timer);
			dev_info(wdt->dev, "Stop watchdog keepalive!\n");
		}
		break;
	}
...
static int s3c2410wdt_probe(struct platform_device *pdev)
{
	...
	if (tmr_atboot && started == 0) {
		...
		wdt->pm_nb.notifier_call = watchdog_pm_notifier;
		register_pm_notifier(&wdt->pm_nb);
	}
#endif
