...
int lbs_stat_get_stat(struct lbs_stat_emon *emon_stat)
{
	int ret = 0;
	int i = 0;
	struct lbs_stat *entry;
	ktime_t total_time, total_gps_time, active_time;

	if (!emon_stat)
		return -EINVAL;

	total_time = total_gps_time =  ktime_set(0,0);
	list_for_each_entry(entry, &lbs_stat_list, list) {
		for (i = 0; i < LBS_METHOD_MAX; i++) {
			if (entry->usage[i].active_count == 0)
				continue;

			total_time = ktime_add(total_time, entry->total_time);
			total_gps_time = ktime_add(total_time, entry->total_gps_time);
			...
			if (entry->usage[i].active) {
				...
				if (i == LBS_METHOD_GPS || i == LBS_METHOD_BATCH_GPS) {
					total_gps_time = ktime_add(total_gps_time, active_time);
				}
			}
		}
	}

	emon_stat->total_time = total_time;
	emon_stat->total_gps_time = total_gps_time;

	return ret;
}

int lbs_stat_get_stat_emon(int type, struct lbs_stat_emon *emon_stat, size_t n)
{
	int ret = 0;
	int i = 0, j;
	unsigned long flags;
