...
int get_last_irq_history(struct irq_history *last)
{
	if (!last)
		return -EINVAL;

	memcpy(last, &last_irq, sizeof(struct irq_history));
...
static int add_irq_history_instance(int irq, char *name)
{
	...
	last_irq.irq = irq;
	strncpy(last_irq.name, irq_name, IRQ_NAME_LENGTH);
