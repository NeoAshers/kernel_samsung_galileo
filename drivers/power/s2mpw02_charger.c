...
static struct device *s2mpw02_pmic_sysfs;
#endif
...
#define S2MPW02_CV_MARGIN			20
#define S2MPW02_CV_LEVLE1_OFFSET	50
#define S2MPW02_CV_LEVLE2_OFFSET	88
...
struct s2mpw02_charger_data {
	...
	int irq_acokf_disabled;
#if defined(CONFIG_MUIC_NOTIFIER)
	muic_attached_dev_t	muic_dev;
#endif
	muic_attached_dev_t	wpc_dev;
...
static int s2mpw02_wpc_online_check(struct s2mpw02_charger_data *charger)
{
	...
	ret = (val.intval == WPC_ON_PAD) ? 1: 0;

	return ret;
}


static int s2mpw02_acokf_irq_set(struct s2mpw02_charger_data *charger, bool enable)
{
	pr_info("%s irq enable = %d irq_acokf_disabled = %d\n",
		__func__, enable, charger->irq_acokf_disabled);

	if (enable) {
		if (charger->irq_acokf_disabled) {
			enable_irq(charger->irq_acokf);
			charger->irq_acokf_disabled = 0;
		}
	} else {
		if (charger->irq_acokf_disabled == 0) {
			disable_irq(charger->irq_acokf);
			charger->irq_acokf_disabled = 1;
		}
	}
		...
		int float_voltage)
{
	...
	pr_info("%s: float_voltage [%d] & top off event [%d]\n", __func__, float_voltage,
		(charger->top_off_event ? 1 : 0));
...
static int s2mpw02_set_cv_drop_level(struct s2mpw02_charger_data *charger, u8 level)
{
	int ret;
	u8 data;

	data = (level << CV_DROP_LEVEL_SHIFT) & CV_DROP_LEVEL_MASK;

	ret = s2mpw02_update_reg(charger->iodev->rtc, S2MP_RTC_REG_CTRL, data, CV_DROP_LEVEL_MASK);
	if (ret) {
		pr_err("%s: Failed to rtc update reg fail\n", __func__);
		return -EINVAL;
	}
	ret = s2mpw02_read_reg(charger->iodev->rtc, S2MP_RTC_REG_CTRL, &data);
	if (ret) {
		pr_err("%s: Failed to rtc read reg fail\n", __func__);
		return -EINVAL;
	}

	pr_info("%s : CV level=(%d)\n", __func__, data);

	return ret;
}

static int s2mpw02_get_cv_drop_level(struct s2mpw02_charger_data *charger)
{
	int ret;
	u8 data;
	...
	ret = s2mpw02_read_reg(charger->iodev->rtc, S2MP_RTC_REG_CTRL, &data);
	if (ret) {
		pr_err("%s: Failed to rtc 0x00 address access\n", __func__);
		return -EINVAL;
	}

	ret = (data & CV_DROP_LEVEL_MASK) >> CV_DROP_LEVEL_SHIFT;
	pr_info("%s cv drop level = %d\n", __func__, ret);

	return ret;
}

static int s2mpw02_get_cv_changed(struct s2mpw02_charger_data *charger)
{
	int lvl;

	lvl = s2mpw02_get_cv_drop_level(charger);

	if (lvl == S2MPW02_CV_DROP_LEVEL_1)
		charger->cv_changed = charger->pdata->chg_float_voltage + S2MPW02_CV_LEVLE1_OFFSET;
	else if (lvl == S2MPW02_CV_DROP_LEVEL_2)
		charger->cv_changed = charger->pdata->chg_float_voltage + S2MPW02_CV_LEVLE2_OFFSET;

	pr_info("%s : CV level=(%d), (%d)-->(%d)\n", __func__, lvl,
		charger->pdata->chg_float_voltage, charger->cv_changed);
...
static bool s2mpw02_chg_init(struct s2mpw02_charger_data *charger)
{
	...
	/* Recharge Forced off Enable & set V_min */
	s2mpw02_update_reg(charger->client, S2MPW02_CHG_REG_CTRL4, 0xD0, 0xF0);
...
static int s2mpw02_monitor_work_debug_info(struct s2mpw02_charger_data *charger)
{
	...
	pr_info("%s charging(%d) chg_current(%d) chg_sts(%d) eoc(%d) health(%d) "
		"float(%d) top_off(%d) cv_status(%d) in2bat(%d) acok diff(%d)\n",
		__func__,
		c->is_charging,
		c->charging_current,
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		if (charger->top_off_event == true) {
			s2mpw02_get_cv_changed(charger);
		}
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_EXT_PROP_MAX:
		switch (ext_psp) {
			...
			case POWER_SUPPLY_EXT_PROP_CHECK_DROP_CV:
				val->intval = s2mpw02_get_cv_drop_level(charger);
				break;
			case POWER_SUPPLY_EXT_PROP_DEBUG_INFO:
				if (val->intval == DEBUG_INFO_ACOK_CNT)
					val->intval = charger->acok_count;
				break;
			default:
				break;
		}
...
/*	int previous_cable_type = charger->cable_type; */
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		if (charger->top_off_event == true) {
			s2mpw02_get_cv_changed(charger);
			dev_dbg(dev, "%s() float voltage(%d), top_off_event(%d)\n", __func__,
				charger->cv_changed, charger->top_off_event);
			s2mpw02_set_regulation_voltage(charger, charger->cv_changed);
		} else {
			dev_dbg(dev,"%s() float voltage(%d), top_off_event(%d)\n", __func__,
				val->intval, charger->top_off_event);
		...
		switch (ext_psp) {
			case POWER_SUPPLY_EXT_PROP_WPC_ONLINE:
				if (val->intval == SEC_WIRELESS_PAD_WPC) {
					...
					charger->wpc_dev = ATTACHED_DEV_WIRELESS_TA_MUIC;
					s2mpw02_muic_detect_handler(charger, true);
				} else if (val->intval == SEC_WIRELESS_PAD_WPC_POWER_SHARE) {
					if (charger->wpc_dev != ATTACHED_DEV_WIRELESS_POWER_SHARE_MUIC) {
						pr_info("%s: Wireless Power share TA connected\n", __func__);
						charger->wpc_dev = ATTACHED_DEV_WIRELESS_POWER_SHARE_MUIC;
						s2mpw02_muic_detect_handler(charger, true);
					}
				} else if (val->intval == SEC_WIRELESS_PAD_NONE) {
					pr_info("%s: Wireless TA disconnected\n", __func__);
					charger->wpc_dev = ATTACHED_DEV_NONE_MUIC;
					muic_notifier_detach_attached_dev(charger->muic_dev);
					charger->muic_dev = ATTACHED_DEV_NONE_MUIC;
					charger->acok_count = 0;
				}
				break;
			case POWER_SUPPLY_EXT_PROP_FORCED_JIG_MODE:
				s2mpw02_set_forced_jig_mode(charger);
				break;
			case POWER_SUPPLY_EXT_PROP_SET_VOUT_IRQ:
				s2mpw02_acokf_irq_set(charger, val->intval);
				if (val->intval == 1) {
					u8 acok;
					s2mpw02_read_reg(charger->iodev->pmic, S2MPW02_PMIC_REG_STATUS1, &acok);
					pr_info("%s:acok:0x%x\n", __func__, acok);

					if (!(acok & ACOK_STATUS_MASK)) {
						s2mpw02_muic_detect_handler(charger, false);
					}
				}
				break;
			case POWER_SUPPLY_EXT_PROP_CHECK_DROP_CV:
				if (val->intval == S2MPW02_CV_DROP_FORCE_CLEAR) {
					charger->top_off_event = false;
					s2mpw02_set_regulation_voltage(charger, charger->pdata->chg_float_voltage);
					s2mpw02_set_cv_drop_level(charger, S2MPW02_CV_DROP_CLEAR);
				} else {
					if (charger->top_off_event == true) {
						charger->top_off_event = false;
						s2mpw02_set_regulation_voltage(charger, charger->pdata->chg_float_voltage);
...
static void s2mpw02_topoff_status_setting(struct s2mpw02_charger_data *charger)
{
	...
	if ((chg_sts1 & TOP_OFF_STATUS_MASK) && (chg_sts3 & CV_OK_STATUS_MASK)) {
		/* 1st full charge */
		if (charger->top_off_event == false) {
			value.intval = SEC_BATTERY_VOLTAGE_AVERAGE;
			psy_do_property("sec-fuelgauge", get, POWER_SUPPLY_PROP_VOLTAGE_AVG, value);
			charger->avg_vbat = value.intval;
			/* In case of top-off interrupt when vbat is too low, SOC cannot reach 100% becuase. */
			/* This is software w/a increasing CV voltage at 1st full charge and ensuring additional charging operation to reach 100% SOC. */
			/* CV Voltage is trimmed as 4.325V in order to ensure the nearest value from 4.35V which is normal specification. */

			/* vbat is lower than CV voltage by 120mV(4.23V)*/
			/* put back whole full charge sequence until 2st full charge */
			if (charger->avg_vbat < charger->pdata->chg_float_voltage -
				(S2MPW02_CV_LEVLE2_OFFSET + S2MPW02_CV_MARGIN)) {
				charger->top_off_event = true;
				/* increase CV voltage to satisfy 4.35V*/
				charger->cv_changed =
					charger->pdata->chg_float_voltage + S2MPW02_CV_LEVLE2_OFFSET;
				s2mpw02_set_regulation_voltage(charger, charger->cv_changed);
				s2mpw02_set_cv_drop_level(charger, S2MPW02_CV_DROP_LEVEL_2);
			/* vbat is lower than CV voltage by 70mV(4.28V) */
			} else if (charger->avg_vbat < charger->pdata->chg_float_voltage -
				(S2MPW02_CV_LEVLE1_OFFSET + S2MPW02_CV_MARGIN)) {
				charger->top_off_event = true;
				/* increase CV voltage to satisfy 4.35V*/
				charger->cv_changed =
					charger->pdata->chg_float_voltage + S2MPW02_CV_LEVLE1_OFFSET;
				s2mpw02_set_regulation_voltage(charger, charger->cv_changed);
				s2mpw02_set_cv_drop_level(charger, S2MPW02_CV_DROP_LEVEL_1);
			/* normal vbat : no need to increase CV voltage */
			} else {
	...
	if (pmic_sts1 & ACOK_STATUS_MASK) {
		...
		if (chg_sts4 & UART_CABLE_MASK) {
			...
			if (chg_sts4 & UART_BOOT_OFF_MASK) {
				...
				if (s2mpw02_wpc_online_check(charger)) {
					psy_do_property(charger->pdata->wireless_charger_name, get,
						POWER_SUPPLY_EXT_PROP_POWER_SHARING_CHARGE, val);

					pr_info("%s: Wireless TA connected ps(%d)\n",
						__func__, val.intval);

					if (val.intval) {
						charger->wpc_dev = ATTACHED_DEV_WIRELESS_POWER_SHARE_MUIC;
						s2mpw02_muic_attach_dev(charger, charger->wpc_dev);
					} else {
			...
			if (!(chg_sts4 & JIGON_MASK) && !(chg_sts4 & USB_BOOT_ON_MASK) &&
				...
				if (chg_sts2 & A2D_CHGINOK_STATUS_MASK) {
					...
					if (val.intval == WPC_ON_PAD) {
						pr_info("%s: Wireless TA connected wpc_dev = %d\n",
							__func__, charger->wpc_dev);
						if (charger->wpc_dev == ATTACHED_DEV_NONE_MUIC)
							s2mpw02_muic_attach_dev(charger, ATTACHED_DEV_WIRELESS_TA_MUIC);
						else
							s2mpw02_muic_attach_dev(charger, charger->wpc_dev);
					}
...
#ifdef CONFIG_SEC_SYSFS
static ssize_t s2mpw02_show_pmic_revision(struct device *dev, struct device_attribute *attr,
		char *buf) {
	u8 id = 0;
	ssize_t ret = 0;

	id = s2mpw02_get_pmic_revision();

	switch(id) {
	case EVT_0:
		ret = snprintf(buf, 6, "EVT0\n");
		break;
	case EVT_0_OTP_NO_TRIM:
		ret = snprintf(buf, 18, "EVT0_OTP_NO_TRIM\n");
		break;
	case EVT_0_5:
		ret = snprintf(buf, 8, "EVT0.5\n");
		break;
	case EVT_0_5_OTP_CHANGE:
		ret = snprintf(buf, 19, "EVT0.5_OTP_CHANGE\n");
		break;
	case EVT_1:
		ret = snprintf(buf, 6, "EVT1\n");
		break;
	case EVT_1_OTP_1:
		ret = snprintf(buf, 11, "EVT1_OTP_1\n");
		break;
	case EVT_2:
		ret = snprintf(buf, 6, "EVT2\n");
		break;
	case EVT_3:
		ret = snprintf(buf, 6, "EVT3\n");
		break;
	default:
		ret = snprintf(buf, 7, "error\n");
		break;
	}
...
static DEVICE_ATTR(revision, S_IRUGO, s2mpw02_show_pmic_revision, NULL);

static struct attribute *s2mpw02_pmic_sysfs_attrs[] = {
	&dev_attr_revision.attr,
	NULL
};
static const struct attribute_group s2mpw02_pmic_sysfs_attr_group = {
	.attrs = s2mpw02_pmic_sysfs_attrs,
};

static int s2mpw02_pmic_sysfs_init(void) {
	int err;

	s2mpw02_pmic_sysfs = sec_device_create(NULL, "pmic");
	if (IS_ERR(s2mpw02_pmic_sysfs)) {
		pr_err("%s failed to create pmic sysfs\n", __func__);
		return -ENODEV;
	}

	err = sysfs_create_group(&s2mpw02_pmic_sysfs->kobj, &s2mpw02_pmic_sysfs_attr_group);
	if (err < 0) {
		pr_err("%s failed to create pmic sysfs attr\n", __func__);
		return err;
	}
...
static int s2mpw02_charger_probe(struct platform_device *pdev)
{
	...
	/* apply CV voltage increase W/A after sw reset */
	charger->top_off_event = false;
	...
	if ((lvl == S2MPW02_CV_DROP_LEVEL_1) || (lvl == S2MPW02_CV_DROP_LEVEL_2)) {
		charger->top_off_event = true;
		pr_info("%s : Check CV level=(%d)\n", __func__, lvl);
	}

	pr_info("%s S2MPW02 charger driver loaded OK\n", __func__);

#ifdef CONFIG_SEC_SYSFS
	s2mpw02_pmic_sysfs_init();
#endif
