...
static void s2mpw02_fuelgauge_read_parameter(struct i2c_client *i2c)
{
	u8 data;
	int reg_addr = 0;

	pr_info("%s: **** READ FG PARAM ****\n", __func__);

	// ADDR 0x0B~0x0F
	pr_cont("%s: [0x0B-0x0F] ", __func__);
	for (reg_addr = S2MPW02_FG_REG_RTEMP2; reg_addr <= S2MPW02_FG_REG_RZADJ2; reg_addr++) {
		s2mpw02_fg_read_reg_byte(i2c, reg_addr, &data);
		pr_cont("%02x ", data);
	}
	pr_cont("\n");

	// ADDR 0x15, 0x19
	pr_info("%s: [0x15 0x19] ", __func__);
	s2mpw02_fg_read_reg_byte(i2c, S2MPW02_FG_REG_IRQ_LVL2, &data);
	pr_cont("%02x ", data);
	s2mpw02_fg_read_reg_byte(i2c, S2MPW02_FG_REG_MONOUT_CFG, &data);
	pr_cont("%02x\n", data);

	// ADDR PARAM 1
	pr_info("%s: FG PARAM 1\n", __func__);
	pr_cont("%s: ", __func__);
	for (reg_addr = S2MPW02_FG_REG_PARAM1; reg_addr < S2MPW02_FG_REG_PARAM1+S2MPW02_FG_PARAM1_NUM; reg_addr++) {
		s2mpw02_fg_read_reg_byte(i2c, reg_addr, &data);
		pr_cont("%02x ", data);
		if((reg_addr+1 - S2MPW02_FG_REG_PARAM1) % 10 == 0) {
			pr_cont("\n");
			pr_cont("%s: ", __func__);
		}
	}
	...
	// ADDR PARAM 2
	pr_info("%s: FG PARAM 2\n", __func__);
	pr_cont("%s: ", __func__);
	for (reg_addr = S2MPW02_FG_REG_PARAM2; reg_addr < S2MPW02_FG_REG_PARAM2+S2MPW02_FG_PARAM2_NUM; reg_addr++) {
		s2mpw02_fg_read_reg_byte(i2c, reg_addr, &data);
		pr_cont("%02x ", data);
		if((reg_addr+1 - S2MPW02_FG_REG_PARAM2) % 10 == 0) {
			pr_cont("\n");
			pr_cont("%s: ", __func__);
		}
	}
...
static void s2mpw02_alert_init(struct s2mpw02_fuelgauge_data *fuelgauge)
{
	...
	data = data | (fuelgauge->pdata->fuel_alert_soc << SOC_L_LVL_SHIFT);

	s2mpw02_fg_write_reg_byte(fuelgauge->i2c, S2MPW02_FG_REG_IRQ_LVL, data);
}
...
static int s2mpw02_get_soc(struct s2mpw02_fuelgauge_data *fuelgauge)
{
	...
	/* CV float voltage can be dropped to SOC 80 level */
	if (rsoc < 8000) {
		value.intval = S2MPW02_CV_DROP_NORMAL_CLEAR;
		psy_do_property(fuelgauge->pdata->charger_name, set,
			POWER_SUPPLY_EXT_PROP_CHECK_DROP_CV, value);
	}
...
static int s2mpw02_get_avgvbat(struct s2mpw02_fuelgauge_data *fuelgauge)
{
	...
	fuelgauge->avg_batt[2] = fuelgauge->avg_batt[1];
	fuelgauge->avg_batt[1] = fuelgauge->avg_batt[0];
	fuelgauge->avg_batt[0] = old_vbat;

	cnt = 0;
	for (i = 0; i < 3; i++)
		if (fuelgauge->avg_batt[i] > 0) cnt++;
	if (cnt == 0)
		fuelgauge->avg_bat = 0;
	else
		fuelgauge->avg_bat = (fuelgauge->avg_batt[0] + fuelgauge->avg_batt[1] + fuelgauge->avg_batt[2]) / cnt;

	dev_dbg(&fuelgauge->i2c->dev, "%s: avg_vbat(%d), array_data(%d %d %d)\n", __func__,
		fuelgauge->avg_bat, fuelgauge->avg_batt[0],
		fuelgauge->avg_batt[1], fuelgauge->avg_batt[2]);

	psy_do_property(fuelgauge->pdata->charger_name, get,
		POWER_SUPPLY_PROP_VOLTAGE_MAX, value);

	/* Below code is to avoid abnormal status.
	 * If float voltage is set over 4350mV,
	 * CV voltage drop work-around code is no longer required.
	 */
	float_voltage = value.intval;
	if (old_vbat >= float_voltage) {
		psy_do_property(fuelgauge->pdata->charger_name, get,
			POWER_SUPPLY_EXT_PROP_CHECK_DROP_CV, value);
		lvl = value.intval;

		dev_dbg(&fuelgauge->i2c->dev, "%s vbat = %d float voltage = %d cv_drop_level = %d\n",
			__func__, old_vbat, float_voltage, lvl);

		if (lvl != S2MPW02_CV_DROP_CLEAR) {
			value.intval = S2MPW02_CV_DROP_FORCE_CLEAR;
			psy_do_property(fuelgauge->pdata->charger_name, set,
				POWER_SUPPLY_EXT_PROP_CHECK_DROP_CV, value);
		}
	}
...
static int s2mpw02_fg_update_bat_param(struct s2mpw02_fuelgauge_data *fuelgauge, int step)
{
	...
	s2mpw02_fg_read_reg_byte(fuelgauge->i2c, S2MPW02_FG_REG_RZADJ2, &batcap2);
	s2mpw02_fg_read_reg_byte(fuelgauge->i2c, S2MPW02_FG_REG_IRQ_LVL2, &batcap3);
	...
	if ((param0 != fuelgauge->pdata->age_data_info[step].model_param1[0]) ||
		...
		((fuelgauge->fg_num_age_step_ext) && 
		((batcap2 != fuelgauge->pdata->age_data_info_ext[step].batcap_ext[0]) ||
		(batcap3 != fuelgauge->pdata->age_data_info_ext[step].batcap_ext[1])))) {
		...
		if (fuelgauge->fg_num_age_step_ext) {
			s2mpw02_fg_write_reg_byte(fuelgauge->i2c, S2MPW02_FG_REG_RZADJ2,
					fuelgauge->pdata->age_data_info_ext[step].batcap_ext[0]);

			s2mpw02_fg_write_reg_byte(fuelgauge->i2c, S2MPW02_FG_REG_IRQ_LVL2,
					fuelgauge->pdata->age_data_info_ext[step].batcap_ext[1]);
		}
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_EXT_PROP_MAX:
		switch (ext_psp) {
			case POWER_SUPPLY_EXT_PROP_UPDATE_BATTERY_DATA:
				...
				break;
			case POWER_SUPPLY_EXT_PROP_BATTERY_CAPACITY_UPDATE:
				fuelgauge->initial_update_of_soc = true;
				pr_info("%s initial_update_of_soc\n", __func__);
				break;
			default:
				break;
		}
...
static int s2mpw02_fuelgauge_parse_dt(struct s2mpw02_fuelgauge_data *fuelgauge)
{
	...
	if (np == NULL) {
		...
		p = of_get_property(np, "fuelgauge,battery_data_ext", &len);
		if (!p)
			pr_info("%s battery_params extention (0xF & 0x15) NULL", __func__);
		else {
			fuelgauge->fg_num_age_step_ext = len / sizeof(struct fg_age_data_info_ext);
			fuelgauge->pdata->age_data_info_ext = kzalloc(len, GFP_KERNEL);
			ret = of_property_read_u32_array(np, "fuelgauge,battery_data_ext",
					(int *)fuelgauge->pdata->age_data_info_ext, len/sizeof(int));
			pr_info("%s: fg_num_age_step_ext %d len = %d\n",
				__func__, fuelgauge->fg_num_age_step_ext, len);

			for (i = 0; i < fuelgauge->fg_num_age_step_ext; i++)
				pr_info("%s age_step = %d batcap_ext[0] = %02x, batcap_ext[1] = %02x",
					__func__, i, fuelgauge->pdata->age_data_info_ext[i].batcap_ext[0],
								 fuelgauge->pdata->age_data_info_ext[i].batcap_ext[1]);
		}
...
static int s2mpw02_fuelgauge_probe(struct platform_device *pdev)
{
	...
	if (fuelgauge->pdata->charger_name == NULL)
		fuelgauge->pdata->charger_name = "sec-charger";
	...
	s2mpw02_fg_test_read(fuelgauge->i2c);
	s2mpw02_fuelgauge_read_parameter(fuelgauge->i2c);
	return 0;
