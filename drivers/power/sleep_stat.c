 ...
 *                                                   Initial Release           *
 * ---- -------------------------------------------- ------------------------- *
 * 1.1   Junho Jang <vincent.jang@samsung.com>       <2019>                    *
 *                                                   apm request info. for exynos           *
 * ---- -------------------------------------------- ------------------------- *
 */
...
static char *apm_master_name[EXYNOS_MIF_MASTER_MAX] = {
	"mif_cp",
	"mif_aud",
	"mif_gnss",
	"mif_vts",
...
int sleep_stat_get_stat(struct sleep_stat *sleep_stat)
{
		...
		case SLEEP_STAT_SOC_EXYNOS:
			{
				...
				sleep_stat->soc.exynos.acpm_sleep_early_wakeup = soc_now.soc.exynos.acpm_sleep_early_wakeup;
				sleep_stat->soc.exynos.acpm_sleep_soc_down = soc_now.soc.exynos.acpm_sleep_soc_down;
				sleep_stat->soc.exynos.acpm_sleep_mif_down = soc_now.soc.exynos.acpm_sleep_mif_down;
				sleep_stat->soc.exynos.acpm_sicd_early_wakeup = soc_now.soc.exynos.acpm_sicd_early_wakeup;
				sleep_stat->soc.exynos.acpm_sicd_soc_down = soc_now.soc.exynos.acpm_sicd_soc_down;
				sleep_stat->soc.exynos.acpm_sicd_mif_down = soc_now.soc.exynos.acpm_sicd_mif_down;

				for (i = 0; i < EXYNOS_MIF_MASTER_MAX; i++) {
					sleep_stat->soc.exynos.apm[i].total_cnt = soc_now.soc.exynos.apm[i].total_cnt;
					sleep_stat->soc.exynos.apm[i].total_time = soc_now.soc.exynos.apm[i].total_time;
					sleep_stat->soc.exynos.mif_wakeup[i].wakeup_count = soc_now.soc.exynos.mif_wakeup[i].wakeup_count;
					sleep_stat->soc.exynos.apm_wakeup[i].apm_req_count = soc_now.soc.exynos.apm_wakeup[i].apm_req_count;
					sleep_stat->soc.exynos.apm_wakeup[i].tcxo_req_count = soc_now.soc.exynos.apm_wakeup[i].tcxo_req_count;
					sleep_stat->soc.exynos.apm_wakeup[i].pwr_req_count = soc_now.soc.exynos.apm_wakeup[i].pwr_req_count;
				}
			}
			break;
		default:
...
int sleep_stat_get_stat_emon(int type, struct sleep_stat *sleep_stat)
{
		...
		case SLEEP_STAT_SOC_EXYNOS:
			{
				...
				for (i = 0; i < EXYNOS_MIF_MASTER_MAX; i++) {
					sleep_stat->soc.exynos.apm[i].total_cnt =
						soc_now.soc.exynos.apm[i].total_cnt -
						snapshot->soc.exynos.apm[i].total_cnt;

					if (soc_now.soc.exynos.apm[i].total_time >=
						snapshot->soc.exynos.apm[i].total_time) {
						sleep_stat->soc.exynos.apm[i].total_time =
							soc_now.soc.exynos.apm[i].total_time -
							snapshot->soc.exynos.apm[i].total_time;
					} else {
						sleep_stat->soc.exynos.apm[i].total_time =
							soc_now.soc.exynos.apm[i].total_time;
					...
					sleep_stat->soc.exynos.mif_wakeup[i].wakeup_count =
						soc_now.soc.exynos.mif_wakeup[i].wakeup_count -
						snapshot->soc.exynos.mif_wakeup[i].wakeup_count;

					sleep_stat->soc.exynos.apm_wakeup[i].apm_req_count =
						soc_now.soc.exynos.apm_wakeup[i].apm_req_count -
						snapshot->soc.exynos.apm_wakeup[i].apm_req_count;
					sleep_stat->soc.exynos.apm_wakeup[i].tcxo_req_count =
						soc_now.soc.exynos.apm_wakeup[i].tcxo_req_count -
						snapshot->soc.exynos.apm_wakeup[i].tcxo_req_count;
					sleep_stat->soc.exynos.apm_wakeup[i].pwr_req_count =
						soc_now.soc.exynos.apm_wakeup[i].pwr_req_count -
						snapshot->soc.exynos.apm_wakeup[i].pwr_req_count;
				}
