...
#ifdef CONFIG_ENERGY_MONITOR
int input_stat_get_stat(struct input_stat_emon *emon_stat)
{
	int i;

	emon_stat->key_press = input_stat->key.press_count;
	emon_stat->key_release = input_stat->key.release_count;
	emon_stat->touch_press = input_stat->touch.press_count;
	emon_stat->touch_release = input_stat->touch.release_count;
	for (i = 0; i < Direction_MAX; i++)
		emon_stat->direction[i] = input_stat->wheel.direction[i];
	for (i = 0; i < EV_WU_MAX; i++)
		emon_stat->ev_wakeup[i] = input_stat->ev_wakeup[i];

	return 0;
}

int input_stat_get_stat_emon(int type, struct input_stat_emon *emon_stat)
{
	int i;
			       ...
			       const struct input_value *vals, unsigned int count)
{
	...
	for (i = 0; i < count; i++) {
		if (vals[i].type == EV_KEY) {
			...
			if (vals[i].code == ABS_MT_TRACKING_ID) {
...
static int input_stat_show(struct seq_file *m, void *unused)
{
	...
	seq_printf(m, "0x%-5x  0x%-10x  %-13d  %-14llu  "
	...
	seq_printf(m, "0x%-5x  0x%-10x  %-13d  %-14u  "
		"%-12u  %-12u  %-12u  %-12u  %-12u  %-12u"
		"%-12u  %-12u\n",
		0,
		0,
...
static int __init input_stat_init(void)
{
	...
	input_stat->touch.ev_type = EV_ABS;
