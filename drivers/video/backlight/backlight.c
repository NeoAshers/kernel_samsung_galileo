		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	steps_by_time = (steps * delay) / ONE_FRAME_DELAY_USEC;

	if (steps >= steps_by_time) {
		steps = steps_by_time;
		delay = ONE_FRAME_DELAY_USEC;
	}

	pr_info("%s: tar_br[%d]steps[%d]delay[%d]\n", __func__,
		tar_br, steps, delay);

	if (delay >= ONE_FRAME_DELAY_USEC)
		delay -= ONE_FRAME_DELAY_USEC;
	else
		delay = 0;
