...
#endif
#ifdef CONFIG_PANEL_TEMP_COMPENSATION
#include <linux/platform_data/sec_thermistor.h>
#define DECON_TEMP_COMPENSATION_DWORK_PERIOD 60000
#endif

int decon_log_level = 6;

#define DECON_TIMEOUT_CNT_FILE "/opt/etc/decon_timeout_cnt"
#define DECON_LOAD_TIMEOUT_CNT_DWORK_DELAY 50000
...
int decon_read_timeout_count(int *count)
{
	struct file *fp = NULL;
	mm_segment_t old_fs;
	int ret;
	...
	set_fs(KERNEL_DS);
	buf_len = sizeof(buf);
	fp = filp_open(DECON_TIMEOUT_CNT_FILE, O_RDONLY, 0);
	if (IS_ERR(fp)) {
		set_fs(old_fs);
		decon_err("File %s doesn't exist\n", DECON_TIMEOUT_CNT_FILE);
		return PTR_ERR(fp);
	}
	...
	if (ret > 0) {
		...
		decon_err("%s: Couldn't read the file %s, ret=%d\n",
			__FUNCTION__, DECON_TIMEOUT_CNT_FILE, ret);
		ret = -1;
	}
	return ret;
}

int decon_write_timeout_count(int count)
{
	struct file *fp = NULL;
	mm_segment_t old_fs;
	int ret = 0;
	...
	fp = filp_open(DECON_TIMEOUT_CNT_FILE, O_RDWR | O_CREAT, 0664);
	if (IS_ERR(fp)) {
		decon_err("%s: Couldn't open file '%s' err %ld\n",
			__FUNCTION__, DECON_TIMEOUT_CNT_FILE, PTR_ERR(fp));
		ret = PTR_ERR(fp);
	}
	else {
		if (fp->f_mode & FMODE_WRITE) {
			...
			if (ret < 0) {
				decon_err("%s: Couldn't write file '%s'\n",
					__FUNCTION__, DECON_TIMEOUT_CNT_FILE);
				ret = -1;
			}
			else {
				ret = 0;
...
static void decon_work_load_timeout_count(struct work_struct *work)
{
	int ret;
	struct dsim_device *dsim;
	struct decon_device *decon = container_of(work,
		struct decon_device, load_timeout_cnt_dwork.work);

	cancel_delayed_work(&decon->load_timeout_cnt_dwork);

	dsim = container_of(decon->out_sd[0], struct dsim_device, sd);
	ret = decon_read_timeout_count(&dsim->decon_timeout_cnt);
	if (ret) {
		decon_info("decon inform count [0]\n");
		dsim->decon_timeout_cnt = 0;
		decon_write_timeout_count(dsim->decon_timeout_cnt);
		return;
	}
	decon_info("decon inform count [%d]\n", dsim->decon_timeout_cnt);
}

int decon_timeout_count_load(struct decon_device *decon)
{
	INIT_DELAYED_WORK(&decon->load_timeout_cnt_dwork, decon_work_load_timeout_count);
	schedule_delayed_work(&decon->load_timeout_cnt_dwork,
		msecs_to_jiffies(DECON_LOAD_TIMEOUT_CNT_DWORK_DELAY));

	return 0;
}

static void decon_reset_block(void)
{
	struct decon_device *decon;

	decon = decon_drvdata[0];
	decon_info("%s [%d]:trig_cnt[%d], block_cnt[%d]\n", __func__, decon->hiber_state,
		atomic_read(&decon->hiber.trig_cnt), atomic_read(&decon->hiber.block_cnt));

	decon_hiber_block_reset(decon);
	decon_hiber_trig_reset(decon);
	atomic_set(&decon->hiber.check_cnt, 0);
	decon->hiber_state = 0;
}

static int hiber_block_count;
static int decon_work_hiber_block_start(struct decon_device *decon, unsigned int delay_ms);
static void decon_work_check_hiber_block(struct work_struct *work)
{
	struct decon_device *decon = container_of(work,
		struct decon_device, hiber_block_dwork.work);

	cancel_delayed_work(&decon->hiber_block_dwork);

	if (decon->aod_state != AOD_ENTER) {
		decon_info("decon hiber block exit\n");
		hiber_block_count = 0;
		return;
	}

	if (hiber_block_count == 3) {
		decon_reset_block();
		hiber_block_count = 0;
		return;
	}
	if (atomic_read(&decon->hiber.block_cnt) > 0) {
		decon_info("decon check hiber block [%d]\n", ++hiber_block_count);
		decon_work_hiber_block_start(decon, 10000);
	}
	else
		hiber_block_count = 0;
}
...
{
	cancel_delayed_work(&decon->hiber_block_dwork);
	schedule_delayed_work(&decon->hiber_block_dwork,
		msecs_to_jiffies(delay_ms));

	return 0;
}

static int decon_work_hiber_block_init(struct decon_device *decon)
{
	INIT_DELAYED_WORK(&decon->hiber_block_dwork, decon_work_check_hiber_block);

	return 0;
}

#ifdef CONFIG_PANEL_TEMP_COMPENSATION
static int decon_work_temp_compensation_start(struct decon_device *decon);
static int decon_work_temp_compensation_stop(struct decon_device *decon)
{
	cancel_delayed_work(&decon->temp_compensation_dwork);
...
static void decon_work_temp_set(struct work_struct *work)
{
	struct decon_device *decon = container_of(work, struct decon_device, temp_compensation_dwork.work);
	struct dsim_device *dsim = v4l2_get_subdevdata(decon->out_sd[0]);
	int temp = 0;

	if (!sec_therm_get_temp(TYPE_SEC_THREM_BATTERY, &temp))
		call_panel_ops(dsim, temperature_set, dsim, temp);

	schedule_delayed_work(&decon->temp_compensation_dwork, msecs_to_jiffies(DECON_TEMP_COMPENSATION_DWORK_PERIOD));
	decon_info("%s [%d]\n", __func__, temp);
}
...
{
	cancel_delayed_work(&decon->temp_compensation_dwork);
	decon_work_temp_set(&decon->temp_compensation_dwork.work);
...
static int decon_enable(struct decon_device *decon)
{
	...
	if (!decon->id && (decon->dt.out_type == DECON_OUT_DSI) &&
		...
		decon->state = DECON_STATE_ON;
#ifdef CONFIG_PANEL_TEMP_COMPENSATION
		decon_work_temp_compensation_start(decon);
#endif
		goto err;
	}
...
#ifdef CONFIG_PANEL_TEMP_COMPENSATION
	decon_work_temp_compensation_stop(decon);
#endif
...
static int decon_blank(int blank_mode, struct fb_info *info)
{
	...
	if (decon->aod_state != AOD_OFF) {
		decon_err("%s:invalid aod_state[%d] aod_watch_param[%d]\n", __func__, decon->aod_state, decon->aod_watch_param);
		if (blank_mode == FB_BLANK_POWERDOWN && decon->aod_watch_param != -1) {
			decon_info("%s: force off [%d]\n", __func__, decon->aod_watch_param);
			decon_exit_aod(decon, decon->aod_watch_param);
		}
		else {
			ret = -EINVAL;
			goto bypass;
...
#ifdef DECON_TIMEOUT_RECOVERY
#ifdef CONFIG_SEC_DEBUG_EXTRA_INFO
#define DECON_TIMEOUT_ERR	1
#define DECON_ESD_ERR	2
#else
...
void decon_vsync_timeout_recovery(struct decon_device *decon)
{
	...
	call_panel_ops(dsim, show_state, dsim);

	if (decon->recovery_cnt >= MAX_RECOVERY_CNT) {
		decon_info("%s:device reset\n", __func__);
#ifdef CONFIG_SEC_DEBUG_EXTRA_INFO
		sec_debug_set_extra_info_decon(DECON_TIMEOUT_ERR);
		panic("decon timeout\n");
#else
		arm_pm_restart(0, 0);
#endif
	...
	dsim->decon_recov_working = false;
	decon->recovery_working = 0;
	decon->recovery_worked = 1;
	decon->recovery_cnt++;
	decon->recovery_te_cnt = 0;
...
static void decon_check_max_vsync_timeout(struct decon_device *decon)
{
	...
	if (decon->dpms > FB_BLANK_NORMAL) {
		decon_info("%s : decon state is POWERDOWN \n", __func__);
		return;
	}
		...
		struct decon_reg_data *regs)
{
	...
	decon->frame_num = regs->frame_num;
#endif
...
#ifdef DECON_TIMEOUT_RECOVERY
				if (decon->recovery_worked) {
					if (decon->recovery_te_cnt == 0) {
						decon_info("%s:device reset\n", __func__);
#ifdef CONFIG_SEC_DEBUG_EXTRA_INFO
						sec_debug_set_extra_info_decon(DECON_TIMEOUT_ERR);
						panic("decon timeout\n");
	...
	if (regs->num_of_window) {
		if (__decon_update_regs(decon, regs) < 0) {
			...
			decon_check_max_vsync_timeout(decon);
#ifdef CONFIG_DRM_TDM_FRAME_DONE
			tdm_send_frame_done(decon->fence_dev, decon->frame_num);
#endif
			goto out;
#else
			decon_dump(decon);
...
#ifdef CONFIG_SEC_UB_DISCONNECT
		if (!dsim->ub_con_connected)
			decon_info("skip decon_update_regs\n");
		else
#endif
		...
		struct decon_win_config_data *win_data)
{
	...
	if (win_data->config[0].transparent_area.x != decon->commit_pid[0] ||
		win_data->config[1].transparent_area.x != decon->commit_pid[1] ||
		win_data->config[2].transparent_area.x != decon->commit_pid[2]) {
			decon_info("%s : pid[%d:%d:%d]\n", __func__, win_data->config[0].transparent_area.x,
				win_data->config[1].transparent_area.x, win_data->config[2].transparent_area.x);
			decon->commit_pid[0] = win_data->config[0].transparent_area.x;
			decon->commit_pid[1] = win_data->config[1].transparent_area.x;
			decon->commit_pid[2] = win_data->config[2].transparent_area.x;
	}
	if (decon->update_count % 60 != 0) {
		if (decon->update_count == 1 ||
			decon->update_count == 2 ||
			decon->update_count % 10 == 0) {
				decon_info("%s : process :%d [%d:%d:%d]\n", __func__, decon->update_count,
					win_data->config[0].transparent_area.x,  win_data->config[1].transparent_area.x,
					win_data->config[2].transparent_area.x);
		}
		decon->update_count++;
	}
	else
		decon->update_count = 0;

#ifdef DECON_FLUSH
        for (i = 0; i < decon->dt.max_win; i++) {
                if (decon->win[i]->state != win_data->config[i].state)
                        flush = true;
        }
        ...
        if (!list_empty(&decon->up.list)) {
                ...
                list_for_each_entry(data, &decon->up.list, list) {
                        cnt++;
                        if (decon->dbg_cnt)
                                decon_info("%s:pending:regs[%p]c[%d]\n", __func__, data, decon->dbg_cnt);
                }
...
#ifdef CONFIG_DRM_TDM_FRAME_DONE
	regs->frame_num = win_data->retire_fence;
#endif
	/*
		...
		struct decon_metadata *metadata)
{
	...
	case METADATA_OP_AOD_SET_INFO:
	{
		...
		decon_info("%s:METADATA_OP_AOD_SET_INFO\n", __func__);
	...
	case METADATA_OP_AOD_SET_STATE:
		...
		case AOD_ENTER:
			...
			decon_work_temp_compensation_stop(decon);
#endif
			decon_info("%s:AOD ENTER [%d]\n", __func__, metadata->aod_cfg.mode);
			call_panel_ops(dsim, aod_ctrl, dsim, metadata->state, metadata->aod_cfg.mode);
			wristup_booster_set_aod_state(1);
		...
		case AOD_EXIT:
			...
			decon_work_temp_compensation_start(decon);
#endif
			decon_info("%s:AOD EXIT [%d]\n", __func__, metadata->aod_cfg.mode);
			call_panel_ops(dsim, aod_ctrl, dsim, metadata->state, metadata->aod_cfg.mode);
			metadata->state = AOD_OFF;
			wristup_booster_set_aod_state(0);
			disp_stat_aod_update(AOD_EXIT);
			break;
		case AOD_CHANGE:
			decon_info("%s:AOD CHANGE [%d]\n", __func__, metadata->aod_cfg.mode);
			call_panel_ops(dsim, aod_ctrl, dsim, metadata->state, metadata->aod_cfg.mode);
			break;
		default:
...
static int decon_set_aod_analog(struct decon_device *decon, struct sclk_analog_cfg_v2 *analog_cfg)
{
	...
	fd = hand->addr;
	if (!fd) {
		decon_err("failed to get hour address");
		ret = -EINVAL;
		goto err_get_hour_fd;
	}
	...
	if (IS_ERR(hour_handle)) {
		...
		goto err_hour_import_dma;
	}
	addr = (unsigned long)ion_map_kernel(decon->ion_client, hour_handle);
	if (!addr) {
		decon_err("Failed to get ion_map hour address\n");
		ret = -EINVAL;
		goto err_hour_map;
	}
	...
	fd = hand->addr;
	if (!fd) {
		decon_err("failed to get minute address");
		ret = -EINVAL;
		goto err_get_min_fd;
	}
	...
	if (IS_ERR(minute_handle)) {
		...
		goto err_min_import_dma;
	}
	addr = (unsigned long)ion_map_kernel(decon->ion_client, minute_handle);
	if (!addr) {
		decon_err("Failed to get ion_map min address\n");
		ret = -EINVAL;
		goto err_min_map;
	}
	...
	fd = hand->addr;
	if (!fd) {
		decon_err("failed to get second address");
		ret = -EINVAL;
		goto err_get_sec_fd;
	}
	...
	if (IS_ERR(second_handle)) {
		...
		goto err_sec_import_dma;
	}
	addr = (unsigned long)ion_map_kernel(decon->ion_client, second_handle);
	if (!addr) {
		decon_err("Failed to get ion_map sec address\n");
		ret = -EINVAL;
		goto err_sec_map;
	}
...
static int decon_set_aod_digital(struct decon_device *decon, struct sclk_digital_cfg_v2 *digital_cfg)
{
	...
	if (!fd) {
		decon_err("failed to get font address");
		ret = -EINVAL;
		goto err_get_font_fd;
	}
	...
	if (IS_ERR(handle)) {
		decon_err("failed to import fd:%lx\n", fd);
		ret = PTR_ERR(handle);
		goto err_font_import_dma;
	}
	addr = (unsigned long)ion_map_kernel(decon->ion_client, handle);
	if (!addr) {
		decon_err("Failed to get ion_map font address\n");
		ret = -EINVAL;
		goto err_font_map;
	}
...
static int decon_set_aod_icon(struct decon_device *decon, struct sclk_icon_cfg_v2 *icon_cfg)
{
	...
	if (icon_cfg->req == AOD_DATA_SET) {
		fd = icon_cfg->addr;
		if (!fd) {
			decon_err("failed to get icon address");
			ret = -EINVAL;
			goto err_get_icon_fd;
		}
		...
		if (IS_ERR(handle)) {
			decon_err("failed to import fd:%lx\n", fd);
			ret = PTR_ERR(handle);
			goto err_icon_import_dma;
		}
		addr = (unsigned long)ion_map_kernel(decon->ion_client, handle);
		if (!addr) {
			decon_err("Failed to get ion_map icon address\n");
			ret = -EINVAL;
			goto err_icon_map;
		}
...
static int decon_enter_aod(struct decon_device *decon, int para)
{
	...
	if (decon->aod_state != AOD_OFF) {
		decon_info("current aod state is ON !!!\n");
		return 0;
	}
...
static int decon_exit_aod(struct decon_device *decon, int para)
{
	...
	if (decon->aod_state == AOD_OFF) {
		decon_info("current aod state is off !!!\n");
		return 0;
	}
...
#ifdef CONFIG_DRM_TDM_FRAME_DONE
	tdm_send_frame_done(decon->fence_dev, regs->frame_num);
#endif
	mutex_lock(&decon->fence_lock);
...
static void decon_set_multi_panel_info(struct decon_device *decon)
{
	struct fb_info *fbinfo;
	struct decon_lcd *lcd_info = decon->lcd_info;
	...
	if (lcd_info->support_multi_panel) {
		fbinfo->var.reserved[0] = lcd_info->support_multi_panel;
		fbinfo->var.reserved[1] = lcd_info->panel_vendor;

		decon_info("Set Multi panel informaton [%s]\n", lcd_info->panel_vendor ? "BOE" : "SDC");
	}
}
#endif
...
#ifdef CONFIG_EXYNOS_DSIM_MULTI_PANEL_SUPPORT
	decon_set_multi_panel_info(decon);
#endif
...
#endif
	tdm_register_reset_block(decon_reset_block);
	tdm_set_irq(decon->res.irq);
	decon_work_hiber_block_init(decon);
#ifdef CONFIG_PANEL_TEMP_COMPENSATION
	INIT_DELAYED_WORK(&decon->temp_compensation_dwork, decon_work_temp_set);
#endif
	decon_timeout_count_load(decon);
#ifdef DECON_TIMEOUT_RECOVERY
	decon->recovery_worked = 0;
#endif
	decon->hiber_state = 0;
	decon->update_count = 1;
