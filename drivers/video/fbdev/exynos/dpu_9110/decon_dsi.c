...
unsigned long long vsync_signal = 0;
#endif
...
int decon_get_out_sd(struct decon_device *decon)
{
	...
	decon_info("hs-clk %d\n", decon->lcd_info->hs_clk);
	decon_info("mic %s\n", decon->lcd_info->mic_enabled ? "enable" : "disable");
	decon_info("dsc %s\n", decon->lcd_info->dsc_enabled ? "enable" : "disable");
...
static irqreturn_t decon_ext_irq_handler(int irq, void *dev_id)
{
	...
	panel_vsync_wakeup_interrupt(decon->vsync.timestamp);
		...
		decon->recovery_cnt = 0;
		decon->recovery_te_cnt++;
#endif
	spin_unlock(&decon->slock);
#ifdef CONFIG_EXYNOS_WD_DVFS
...
int decon_enter_hiber(struct decon_device *decon)
{
	...
	pm_relax(decon->dev);
	if (prev_state != decon->state && decon->aod_state == AOD_ENTER)
		decon_info("%s:pm_relax[%d]\n", __func__, prev_state);
...
#define DECON_HIBER_CHECH_COUNT		500
static void decon_hiber_handler(struct kthread_work *work)
{
	...
	if ((atomic_read(&decon->hiber.check_cnt) % DECON_HIBER_CHECH_COUNT == 0) || decon->aod_state == AOD_ENTER) {
		decon_info("%s[%d:%d]: init_status[%d], trig_cnt[%d], block_cnt[%d]\n",
			__func__, decon->state, decon->aod_state, decon->hiber.init_status,
			atomic_read(&decon->hiber.trig_cnt), atomic_read(&decon->hiber.block_cnt));
		atomic_set(&decon->hiber.check_cnt, 0);
	}
	atomic_inc(&decon->hiber.check_cnt);
