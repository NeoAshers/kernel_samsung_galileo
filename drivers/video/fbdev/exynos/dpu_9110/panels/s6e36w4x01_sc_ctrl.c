/* drivers/video/fbdev/exynos/dpu_9110/panels/s6e36w4x01_sc_ctrl.c
 *
...
static char sc_afpc_cfg[SC_AFPC_CFG_SIZE] = {
	0x81, 0x00, 0x01, 0xFB, 0xE9, 0xD2, 0x00, 0x00,
	0x00, 0x01, 0x03, 0x45, 0xFF, 0xFF
};
...
static const char* panel_refresh_rate_str[] = {
	"REFRESH_LOW",
	"REFRESH_HIGH",
};

static const char* get_sc_sel_cfg_str(enum sc_select_cfg_t cmd)
{
	if (cmd >= SC_SEL_MAX)
		return "";

	return &sc_sel_cfg_str[cmd][0];
}

static const char* get_sc_comm_cfg_str(enum sc_common_cfg_t cmd)
{
	if (cmd >= SC_COMM_MAX)
		return "";

	return &sc_comm_cfg_str[cmd][0];
}

static const char* get_sc_digi_cfg_str(enum sc_digital_cfg_t cmd)
{
	if (cmd >= SC_D_MAX)
		return "";

	return &sc_digi_cfg_str[cmd][0];
}

static const char* get_sc_anal_cfg_str(enum sc_analog_cfg_t cmd)
{
	if (cmd >= SC_A_MAX)
		return "";

	return &sc_anal_cfg_str[cmd][0];
}

static const char* get_aod_mode_str(enum aod_mode cmd)
{
	if (cmd > AOD_NORMAL_CHARGING)
		return "";

	return &aod_mode_str[cmd][0];
}

static const char* get_aod_request_type_str(enum aod_request_type cmd)
{
	if (cmd > AOD_SET_PANEL_REFRESH_RATE)
		return "";

	return &aod_request_type_str[cmd][0];
}

static const char* get_aod_data_cmd_str(enum aod_data_cmd cmd)
{
	if (cmd > AOD_DATA_POS)
		return "";

	return &aod_data_cmd_str[cmd][0];
}

static const char* get_aod_mask_str(enum sc_mask_state_t cmd)
{
	if (cmd > SC_MASK_ENABLE)
		return "";

	return &aod_mask_str[cmd][0];
}

static const char* get_sc_blink_cfg_str(enum sc_blink_cfg_t cmd)
{
	if (cmd >= SC_BLINK_MAX)
		return "";

	return &sc_blink_cfg_str[cmd][0];
}

static const char* get_sc_afpc_cfg_str(enum sc_afpc_cfg_t cmd)
{
	if (cmd >= SCP_MAX)
		return "";

	return &sc_afpc_cfg_str[cmd][0];
}

static const char* get_sc_icon_cfg_str(enum sc_icon_cfg_t cmd)
{
	if (cmd >= SI_MAX)
		return "";

	return &sc_icon_cfg_str[cmd][0];
}

static const char* get_sc_move_cfg_str(enum sc_move_cfg_t cmd)
{
	if (cmd >= SC_MOVE_MAX)
		return "";

	return &sc_move_cfg_str[cmd][0];
}



static const char* get_panel_refresh_rate_str(enum aod_panel_refresh_rate cmd)
{
	if (cmd > AOD_PANEL_REFRESH_RATE_HIGH)
		return "";

	return &panel_refresh_rate_str[cmd][0];
}

static int s6e36w4x01_set_select_cfg(enum sc_digital_cfg_t cmd)
{
	switch (cmd) {
	case AC_ENC_BP:
	...
	pr_info("%s[%s]\n", __func__, get_sc_sel_cfg_str(cmd));

	return 0;
}

int s6e36w4x01_get_common_cfg(enum sc_common_cfg_t cmd)
{
	int value = 0;
	...
	pr_info("%s[%s][%d]\n", __func__, get_sc_comm_cfg_str(cmd), value);

	return value;
}

int s6e36w4x01_set_common_cfg(enum sc_common_cfg_t cmd, int value)
{
	switch (cmd) {
	case SC_TIME_STOP_EN:
		value &= 0x01;
...
int s6e36w4x01_set_analog_cfg(enum sc_analog_cfg_t cmd, int value)
{
	switch (cmd) {
	case SC_ON_SEL:
		value &= 0x01;
	...
	pr_info("%s[%s][%d]\n", __func__, get_sc_anal_cfg_str(cmd), value);

	return 0;
}

int s6e36w4x01_set_digital_cfg(enum sc_digital_cfg_t cmd, int value)
{
	switch (cmd) {
	case SC_D_CLOCK_EN:
		value &= 0x01;
	...
	case SC_D_ALIGN_EN:
		value &= 0x01;
		sc_digital_cfg[4] = (sc_digital_cfg[4] & ~0x80) | (value<<7);
		break;
	case SC_D_WD_ALIGN_EN:
		value &= 0x01;
		sc_digital_cfg[4] = (sc_digital_cfg[4] & ~0x40) | (value<<6);
		break;
	case SC_D_ALIGN_OFFSET:
		sc_digital_cfg[80] = (sc_digital_cfg[80] & ~0x03) | ((value & 0x300) >> 8);
		sc_digital_cfg[81] = value & 0xff;
		break;
	case SC_D_WD_ALIGN_OFFSET:
		sc_digital_cfg[82] = (sc_digital_cfg[82] & ~0x03) | ((value & 0x300) >> 8);
		sc_digital_cfg[83] = value & 0xff;
		break;
	case SC_D_REF_32B_DATA:
		sc_digital_cfg[76] = (value & 0xff000000) >> 24;
		sc_digital_cfg[77] = (value & 0xff0000) >> 16;
		sc_digital_cfg[78] = (value & 0xff00) >> 8;
		sc_digital_cfg[79] = value & 0xff;
		break;
	default:
		pr_err("%s:unkown cmd[%d]\n", __func__, cmd);
		return -1;
	...
	pr_info("%s[%s][%d]\n", __func__, get_sc_digi_cfg_str(cmd), value);

	return 0;
}

static int s6e36w4x01_set_blink_cfg(enum sc_blink_cfg_t cmd, int value)
{
	switch (cmd) {
	case SB_ON_SEL:
		value &= 0x01;
	...
	pr_info("%s[%s][%d]\n", __func__, get_sc_blink_cfg_str(cmd), value);

	return 0;
}

static int s6e36w4x01_set_afpc_cfg(enum sc_afpc_cfg_t cmd, int value)
{
	switch (cmd) {
	case SCP_ON_SEL:
		value &= 0x01;
	...
	case SCP_SIZE_RED:
		value &= 0x0f;
		sc_afpc_cfg[10] = (sc_afpc_cfg[10] & ~0xf) | (value);
		break;
	case SCP_SIZE_GREEN:
		value &= 0x0f;
		sc_afpc_cfg[11] = (sc_afpc_cfg[11] & ~0xf0) | (value << 4);
		break;
	case SCP_SIZE_BLUE:
		value &= 0x0f;
		sc_afpc_cfg[11] = (sc_afpc_cfg[11] & ~0x0f) | (value);
		break;
	case SCP_COMP_LEVEL1:
		value &= 0xff;
		sc_afpc_cfg[12] = (value);
		break;
	case SCP_COMP_LEVEL2:
		value &= 0xff;
		sc_afpc_cfg[13] = (value);
		break;
	default:
		pr_err("%s:unkown cmd[%d]\n", __func__, cmd);
		break;
	...
	pr_info("%s[%s][%d]\n", __func__, get_sc_afpc_cfg_str(cmd), value);

	return 0;
}

static int s6e36w4x01_set_icon_cfg(enum sc_icon_cfg_t cmd, int value)
{
	switch (cmd) {
	case SI_ON_SEL:
		value &= 0x01;
	...
	pr_info("%s[%s][%d]\n", __func__, get_sc_icon_cfg_str(cmd), value);

	return 0;
}

static int s6e36w4x01_set_move_cfg(enum sc_move_cfg_t cmd, int value)
{
	switch (cmd) {
	case MV_ON_SEL:
		value &= 0x01;
	...
	pr_info("%s[%s][%d]\n", __func__, get_sc_move_cfg_str(cmd), value);

	return 0;
}

void s6e36w4x01_update_sample_sc(struct s6e36w4x01 *lcd, enum sc_func_t func);
int s6e36w4x01_sc_alloc(struct s6e36w4x01 *lcd)
{
	int ret = 0;
...
extern void s6e36w4x01_testkey_enable_lv3(u32 id);
extern void s6e36w4x01_testkey_disable_lv3(u32 id);
void s6e36w4x01_write_blink_cfg(struct s6e36w4x01 *lcd)
{
...
void s6e36w4x01_write_time_update_cfg(struct s6e36w4x01 *lcd)
{
	struct dsim_device *dsim = lcd->dsim;
...
void s6e36w4x01_write_common_cfg(struct s6e36w4x01 *lcd)
{
	struct dsim_device *dsim = lcd->dsim;
...
void s6e36w4x01_write_icon_cfg(struct s6e36w4x01 *lcd)
{
	struct dsim_device *dsim = lcd->dsim;
...
void s6e36w4x01_write_afpc_cfg(struct s6e36w4x01 *lcd)
{
	struct dsim_device *dsim = lcd->dsim;
...
void s6e36w4x01_write_analog_cfg(struct s6e36w4x01 *lcd)
{
	struct dsim_device *dsim = lcd->dsim;
...
void s6e36w4x01_write_digital_cfg(struct s6e36w4x01 *lcd)
{
	struct dsim_device *dsim = lcd->dsim;
...
void s6e36w4x01_write_move_cfg(struct s6e36w4x01 *lcd)
{
	struct dsim_device *dsim = lcd->dsim;
...
void s6e36w4x01_read_dsi_err(struct s6e36w4x01 *lcd)
{
	struct dsim_device *dsim = lcd->dsim;
	char read_buff[2];
...
void s6e36w4x01_write_sideram_analog(struct s6e36w4x01 *lcd)
{
	struct dsim_device *dsim = lcd->dsim;
	int i;
	...
	s6e36w4x01_set_select_cfg(AC_ENC_BP);
...
void s6e36w4x01_write_sideram_digital(struct s6e36w4x01 *lcd)
{
	struct dsim_device *dsim = lcd->dsim;
	int i;
	...
	s6e36w4x01_set_select_cfg(DC_SEL);
...
void s6e36w4x01_write_sideram_icon(struct s6e36w4x01 *lcd)
{
	struct dsim_device *dsim = lcd->dsim;
	int time_en, disp_on;
	int i;
	...
	time_en = s6e36w4x01_get_common_cfg(SC_TIME_EN);
	disp_on = s6e36w4x01_get_common_cfg(SC_DISP_ON);

	s6e36w4x01_set_common_cfg(SC_TIME_EN, true);
	s6e36w4x01_set_common_cfg(SC_DISP_ON, false);
	s6e36w4x01_write_common_cfg(lcd);
	panel_vsync_wait(PANEL_30HZ_1FRAME_MSEC);
	s6e36w4x01_testkey_enable(dsim->id);

	s6e36w4x01_set_select_cfg(IC_SEL);
	...
	s6e36w4x01_set_common_cfg(SC_TIME_EN, time_en);
	s6e36w4x01_set_common_cfg(SC_DISP_ON, disp_on);
	s6e36w4x01_write_common_cfg(lcd);
	panel_vsync_wait(PANEL_30HZ_1FRAME_MSEC);
...
void s6e36w4x01_write_sideram_afpc(struct s6e36w4x01 *lcd)
{
	struct dsim_device *dsim = lcd->dsim;
	int i;
	...
	s6e36w4x01_set_select_cfg(SCP_SEL);
...
void s6e36w4x01_restore_sideram_data(struct s6e36w4x01 *lcd, enum aod_mode watch_type)
{
	if (!lcd->panel_icon_state)
		s6e36w4x01_write_sideram_icon(lcd);

	switch(watch_type) {
	case AOD_SCLK_ANALOG:
		if (!lcd->panel_sc_state)
			s6e36w4x01_write_sideram_analog(lcd);
		break;
	case AOD_SCLK_DIGITAL:
		if (!lcd->panel_sc_state)
			s6e36w4x01_write_sideram_digital(lcd);
		break;
	default:
		break;
	}
...
static int s6e36w4x01_update_analog_sc_buf(struct s6e36w4x01 *lcd, char *source_addr[SC_HANDS_SECTION])
{
	int i, j, k;

	if ((source_addr[0] == (void *)NULL) ||
	    (source_addr[1] == (void *)NULL) ||
	    (source_addr[2] == (void *)NULL)) {
		pr_err("%s:Hands addr is NULL\n", __func__);
		return -EINVAL;
	}
...
static int s6e36w4x01_update_digital_sc_buf(struct s6e36w4x01 *lcd,
						char *source_addr)
{
	int i, j;

	if (source_addr == (void *)NULL) {
		pr_err("%s:source_addr is NULL\n", __func__);
		return -EINVAL;
	}
...
static int s6e36w4x01_update_afpc_buf(struct s6e36w4x01 *lcd,
						char *source_addr)
{
	int i, j;
...
static int s6e36w4x01_update_icon_buf(struct s6e36w4x01 *lcd,
						char *source_addr)
{
	int i, j;
...
static void s6e36w4x01_sc_set_afpc_sample_cfg(void);
void s6e36w4x01_update_sample_sc(struct s6e36w4x01 *lcd, enum sc_func_t func)
{
	...
	case SC_ANALOG:
		...
		//s6e36w4x01_update_analog_sc_buf(lcd, (char**)&addr[0]);
		break;
	case SC_DIGITAL:
		//s6e36w4x01_update_digital_sc_buf(lcd, (char*)&Digital_Clock_Sample[0]);
		break;
	case SC_AFPC:
		s6e36w4x01_sc_set_afpc_sample_cfg();
//		s6e36w4x01_update_afpc_buf(lcd, (char*)&Afpc_Sample[0]);
		break;
	case SC_ICON:
		//s6e36w4x01_update_icon_buf(lcd, (char*)&Icon_Sample[0]);
		break;
	default:
...
static void s6e36w4x01_set_time(struct s6e36w4x01 *lcd)
{
	...
	pr_info("%s: %02d:%02d:%02d:%03d\n", __func__,
		lcd->sc_time.sc_h, lcd->sc_time.sc_m ,
		lcd->sc_time.sc_s, lcd->sc_time.sc_ms);

	s6e36w4x01_set_common_cfg(SC_SET_HH, lcd->sc_time.sc_h);
	s6e36w4x01_set_common_cfg(SC_SET_MM, lcd->sc_time.sc_m);
	s6e36w4x01_set_common_cfg(SC_SET_SS, lcd->sc_time.sc_s);
	s6e36w4x01_set_common_cfg(SC_SET_MSS, lcd->sc_time.sc_ms/SC_MSEC_DIVIDER);
}

extern int s6e36w4x01_read_mtp_reg(int id, u32 addr, char* buffer, u32 size);
int s6e36w4x01_get_panel_time(struct s6e36w4x01 *lcd, struct sc_time_st *panel_time)
{
	struct dsim_device *dsim = lcd->dsim;
	unsigned char read_buf[SC_TIME_REG_SIZE];
	int ret;

	ret = s6e36w4x01_read_mtp_reg(dsim->id, SC_TIME_REG, read_buf, SC_TIME_REG_SIZE);
	if (ret) {
...
static void s6e36w4x01_set_time_comp(struct s6e36w4x01 *lcd)
{
	...
	ret = s6e36w4x01_get_panel_time(lcd, &panel_time);
	if (ret) {
		pr_err("%s: Failed to get panel time[%d]\n", __func__, ret);
		s6e36w4x01_set_common_cfg(SC_COMP_EN, false);
		return;
	}
	...
	if (time_diff > (60*60*12))
		time_diff %= (60*60*12);

	pr_info("%s: time_diff:%d.%d sec\n",
		__func__, time_diff/10, time_diff%10);

	s6e36w4x01_set_common_cfg(SC_COMP_STEP, SC_COMP_STEP_VALUE);

	if (time_diff < SC_COMP_NEED_TIME)
		s6e36w4x01_set_common_cfg(SC_COMP_EN, true);
	else
		s6e36w4x01_set_common_cfg(SC_COMP_EN, false);
}

int s6e36w4x01_set_aod_time(struct dsim_device *dsim, struct sclk_time_cfg_v2 *time_cfg)
{
	...
	if (lcd->aod_mode != AOD_DISABLE) {
		s6e36w4x01_set_time(lcd);
		s6e36w4x01_set_time_comp(lcd);
		s6e36w4x01_write_common_cfg(lcd);
		s6e36w4x01_write_time_update_cfg(lcd);
	}
...
static void s6e36w4x01_set_hands_layer(struct s6e36w4x01 *lcd, struct sclk_analog_cfg_v2 *analog_cfg)
{
	...
	s6e36w4x01_set_analog_cfg(SC_HMS_LAYER, order_tbl[min->layer_order][hour->layer_order]);
...
static void s6e36w4x01_set_update_rate(struct s6e36w4x01 *lcd, unsigned int rate)
{
	int step, count;
	const char interval[4][2] = {{0, 3}, {1, 6}, {2, 15}, {3, 30}};

	/* Because of the HOP, It can not support over 1Hz clock rate in s6e36w4x01 */
	if (rate > 1) {
		pr_info("%s:can't support [%d]rate in s6e36w4x01 panel\n", __func__, rate);
		pr_info("%s:force set to 1 rate\n", __func__);
		rate = 1;
	}
	...
	s6e36w4x01_set_common_cfg(SC_INC_STEP, step);
	s6e36w4x01_set_common_cfg(SC_UPDATE_RATE, count);
}

int s6e36w4x01_set_aod_analog (struct dsim_device *dsim, struct sclk_analog_cfg_v2 *analog_cfg)
{
	struct lcd_device *panel = dsim->ld;
	struct s6e36w4x01 *lcd = dev_get_drvdata(&panel->dev);
	struct sclk_analog_hand *hour = &analog_cfg->hour;
	...
	if (analog_cfg->req > AOD_UPDATE_DATA) {
		pr_err("%s:Invalid request_type[%d]\n", __func__, analog_cfg->req);
		return -EINVAL;
	}

	pr_info("%s[%s]\n", __func__, get_aod_request_type_str(analog_cfg->req));

	if (hour->layer_mask == SC_MASK_DISABLE) {
		s6e36w4x01_set_analog_cfg(SC_HH_HAND_CENTER_X, hour->x);
		s6e36w4x01_set_analog_cfg(SC_HH_HAND_CENTER_Y, hour->y);
	}

	if (min->layer_mask == SC_MASK_DISABLE) {
		s6e36w4x01_set_analog_cfg(SC_MM_HAND_CENTER_X, min->x);
		s6e36w4x01_set_analog_cfg(SC_MM_HAND_CENTER_Y, min->y);
	}

	if (sec->layer_mask == SC_MASK_DISABLE) {
		s6e36w4x01_set_analog_cfg(SC_SS_HAND_CENTER_X, sec->x);
		s6e36w4x01_set_analog_cfg(SC_SS_HAND_CENTER_Y, sec->y);
	}

	if (analog_cfg->req == AOD_SET_CONFIG) {
		s6e36w4x01_set_analog_cfg(SC_HMS_HH_MASK, hour->layer_mask);
		s6e36w4x01_set_analog_cfg(SC_HMS_MM_MASK, min->layer_mask);
		s6e36w4x01_set_analog_cfg(SC_HMS_SS_MASK, sec->layer_mask);
		s6e36w4x01_set_analog_cfg(SC_FLIP_ST, analog_cfg->flip_start);
		s6e36w4x01_set_analog_cfg(SC_FLIP_ED, analog_cfg->flip_end);
		s6e36w4x01_set_update_rate(lcd, analog_cfg->rate);
		s6e36w4x01_set_hands_layer(lcd, analog_cfg);

		if (hour->layer_mask == SC_MASK_DISABLE) {
			s6e36w4x01_set_analog_cfg(SC_HH_X_ST_MASK, hour->mask_x);
			s6e36w4x01_set_analog_cfg(SC_HH_X_ED_MASK, hour->mask_x+hour->mask_w-1);
			s6e36w4x01_set_analog_cfg(SC_HH_Y_ST_MASK, hour->mask_y);
			s6e36w4x01_set_analog_cfg(SC_HH_Y_ED_MASK, hour->mask_y+hour->mask_h-1);
			s6e36w4x01_set_analog_cfg(SC_HH_MOTION, hour->motion);
			s6e36w4x01_set_analog_cfg(SC_HH_FLIP_EN, hour->flip_enable);
		}

		if (min->layer_mask == SC_MASK_DISABLE) {
			s6e36w4x01_set_analog_cfg(SC_MM_X_ST_MASK, min->mask_x);
			s6e36w4x01_set_analog_cfg(SC_MM_X_ED_MASK, min->mask_x+min->mask_w-1);
			s6e36w4x01_set_analog_cfg(SC_MM_Y_ST_MASK, min->mask_y);
			s6e36w4x01_set_analog_cfg(SC_MM_Y_ED_MASK, min->mask_y+min->mask_h-1);
			s6e36w4x01_set_analog_cfg(SC_MM_MOTION, min->motion);
			s6e36w4x01_set_analog_cfg(SC_MM_FLIP_EN, min->flip_enable);
		}

		if (sec->layer_mask == SC_MASK_DISABLE) {
			s6e36w4x01_set_analog_cfg(SC_SS_X_ST_MASK, sec->mask_x);
			s6e36w4x01_set_analog_cfg(SC_SS_X_ED_MASK, sec->mask_x+sec->mask_w-1);
			s6e36w4x01_set_analog_cfg(SC_SS_Y_ST_MASK, sec->mask_y);
			s6e36w4x01_set_analog_cfg(SC_SS_Y_ED_MASK, sec->mask_y+sec->mask_h-1);
			s6e36w4x01_set_analog_cfg(SC_SS_FLIP_EN, sec->flip_enable);
		}
		...
		if (lcd->aod_mode != AOD_DISABLE)
			s6e36w4x01_write_analog_cfg(lcd);
	}

	return 0;
}

static void s6e36w4x01_update_blink_cfg(struct s6e36w4x01 *lcd, struct sclk_digital_cfg_v2 *digital_cfg)
{
	...
	s6e36w4x01_set_blink_cfg(SB_BLK0_EN, blink1->mask);
	s6e36w4x01_set_blink_cfg(SB_BLK1_EN, blink2->mask);

	if (blink1->mask == SC_MASK_DISABLE) {
		s6e36w4x01_set_blink_cfg(SB_BLINK0_FONT, blink1->font_index);
		s6e36w4x01_set_blink_cfg(SB_ST_BLINK0_X, blink1->x[0]);
		s6e36w4x01_set_blink_cfg(SB_ST_BLINK0_Y, blink1->y[0]);
	}

	if (blink2->mask == SC_MASK_DISABLE) {
		s6e36w4x01_set_blink_cfg(SB_BLINK1_FONT, blink2->font_index);
		s6e36w4x01_set_blink_cfg(SB_ST_BLINK1_X, blink2->x[0]);
		s6e36w4x01_set_blink_cfg(SB_ST_BLINK1_Y, blink2->y[0]);
	}

	if ((blink1->mask == SC_MASK_DISABLE) ||
	    (blink2->mask == SC_MASK_DISABLE)) {
		s6e36w4x01_set_common_cfg(SC_INC_STEP, BLINK_DEFAULT_STEP);
		s6e36w4x01_set_common_cfg(SC_UPDATE_RATE, BLINK_DEFAULT_RATE);
		s6e36w4x01_set_blink_cfg(SB_BLINK_RATE, digital_cfg->blink_rate);

		if ( digital_cfg->blink_sync > BLINK_MAX_SYNC_INDEX)
			digital_cfg->blink_sync = BLINK_MAX_SYNC_INDEX;
		s6e36w4x01_set_blink_cfg(SB_BLINK_SYNC, sync_trans_tbl[digital_cfg->blink_sync]);
	}

	if (digital_cfg->req == AOD_SET_CONFIG) {
		s6e36w4x01_set_blink_cfg(SB_FONT0_IMG_WIDTH, font1->colon_w);
		s6e36w4x01_set_blink_cfg(SB_FONT0_IMG_HEIGHT, font1->colon_h);
		s6e36w4x01_set_blink_cfg(SB_FONT1_IMG_WIDTH, font2->colon_w);
		s6e36w4x01_set_blink_cfg(SB_FONT1_IMG_HEIGHT, font2->colon_h);
	}
}


static void s6e36w4x01_update_wdigital_cfg(struct s6e36w4x01 *lcd, struct sclk_digital_cfg_v2 *digital_cfg)
{
	...
	s6e36w4x01_set_digital_cfg(SC_D_EN_WD1, digital_cfg->world_clock1_mask);
	s6e36w4x01_set_digital_cfg(SC_D_EN_WD2, digital_cfg->world_clock2_mask);

	if (digital_cfg->world_clock1_mask == SC_MASK_DISABLE) {
		s6e36w4x01_set_digital_cfg(SC_D_ST_WD1_HH_X10, wclk1_hour->x[0]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_WD1_HH_Y10, wclk1_hour->y[0]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_WD1_HH_X01, wclk1_hour->x[1]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_WD1_HH_Y01, wclk1_hour->y[1]);

		s6e36w4x01_set_digital_cfg(SC_D_ST_WD1_MM_X10, wclk1_min->x[0]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_WD1_MM_Y10, wclk1_min->y[0]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_WD1_MM_X01, wclk1_min->x[1]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_WD1_MM_Y01, wclk1_min->y[1]);

		s6e36w4x01_set_digital_cfg(SC_D_EN_WD1_FONT, wclk1_hour->font_index);
		s6e36w4x01_set_digital_cfg(SC_D_WD1_HH, wclk1_hour->time_diff);
		s6e36w4x01_set_digital_cfg(SC_D_WD1_MM, wclk1_min->time_diff);

		if (wclk1_hour->time_diff < 0 || wclk1_min->time_diff < 0)
			s6e36w4x01_set_digital_cfg(SC_D_WD1_SIGN, WCLK_M_OFFSET);
		else
			s6e36w4x01_set_digital_cfg(SC_D_WD1_SIGN, WCLK_P_OFFSET);
	}

	if (digital_cfg->world_clock2_mask == SC_MASK_DISABLE) {
		s6e36w4x01_set_digital_cfg(SC_D_ST_WD2_HH_X10, wclk2_hour->x[0]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_WD2_HH_Y10, wclk2_hour->y[0]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_WD2_HH_X01, wclk2_hour->x[1]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_WD2_HH_Y01, wclk2_hour->y[1]);

		s6e36w4x01_set_digital_cfg(SC_D_ST_WD2_MM_X10, wclk2_min->x[0]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_WD2_MM_Y10, wclk2_min->y[0]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_WD2_MM_X01, wclk2_min->x[1]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_WD2_MM_Y01, wclk2_min->y[1]);

		s6e36w4x01_set_digital_cfg(SC_D_EN_WD2_FONT, wclk2_hour->font_index);
		s6e36w4x01_set_digital_cfg(SC_D_WD2_HH, wclk2_hour->time_diff);
		s6e36w4x01_set_digital_cfg(SC_D_WD2_MM, wclk2_min->time_diff);

		if (wclk2_hour->time_diff < 0 || wclk2_min->time_diff < 0)
			s6e36w4x01_set_digital_cfg(SC_D_WD2_SIGN, WCLK_M_OFFSET);
		else
			s6e36w4x01_set_digital_cfg(SC_D_WD2_SIGN, WCLK_P_OFFSET);
	}

	if ((digital_cfg->world_clock1_mask == SC_MASK_DISABLE) ||
	    (digital_cfg->world_clock2_mask == SC_MASK_DISABLE)) {
		s6e36w4x01_set_digital_cfg(SC_D_WD_DISP_OP, digital_cfg->world_clock_position);
		s6e36w4x01_set_digital_cfg(SC_D_WD_END_LINE, digital_cfg->world_clock_y);
	}
}

static void s6e36w4x01_update_digital_cfg(struct s6e36w4x01 *lcd, struct sclk_digital_cfg_v2 *digital_cfg)
{
	...
	s6e36w4x01_set_digital_cfg(SC_D_EN_HH, hour->mask);
	s6e36w4x01_set_digital_cfg(SC_D_EN_MM, min->mask);
	s6e36w4x01_set_digital_cfg(SC_D_EN_SS, sec->mask);

	if (hour->mask == SC_MASK_DISABLE) {
		s6e36w4x01_set_digital_cfg(SC_D_EN_HH_FONT, hour->font_index);
		s6e36w4x01_set_digital_cfg(SC_D_ST_HH_X10, hour->x[0]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_HH_Y10, hour->y[0]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_HH_X01, hour->x[1]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_HH_Y01, hour->y[1]);
	}

	if (min->mask == SC_MASK_DISABLE) {
		s6e36w4x01_set_digital_cfg(SC_D_EN_MM_FONT, min->font_index);
		s6e36w4x01_set_digital_cfg(SC_D_ST_MM_X10, min->x[0]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_MM_Y10, min->y[0]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_MM_X01, min->x[1]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_MM_Y01, min->y[1]);
	}

	if (sec->mask == SC_MASK_DISABLE) {
		s6e36w4x01_set_digital_cfg(SC_D_EN_SS_FONT, sec->font_index);
		s6e36w4x01_set_digital_cfg(SC_D_ST_SS_X10, sec->x[0]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_SS_Y10, sec->y[0]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_SS_X01, sec->x[1]);
		s6e36w4x01_set_digital_cfg(SC_D_ST_SS_Y01, sec->y[1]);
	}

	if (digital_cfg->req == AOD_SET_CONFIG) {
		s6e36w4x01_set_digital_cfg(SELF_FONT_MODE, digital_cfg->font_type);
		s6e36w4x01_set_digital_cfg(SC_24H_EN, digital_cfg->font_format);
		s6e36w4x01_set_digital_cfg(SC_D_FONT0_IMG_WIDTH, font1->digit_w);
		s6e36w4x01_set_digital_cfg(SC_D_FONT0_IMG_HEIGHT, font1->gitit_h);
		s6e36w4x01_set_digital_cfg(SC_D_FONT1_IMG_WIDTH, font2->digit_w);
		s6e36w4x01_set_digital_cfg(SC_D_FONT1_IMG_HEIGHT, font2->gitit_h);
	}
}

int s6e36w4x01_set_aod_digital (struct dsim_device *dsim, struct sclk_digital_cfg_v2 *digital_cfg)
{
	struct lcd_device *panel = dsim->ld;
	struct s6e36w4x01 *lcd = dev_get_drvdata(&panel->dev);
	int ret = 0;

	if (digital_cfg->req > AOD_UPDATE_DATA) {
		pr_err("%s:Invalid request_type[%d]\n", __func__, digital_cfg->req);
		return -EINVAL;
	}

	pr_info("%s[%s]\n", __func__, get_aod_request_type_str(digital_cfg->req));

	s6e36w4x01_update_digital_cfg(lcd, digital_cfg);
	s6e36w4x01_update_wdigital_cfg(lcd, digital_cfg);
	s6e36w4x01_update_blink_cfg(lcd, digital_cfg);

	switch (digital_cfg->req) {
	case AOD_SET_CONFIG:
		ret = s6e36w4x01_update_digital_sc_buf(lcd, (char*)digital_cfg->font_addr);
		if (ret) {
			pr_err("%s:failed update_digital_sc_buf[%d]\n", __func__, ret);
			return ret;
		}
		s6e36w4x01_write_sideram_digital(lcd);
		break;
	case AOD_UPDATE_DATA:
		if (lcd->aod_mode != AOD_DISABLE) {
			s6e36w4x01_write_digital_cfg(lcd);
			s6e36w4x01_write_blink_cfg(lcd);
		}
		break;
	default:
...
int s6e36w4x01_set_aod_icon (struct dsim_device *dsim, struct sclk_icon_cfg_v2 *icon_cfg)
{
	struct lcd_device *panel = dsim->ld;
	struct s6e36w4x01 *lcd = dev_get_drvdata(&panel->dev);
	int ret;

	if (icon_cfg->req > AOD_DATA_POS) {
		pr_err("%s:Invalid request_type[%d]\n", __func__, icon_cfg->req);
		return -EINVAL;
	}

	pr_info("%s[%s]\n", __func__, get_aod_data_cmd_str(icon_cfg->req));

	switch (icon_cfg->req) {
	case AOD_DATA_ENABLE:
		s6e36w4x01_set_icon_cfg(SI_ICON_EN, icon_cfg->mask);
		if (lcd->aod_mode != AOD_DISABLE)
			s6e36w4x01_write_icon_cfg(lcd);
		break;
	case AOD_DATA_SET:
		s6e36w4x01_set_icon_cfg(SI_IMG_WIDTH, icon_cfg->w);
		s6e36w4x01_set_icon_cfg(SI_IMG_HEIGHT, icon_cfg->h);
		ret = s6e36w4x01_update_icon_buf(lcd, (char*)icon_cfg->addr);
		if (ret) {
			pr_info("%s:failed update_icon_buf[%d]\n", __func__, ret);
			return ret;
		}
		s6e36w4x01_write_sideram_icon(lcd);
		break;
	case AOD_DATA_POS:
		s6e36w4x01_set_icon_cfg(SI_ST_X, icon_cfg->x);
		s6e36w4x01_set_icon_cfg(SI_ST_Y, icon_cfg->y);
		if (lcd->aod_mode != AOD_DISABLE)
			s6e36w4x01_write_icon_cfg(lcd);
		break;
	default:
...
int s6e36w4x01_set_aod_move (struct dsim_device *dsim, struct sclk_move_cfg_v2 *move_cfg)
{
	struct lcd_device *panel = dsim->ld;
	struct s6e36w4x01 *lcd = dev_get_drvdata(&panel->dev);

	if (move_cfg->req > SC_MASK_ENABLE) {
		pr_err("%s:Invalid request_type[%d]\n", __func__, move_cfg->req);
		return -EINVAL;
	}

	pr_info("%s[%s]\n", __func__, get_aod_mask_str(move_cfg->req));

	s6e36w4x01_set_move_cfg(SCHEDULER_EN_I, move_cfg->mask);
	s6e36w4x01_set_move_cfg(SC_MOVE_ON, move_cfg->clock_mask);
	s6e36w4x01_set_move_cfg(SB_MOVE_ON, move_cfg->blink_mask);
	s6e36w4x01_set_move_cfg(SI_MOVE_ON, move_cfg->icon_mask);
	s6e36w4x01_set_move_cfg(DSP_X_ORG, move_cfg->x);
	s6e36w4x01_set_move_cfg(DSP_Y_ORG, move_cfg->y);

	s6e36w4x01_write_move_cfg(lcd);

	return 0;
}

void s6e36w4x01_sc_enable_afpc(struct s6e36w4x01 *lcd);
void s6e36w4x01_sc_disable_afpc(struct s6e36w4x01 *lcd);
extern void s6e36w4x01_mdnie_restore(struct s6e36w4x01 *lcd, bool aod_state);
extern void s6e36w4x01_hlpm_on_10hz(struct s6e36w4x01 *lcd);
extern void s6e36w4x01_hlpm_on_30hz(struct s6e36w4x01 *lcd);
extern void s6e36w4x01_hlpm_off(struct s6e36w4x01 *lcd);
extern void s6e36w4x01_normal_aod_on(struct s6e36w4x01 *lcd);
extern void s6e36w4x01_normal_aod_off(struct s6e36w4x01 *lcd);
int s6e36w4x01_aod_enter(struct dsim_device *dsim, enum aod_mode watch_type)
{
	struct lcd_device *panel = dsim->ld;
	struct s6e36w4x01 *lcd = dev_get_drvdata(&panel->dev);
	enum aod_mode prev_aod_mode;
	ktime_t disp_time;
	...
	pr_info("%s[%s]\n", __func__, get_aod_mode_str(watch_type));

	s6e36w4x01_restore_sideram_data(lcd, watch_type);

	if (lcd->enable_afpc)
		s6e36w4x01_sc_disable_afpc(lcd);

	prev_aod_mode = lcd->aod_mode;
	lcd->aod_mode = watch_type;

	switch(watch_type) {
	case AOD_SCLK_ANALOG:
		s6e36w4x01_set_common_cfg(SC_TIME_EN, true);
		s6e36w4x01_set_common_cfg(SC_DISP_ON, true);
		s6e36w4x01_set_analog_cfg(SC_ANA_CLOCK_EN, true);
		s6e36w4x01_mdnie_restore(lcd, true);
		s6e36w4x01_set_time(lcd);
		s6e36w4x01_hlpm_on_10hz(lcd);
		s6e36w4x01_write_analog_cfg(lcd);
		s6e36w4x01_write_common_cfg(lcd);
		break;
	case AOD_SCLK_DIGITAL:
		s6e36w4x01_set_common_cfg(SC_TIME_EN, true);
		s6e36w4x01_set_common_cfg(SC_DISP_ON, true);
		s6e36w4x01_set_digital_cfg(SC_D_CLOCK_EN, true);
		s6e36w4x01_mdnie_restore(lcd, true);
		s6e36w4x01_set_time(lcd);
		s6e36w4x01_hlpm_on_30hz(lcd);
		s6e36w4x01_write_blink_cfg(lcd);
		s6e36w4x01_write_digital_cfg(lcd);
		s6e36w4x01_write_common_cfg(lcd);
		break;
	...
	case AOD_ALPM:
		...
		break;
	case AOD_NORMAL:
	case AOD_NORMAL_CHARGING:
		s6e36w4x01_normal_aod_on(lcd);
		break;
	default:
	...
	if (prev_aod_mode == AOD_DISABLE) {
		disp_time = ktime_get();
		lcd->disp_total_time += ((unsigned int)ktime_ms_delta(disp_time, lcd->disp_stime) / 1000);
	}
#if defined(ENABLE_PRINT_DISPLAY_DQA)
	s6e36w4x01_print_dqa(lcd, 1, 1);
#endif
	return 0;
}

int s6e36w4x01_aod_exit(struct dsim_device *dsim, enum aod_mode watch_type)
{
	struct lcd_device *panel = dsim->ld;
	struct s6e36w4x01 *lcd = dev_get_drvdata(&panel->dev);
	ktime_t aod_time;
	...
	switch(watch_type) {
	case AOD_SCLK_ANALOG:
		s6e36w4x01_set_common_cfg(SC_TIME_EN, false);
		s6e36w4x01_set_common_cfg(SC_DISP_ON, false);
		s6e36w4x01_set_analog_cfg(SC_ANA_CLOCK_EN, false);
		s6e36w4x01_set_icon_cfg(SI_ICON_EN, SC_MASK_ENABLE);
		s6e36w4x01_set_move_cfg(SCHEDULER_EN_I, SC_MASK_ENABLE);
		s6e36w4x01_write_move_cfg(lcd);
		s6e36w4x01_write_icon_cfg(lcd);
		s6e36w4x01_write_analog_cfg(lcd);
		s6e36w4x01_write_common_cfg(lcd);
		s6e36w4x01_hlpm_off(lcd);
		s6e36w4x01_mdnie_restore(lcd, false);
		break;
	case AOD_SCLK_DIGITAL:
		s6e36w4x01_set_common_cfg(SC_TIME_EN, false);
		s6e36w4x01_set_common_cfg(SC_DISP_ON, false);
		s6e36w4x01_set_digital_cfg(SC_D_CLOCK_EN, false);
		s6e36w4x01_set_blink_cfg(SB_BLK0_EN, SC_MASK_ENABLE);
		s6e36w4x01_set_blink_cfg(SB_BLK1_EN, SC_MASK_ENABLE);
		s6e36w4x01_set_icon_cfg(SI_ICON_EN, SC_MASK_ENABLE);
		s6e36w4x01_set_move_cfg(SCHEDULER_EN_I, SC_MASK_ENABLE);
		s6e36w4x01_write_move_cfg(lcd);
		s6e36w4x01_write_icon_cfg(lcd);
...
int s6e36w4x01_aod_enable(struct s6e36w4x01 *lcd, enum aod_mode watch_type)
{
	ktime_t disp_time;
	int ret;

	if (lcd->aod_mode != AOD_DISABLE) {
		pr_info("%s:aod already enable[%d]\n", __func__, lcd->aod_mode);
		return 0;
	}
...
int s6e36w4x01_aod_disable(struct s6e36w4x01 *lcd, enum aod_mode watch_type)
{
	ktime_t aod_time;
	unsigned int diff_time;

	if (lcd->aod_mode == AOD_DISABLE) {
		pr_info("%s:aod already disable[%d]\n", __func__, lcd->aod_mode);
		return 0;
	}
...
int s6e36w4x01_metadata_set(struct dsim_device *dsim, struct decon_metadata *metadata)
{
	...
	if (metadata->ops != METADATA_OP_AOD_SET_INFO) {
		pr_err("%s:invalid ops[%d] req[%d] mode[%d] refresh_rate[%d]\n",
	...
	if ((aod_cfg->req != AOD_SET_PANEL_REFRESH_RATE) &&
		...
		pr_err("%s:invalid req[%d] ops[%d] mode[%d] refresh_rate[%d]\n",
			__func__, metadata->aod_cfg.req, metadata->ops, metadata->aod_cfg.mode,
			metadata->aod_cfg.panel_refresh_rate);
		return -EINVAL;
	}

	if (aod_cfg->mode < AOD_NORMAL) {
		pr_err("%s:invalid mode[%d] req[%d] ops[%d] refresh_rate[%d]\n",
			__func__, metadata->aod_cfg.mode, metadata->aod_cfg.req, metadata->ops,
			metadata->aod_cfg.panel_refresh_rate);
		return -EINVAL;
	}

	if (aod_cfg->panel_refresh_rate > AOD_PANEL_REFRESH_RATE_HIGH) {
		pr_err("%s:invalid refresh_rate[%d] mode[%d] req[%d] ops[%d]\n",
			__func__, metadata->aod_cfg.panel_refresh_rate,
			metadata->aod_cfg.mode, metadata->aod_cfg.req, metadata->ops);
		return -EINVAL;
	}
	...
	pr_info("%s[%s]\n", __func__,
			get_panel_refresh_rate_str(lcd->aod_refresh));

	return 0;
}

int s6e36w4x01_aod_ctrl_enter(struct s6e36w4x01 *lcd, enum aod_mode aod_type)
{
	int ret = 0;

	pr_info("%s[%s]", __func__, get_aod_mode_str(aod_type));

	if (lcd->aod_refresh == AOD_PANEL_REFRESH_RATE_HIGH) {
		pr_info("%s:skip enter aod[%s]\n", __func__,
				get_panel_refresh_rate_str(lcd->aod_refresh));
	} else {
		ret = s6e36w4x01_aod_enable(lcd, aod_type);
		if (ret) {
			pr_err("%s:failed aod_enable[%d]\n", __func__, ret);
			return -EIO;
		}
	}

	lcd->aod_refresh_status = lcd->aod_refresh;

	return ret;
}
int s6e36w4x01_aod_ctrl_change(struct s6e36w4x01 *lcd, enum aod_mode aod_type)
{
	int ret = 0;

	pr_info("%s[%s]", __func__, get_aod_mode_str(aod_type));

	if (lcd->aod_refresh == AOD_PANEL_REFRESH_RATE_LOW) {
		if (lcd->aod_refresh_status == AOD_PANEL_REFRESH_RATE_LOW) {
			pr_info("%s:Same aod_refresh[%s]\n", __func__,
					get_panel_refresh_rate_str(lcd->aod_refresh));
		} else {
			ret = s6e36w4x01_aod_enable(lcd, aod_type);
			if (ret) {
		...
		if (lcd->aod_refresh_status == AOD_PANEL_REFRESH_RATE_HIGH) {
			pr_info("%s:Same aod_refresh[%s]\n", __func__,
					get_panel_refresh_rate_str(lcd->aod_refresh));
		} else {
			ret = s6e36w4x01_aod_disable(lcd, aod_type);
			if (ret) {
				pr_err("%s:failed aod_disable[%d]\n", __func__, ret);
				return -EIO;
			}
		}
	}
...
int s6e36w4x01_aod_ctrl_exit(struct s6e36w4x01 *lcd, enum aod_mode aod_type)
{
	int ret = 0;
	...
	if (lcd->aod_refresh_status == AOD_PANEL_REFRESH_RATE_HIGH) {
		pr_info("%s:already disable[%d]\n", __func__, lcd->aod_refresh_status);
	} else {
		ret = s6e36w4x01_aod_disable(lcd, aod_type);
		if (ret) {
			pr_err("%s:failed aod_disable[%d]\n", __func__, ret);
...
int s6e36w4x01_aod_ctrl(struct dsim_device *dsim, enum aod_state state, enum aod_mode aod_type)
{
	struct lcd_device *panel = dsim->ld;
	struct s6e36w4x01 *lcd = dev_get_drvdata(&panel->dev);
	int ret = 0;
	...
	if (state == AOD_ENTER) {
		ret = s6e36w4x01_aod_ctrl_enter(lcd, aod_type);
	} else if (state == AOD_CHANGE) {
		ret = s6e36w4x01_aod_ctrl_change(lcd, aod_type);
	} else if (state == AOD_EXIT) {
		ret = s6e36w4x01_aod_ctrl_exit(lcd, aod_type);
	} else {
...
int s6e36w4x01_afpc_set_compensation(struct dsim_device *dsim, struct afpc_compensation_v2 *comp)
{
	struct lcd_device *panel = dsim->ld;
	struct s6e36w4x01 *lcd = dev_get_drvdata(&panel->dev);
	char* addr = (char*)comp->addr;
	int ret;

	s6e36w4x01_set_afpc_cfg(SCP_MIN_BLUE, addr[AFPC_DATA_SIZE]);
	s6e36w4x01_set_afpc_cfg(SCP_MIN_GREEN, addr[(AFPC_DATA_SIZE+1)]);
	s6e36w4x01_set_afpc_cfg(SCP_MIN_RED, addr[(AFPC_DATA_SIZE+2)]);
	s6e36w4x01_set_afpc_cfg(SCP_BASE_BLUE, addr[(AFPC_DATA_SIZE+3)]);
	s6e36w4x01_set_afpc_cfg(SCP_BASE_GREEN, addr[(AFPC_DATA_SIZE+4)]);
	s6e36w4x01_set_afpc_cfg(SCP_BASE_RED, addr[(AFPC_DATA_SIZE+5)]);
	s6e36w4x01_set_afpc_cfg(SCP_BASE_OPT, addr[(AFPC_DATA_SIZE+6)]);

	if (lcd->enable_afpc)
		s6e36w4x01_sc_disable_afpc(lcd);

	ret = s6e36w4x01_update_afpc_buf(lcd, addr);
	if (ret) {
		pr_err("%s:failed update_afpc_buf[%d]\n", __func__, ret);
		return ret;
	}
	s6e36w4x01_write_sideram_afpc(lcd);
...
ssize_t s6e36w4x01_sc_store(struct device *dev,
				...
				size_t size)
{
	...
	case SC_ANALOG:
		if (1 == enable) {
			...
			s6e36w4x01_update_sample_sc(lcd, SC_ANALOG);
			s6e36w4x01_write_sideram_analog(lcd);
		}
		break;
	case SC_DIGITAL:
		if (1 == enable) {
			...
			s6e36w4x01_update_sample_sc(lcd, SC_DIGITAL);
			s6e36w4x01_write_sideram_digital(lcd);
		}
		break;
	case SC_AFPC:
		...
		if (1 == enable) {
			...
			lcd->enable_afpc = false;
			s6e36w4x01_sc_disable_afpc(lcd);
		} else {
			s6e36w4x01_update_sample_sc(lcd, SC_AFPC);
			s6e36w4x01_write_sideram_afpc(lcd);
		}
#endif
		break;
	case SC_ICON:
		if (1 == enable) {
			s6e36w4x01_set_common_cfg(SC_DISP_ON, true);
			s6e36w4x01_set_icon_cfg(SI_ICON_EN, SC_MASK_DISABLE);
			s6e36w4x01_write_icon_cfg(lcd);
			s6e36w4x01_write_common_cfg(lcd);
		} else if (0 == enable){
			s6e36w4x01_set_common_cfg(SC_DISP_ON, false);
			...
			s6e36w4x01_update_sample_sc(lcd, SC_ICON);
			s6e36w4x01_write_sideram_icon(lcd);
		}
		break;
	case SC_MOVE:
		if (1 == enable) {
			s6e36w4x01_set_move_cfg(SCHEDULER_EN_I, false);
			s6e36w4x01_set_move_cfg(DSP_X_ORG, -5);
			s6e36w4x01_set_move_cfg(DSP_Y_ORG, -5);
			s6e36w4x01_write_move_cfg(lcd);
		} else if (0 == enable){
			s6e36w4x01_set_move_cfg(SCHEDULER_EN_I, true);
			s6e36w4x01_write_move_cfg(lcd);
		}
		break;
	default:
...
ssize_t s6e36w4x01_sc_show(struct device *dev,
...
static void s6e36w4x01_sc_set_afpc_sample_cfg(void)
{
	const char afpc_cfg_sample[SC_AFPC_CFG_SIZE] = {
			0x81, 0x00, 0x01, 0xFB, 0xE9, 0xD2, 0x00,
			0x00, 0x00, 0x01, 0x03, 0x45, 0xFF, 0xFF};

	memcpy(&sc_afpc_cfg[0], &afpc_cfg_sample[0],
			sizeof(char) * SC_AFPC_CFG_SIZE);
...
void s6e36w4x01_sc_enable_afpc(struct s6e36w4x01 *lcd)
{
	...
	pr_info("%s: Can't enable in factory mode.\n", __func__);
	return;
#endif
#ifndef CONFIG_EXYNOS_DECON_LCD_S6E36W4X01_AFPC
	pr_err("%s:AFPC feature is not enabled\n", __func__);
	return;
#endif

	pr_info("%s\n", __func__);
	s6e36w4x01_set_afpc_cfg(SCP_COMPEN_ON, true);
	s6e36w4x01_write_afpc_cfg(lcd);
	panel_vsync_wait(PANEL_30HZ_1FRAME_MSEC);
	lcd->enable_afpc = true;
}

void s6e36w4x01_sc_disable_afpc(struct s6e36w4x01 *lcd)
{
	s6e36w4x01_set_afpc_cfg(SCP_COMPEN_ON, false);
	s6e36w4x01_write_afpc_cfg(lcd);
	panel_vsync_wait(PANEL_30HZ_1FRAME_MSEC);
	pr_info("%s\n", __func__);
}

void s6e36w4x01_sc_afpc_dwork(struct work_struct *work)
{
	struct s6e36w4x01 *lcd = container_of(work,
				struct s6e36w4x01, afpc_dwork.work);

	mutex_lock(&lcd->afpc_lock);

	if (lcd->power == FB_BLANK_POWERDOWN) {
		pr_err("%s:invalid dsim_state [%d]\n", __func__, lcd->power);
		goto out;
	}
