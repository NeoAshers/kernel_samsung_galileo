...
static void s6e36w3x01_write_sideram_afpc(struct s6e36w3x01 *lcd)
{
	struct dsim_device *dsim = lcd->dsim;
	int i;
	const char AFPC_2A_CMD[5] = {0x2a, 0x00, 0x00, 0x01, 0x67};
	const char AFPC_2B_CMD[5] = {0x2b, 0x00, 0x00, 0x01, 0x67};
	const char area_array[4][5] = {{0x2b, 0x00, 0x00, 0x01, 0x67},
								{0x2b, 0x00, 0x2D, 0x00, 0x59},
								{0x2b, 0x00, 0x5A, 0x00, 0x86},
								{0x2b, 0x00, 0x87, 0x01, 0x67},};
	const int index_array[4][2] = {{(AFPC_RAM_HIGHT/AFPC_WRITE_DEVIDER)*0,
				       (AFPC_RAM_HIGHT/AFPC_WRITE_DEVIDER)*1},
				     {(AFPC_RAM_HIGHT/AFPC_WRITE_DEVIDER)*1,
				       (AFPC_RAM_HIGHT/AFPC_WRITE_DEVIDER)*2},
				     {(AFPC_RAM_HIGHT/AFPC_WRITE_DEVIDER)*2,
				       (AFPC_RAM_HIGHT/AFPC_WRITE_DEVIDER)*3},
				     {(AFPC_RAM_HIGHT/AFPC_WRITE_DEVIDER)*3,
				       (AFPC_RAM_HIGHT/AFPC_WRITE_DEVIDER)*4}};

	if (lcd->afpc_write_cnt >= AFPC_WRITE_DEVIDER)
		return;
	...
	if (dsim_wr_data(dsim->id, MIPI_DSI_DCS_LONG_WRITE,
		(unsigned long)AFPC_2A_CMD,
		ARRAY_SIZE(AFPC_2A_CMD)) < 0)
		dsim_err("failed to send area_array[5].\n");

	if (dsim_wr_data(dsim->id, MIPI_DSI_DCS_LONG_WRITE,
		(unsigned long)area_array[lcd->afpc_write_cnt],
		ARRAY_SIZE(AFPC_2B_CMD)) < 0)
		dsim_err("failed to send area_array[%d]\n", lcd->afpc_write_cnt);
	...
	for (i = index_array[lcd->afpc_write_cnt][0]; i < index_array[lcd->afpc_write_cnt][1]; i++) {
		if (dsim_wr_data(dsim->id, MIPI_DSI_DCS_LONG_WRITE,
	...
	if (dsim_wr_data(dsim->id, MIPI_DSI_DCS_LONG_WRITE,
		(unsigned long)AFPC_2A_CMD,
		ARRAY_SIZE(AFPC_2A_CMD)) < 0)
		dsim_err("failed to send AFPC_2A_CMD\n");

	if (dsim_wr_data(dsim->id, MIPI_DSI_DCS_LONG_WRITE,
		(unsigned long)AFPC_2B_CMD,
		ARRAY_SIZE(AFPC_2B_CMD)) < 0)
		dsim_err("failed to send AFPC_2B_CMD\n");
						...
						char *source_addr)
{
	...
	for (i = 0, j = 0; j < AFPC_RAM_HIGHT; j++) {
		if ((j == 0) || ((j % (AFPC_RAM_HIGHT/AFPC_WRITE_DEVIDER)) == 0)) {
			lcd->afpc_buf[i] = SC_START_CMD;
			pr_debug("%s: [%02d][0x%02x]\n", __func__, j, lcd->afpc_buf[i]);
		} else
...
void s6e36w3x01_sc_enable_afpc(struct s6e36w3x01 *lcd);
void s6e36w3x01_sc_disable_afpc(struct s6e36w3x01 *lcd);
...
int s6e36w3x01_aod_enter(struct dsim_device *dsim, enum aod_mode watch_type)
{
	...
	if (lcd->enable_afpc)
		s6e36w3x01_sc_disable_afpc(lcd);
...
int s6e36w3x01_afpc_set_compensation(struct dsim_device *dsim, struct afpc_compensation_v2 *comp)
{
	...
	s6e36w3x01_update_afpc_buf(lcd, addr);

	sc_afpc_cfg[SCP_MIN_BLUE+ARRAY_OFFSET] = addr[AFPC_DATA_SIZE];
	sc_afpc_cfg[SCP_MIN_GREEN+ARRAY_OFFSET] = addr[(AFPC_DATA_SIZE+1)];
	sc_afpc_cfg[SCP_MIN_RED+ARRAY_OFFSET] = addr[(AFPC_DATA_SIZE+2)];
	sc_afpc_cfg[SCP_BASE_BLUE+ARRAY_OFFSET] = addr[(AFPC_DATA_SIZE+3)];
	sc_afpc_cfg[SCP_BASE_GREEN+ARRAY_OFFSET] = addr[(AFPC_DATA_SIZE+4)];
	sc_afpc_cfg[SCP_BASE_RED+ARRAY_OFFSET] = addr[(AFPC_DATA_SIZE+5)];
	sc_afpc_cfg[SCP_BASE_OPT+ARRAY_OFFSET] = addr[(AFPC_DATA_SIZE+6)];

	pr_info("%s: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
		__func__,
		sc_afpc_cfg[SCP_MIN_BLUE+ARRAY_OFFSET],
		sc_afpc_cfg[SCP_MIN_GREEN+ARRAY_OFFSET],
		sc_afpc_cfg[SCP_MIN_RED+ARRAY_OFFSET],
		sc_afpc_cfg[SCP_BASE_BLUE+ARRAY_OFFSET],
		sc_afpc_cfg[SCP_BASE_GREEN+ARRAY_OFFSET],
		sc_afpc_cfg[SCP_BASE_RED+ARRAY_OFFSET],
		sc_afpc_cfg[SCP_BASE_OPT+ARRAY_OFFSET]);

	if (lcd->enable_afpc)
		s6e36w3x01_sc_disable_afpc(lcd);

	lcd->afpc_write_cnt = 0;
	schedule_delayed_work(&lcd->afpc_dwork, 0);
...
void s6e36w3x01_sc_afpc_dwork(struct work_struct *work)
{
	struct s6e36w3x01 *lcd = container_of(work,
				struct s6e36w3x01, afpc_dwork.work);
