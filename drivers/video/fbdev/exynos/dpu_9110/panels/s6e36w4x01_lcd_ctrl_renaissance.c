...
#define S6E36W4X01_ID3_82	2
#define S6E36W4X01_ID3_83	3
#define S6E36W4X01_ID3_84	4
...
int s6e36w4x01_get_vbias(struct s6e36w4x01 *lcd, u32 backlightlevel)
{
	struct dsim_device *dsim = lcd->dsim;

	if (backlightlevel > s6e36w4x01_60nit_gamma_level)
		return dsim->priv.vbias;
	else
		return dsim->priv.vbias + s6e36w4x01_vbias_offset_table[backlightlevel];
}

int s6e36w4x01_gamma_ctrl(struct s6e36w4x01 *lcd, u32 backlightlevel)
{
	struct dsim_device *dsim = lcd->dsim;
	int i, count;
	...
	for (i=0; i < GAMMA_CMD_CNT; i++) {
		GAMMA_SET[i] = s6e36w4x01_gamma_set[backlightlevel][i];
	}

	for (count=0; count < 2; count++) {
		AID_SETTING[count+1] = s6e36w4x01_aid[(backlightlevel*2)+count];
	}

	ELVSS_SETTING[3] = s6e36w4x01_elvss[backlightlevel];
	...
	if (dsim->priv.panel_rev >= S6E36W4X01_ID3_83)
		VBIAS_SETTING[S6E36W4_VBIAS_SIZE] = s6e36w4x01_get_vbias(lcd, backlightlevel);

	if (lcd->enable_hop)
		VBIAS_SETTING[1] = 0x21;
	else
		VBIAS_SETTING[1] = 0x20;
...
void s6e36w4x01_hlpm_on_30hz(struct s6e36w4x01 *lcd, bool enable)
{
	...
	if (panel->panel_rev >= S6E36W4X01_ID3_82) {
		if (dsim_wr_data(dsim->id, MIPI_DSI_DCS_LONG_WRITE,
			(unsigned long) HLPM_30HZ, ARRAY_SIZE(HLPM_30HZ)) < 0)
			dsim_err("failed to send HLPM_30HZ.\n");
	}
...
void s6e36w4x01_hlpm_on_10hz(struct s6e36w4x01 *lcd, bool enable)
{
	...
	if (panel->panel_rev >= S6E36W4X01_ID3_82) {
		if (dsim_wr_data(dsim->id, MIPI_DSI_DCS_LONG_WRITE,
			(unsigned long) HLPM_10HZ, ARRAY_SIZE(HLPM_10HZ)) < 0)
			dsim_err("failed to send HLPM_10HZ.\n");
	}
	...
	pr_info("%s:on:hnit[%d]br[%d]id[%d]\n",
			__func__, lcd->hlpm_nit, brightness, panel->panel_rev);
...
void s6e36w4x01_hlpm_on_30hz(struct s6e36w4x01 *lcd, bool enable)
{
...
#ifdef CONFIG_RENAISSANCE_SMALL
		SMALL_HLPM_30HZ_84[8] = dsim->priv.vbias + SMALL_HLPM_VBIAS_OFFSET;
#endif
	...
	if (panel->panel_rev >= S6E36W4X01_ID3_84) {
#ifdef CONFIG_RENAISSANCE_SMALL
		if (dsim_wr_data(dsim->id, MIPI_DSI_DCS_LONG_WRITE,
...
#endif
	} else if (panel->panel_rev >= S6E36W4X01_ID3_82) {
		if (dsim_wr_data(dsim->id, MIPI_DSI_DCS_LONG_WRITE,
			(unsigned long) HLPM_30HZ_82, ARRAY_SIZE(HLPM_30HZ_82)) < 0)
			dsim_err("failed to send HLPM_30HZ_82.\n");
	}
...
void s6e36w4x01_hlpm_on_10hz(struct s6e36w4x01 *lcd, bool enable)
{
	...
	if (panel->panel_rev >= S6E36W4X01_ID3_84) {
		...
		if (dsim_wr_data(dsim->id, MIPI_DSI_DCS_LONG_WRITE,
			(unsigned long) HLPM_10HZ_82, ARRAY_SIZE(HLPM_10HZ_82)) < 0)
			dsim_err("failed to send HLPM_10HZ_82.\n");
	}
...
void s6e36w4x01_set_offset_comp(struct s6e36w4x01 *lcd, int temperature)
{
	if (temperature < 0) {
		temperature *= (-1);
		ELVSS_SETTING[1] = (0x80 | (temperature & 0xff));
	} else
		ELVSS_SETTING[1] = (temperature & 0xff);

	dsim_info("%s:elvss[1]=[0x%02x]\n",	__func__, ELVSS_SETTING[1]);
}

unsigned char s6e36w4x01_get_offset_comp(void)
{
	return ELVSS_SETTING[1];
}
