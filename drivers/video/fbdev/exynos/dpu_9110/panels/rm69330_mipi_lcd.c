...
void rm69330_print_dqa(struct rm69330 *lcd, int display_on, int aod)
{
...
static void rm69330_show_state(struct dsim_device *dsim)
{
...
static ssize_t rm69330_lcd_state_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct rm69330 *lcd = dev_get_drvdata(dev);
	struct dsim_device *dsim = lcd->dsim;

	rm69330_show_state(dsim);

	return 0;
}

static ssize_t rm69330_lpm_br_max_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct rm69330 *lcd = dev_get_drvdata(dev);
	int ret;
...
static ssize_t rm69330_lpm_br_normal_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct rm69330 *lcd = dev_get_drvdata(dev);
	int ret;
...
static ssize_t rm69330_lpm_br_min_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct rm69330 *lcd = dev_get_drvdata(dev);
	int ret;
...
static ssize_t rm69330_lpm_br_charging_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct rm69330 *lcd = dev_get_drvdata(dev);
	int ret;
...
static struct device_attribute rm69330_dev_attrs[] = {
	__ATTR(state, S_IRUGO, rm69330_lcd_state_show, NULL),
	...
	__ATTR(lpm_br_max, S_IRUGO , rm69330_lpm_br_max_show, NULL),
	__ATTR(lpm_br_normal, S_IRUGO , rm69330_lpm_br_normal_show, NULL),
	__ATTR(lpm_br_min, S_IRUGO , rm69330_lpm_br_min_show, NULL),
	__ATTR(lpm_br_charging, S_IRUGO, rm69330_lpm_br_charging_show, NULL),
};
...
static int rm69330_aod_ctrl(struct dsim_device *dsim, enum aod_state state, enum aod_mode aod_type)
{
	struct lcd_device *panel = dsim->ld;
	struct rm69330 *lcd = dev_get_drvdata(&panel->dev);
	...
	pr_info("%s:enable[%d]lpm_on[%d]\n", __func__, lcd->lpm_on, lcd->lpm_on);
