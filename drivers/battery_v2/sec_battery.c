...
//#define CONFIG_FEATURE_DYNAMIC_SAFTY_TIMER
...
static struct device_attribute sec_battery_attrs[] = {
	...
	SEC_BATTERY_ATTR(batt_current_test),
#endif
	SEC_BATTERY_ATTR(tct_low_current_mode),
	SEC_BATTERY_ATTR(chg_retail_mode),
	SEC_BATTERY_ATTR(wc_op_freq_strength),
	SEC_BATTERY_ATTR(batt_tune_wired_fast_charge_current),
	SEC_BATTERY_ATTR(acok_count),
	SEC_BATTERY_ATTR(pdetb_count),
	SEC_BATTERY_ATTR(charge_db_list),
	SEC_BATTERY_ATTR(charge_db_value),

	SEC_BATTERY_ATTR(device_id),
};
...
static enum power_supply_property sec_battery_props[] = {
	...
	POWER_SUPPLY_PROP_WPC_FREQ_STRENGTH,
};
...
int fg_reset = 0;
int charging_night_mode = '0';

static int sec_bat_set_charge(struct sec_battery_info *battery, int chg_mode);
...
static int sec_bat_get_wireless_expired_time(struct sec_battery_info *battery)
{
	int i;
	union power_supply_propval value = {0, };
	int expired_time = battery->pdata->expired_time;

	psy_do_property(battery->pdata->wireless_charger_name, get,
		POWER_SUPPLY_EXT_PROP_WIRELESS_TX_ID, value);

	for (i = 0; i < battery->pdata->num_tx_type; i++) {
		if (value.intval == battery->pdata->wireless_tx_type[i].tx_id) {
			expired_time = battery->pdata->wireless_tx_type[i].safety_timer * 1000;
			break;
		}
	}

	pr_debug("%s tx_id(%x) expired_time(%d)\n", __func__, value.intval, expired_time);
	return expired_time;
}

static int sec_bat_get_safty_time(struct sec_battery_info *battery)
{
	int safty_time;

	if (battery->swelling_mode == SWELLING_MODE_CHARGING)
		safty_time = battery->pdata->swell_expired_time;
	else if (battery->current_event & SEC_BAT_CURRENT_EVENT_LOW_TEMP)
		safty_time = battery->pdata->swell_expired_time;
	else if ((battery->incompatible_wpc) &&
		(is_wireless_type(battery->cable_type))) {
		safty_time = battery->pdata->incompatible_expired_time;
	} else {
		safty_time = sec_bat_get_wireless_expired_time(battery);
	}
	pr_info("%s safty_time = %d\n", __func__, safty_time);
	return safty_time;
}
...
#endif
#if !defined(CONFIG_SEC_BATT_FACTORY)
...
static int sec_bat_wpc_charaging_preset(struct sec_battery_info *battery)
{
	union power_supply_propval value = {0, };
	int chg_enabled = 0;
	...
	if (is_wireless_type(battery->cable_type)) {
		...
		chg_enabled = value.intval;
		pr_info("%s wpc charge status = %d\n", __func__, chg_enabled);

		if (chg_enabled == 0) {
			psy_do_property(battery->pdata->charger_name, get,
					POWER_SUPPLY_PROP_CHARGING_ENABLED, value);

			pr_info("%s charge status = %d\n", __func__, value.intval);
			if (value.intval == 1) {
			...
			value.intval = 1;
			psy_do_property(battery->pdata->wireless_charger_name, set,
				POWER_SUPPLY_EXT_PROP_IS_RECHARGE, value);
		}
	}
...
static int sec_bat_set_wireless_current_by_tx_id(struct sec_battery_info *battery, int* charging_current)
{
	int i;
	union power_supply_propval value = {0, };
	...
	for (i = 0; i < battery->pdata->num_tx_type; i++) {
		if (value.intval == battery->pdata->wireless_tx_type[i].tx_id) {
			*charging_current = battery->pdata->wireless_tx_type[i].chg_current;
			break;
		}
	}

	pr_debug("%s tx_id(%x) chg_currnet(%d)\n", __func__, value.intval, *charging_current);
	return 0;
}
...
static int sec_bat_set_charging_current(struct sec_battery_info *battery)
{
	...
	if (battery->lpcharge_boost) {
		if (is_wireless_type(battery->cable_type) ||is_wired_type(battery->cable_type)) {
			psy_do_property(battery->pdata->charger_name, get,
					POWER_SUPPLY_PROP_CURRENT_NOW, value);
			pr_info("%s chg_curr : %d boot_chg_curr = %d\n",
				__func__, value.intval, battery->pdata->boot_charge_current);

			if (value.intval != battery->pdata->boot_charge_current) {
				charging_current = battery->pdata->boot_charge_current;
	...
	if (battery->cable_type == SEC_BATTERY_CABLE_NONE) {
		...
		if (is_wireless_type(battery->cable_type)) {
			sec_bat_set_wireless_current_by_tx_id(battery, &charging_current);
			...
			if (battery->current_event & SEC_BAT_CURRENT_EVENT_LOW_TEMP_SWELLING) {
				...
				charging_current = (charging_current > battery->pdata->swelling_wc_low_temp_current) ?
					battery->pdata->swelling_wc_low_temp_current : charging_current;
			} else if (battery->current_event & SEC_BAT_CURRENT_EVENT_LOW_CURRENT_TCT) {
				charging_current = (charging_current > battery->pdata->tct_low_current) ?
					battery->pdata->tct_low_current : charging_current;
			}

			if (battery->incompatible_wpc) {
				charging_current = (charging_current > battery->pdata->incompatible_wpc_current) ?
					battery->pdata->incompatible_wpc_current : charging_current;
			}

			if (battery->store_mode) {
				charging_current = battery->pdata->store_mode_wpc_current;
			}
		} else {
			if (battery->current_event & SEC_BAT_CURRENT_EVENT_LOW_TEMP_SWELLING) {
...
charge_set:
	if (battery->store_mode) {
		pr_info("%s battery->charging_current(%d) charging_current(%d)\n",
			__func__, battery->charging_current, charging_current);
	}
	/* In wireless charging, must be set charging current before input current. */
	if (is_wireless_type(battery->cable_type) &&
		battery->charging_current != charging_current) {
		if (battery->store_mode) {
			if (charging_current == battery->pdata->store_mode_wpc_current) {
...
static void sec_bat_set_charging_status(struct sec_battery_info *battery,
	...
	case POWER_SUPPLY_STATUS_DISCHARGING:
		...
		battery->expired_time = sec_bat_get_safty_time(battery);
...
#if defined(CONFIG_USE_WDT_KICK_FEATURE)
...
static bool sec_bat_voltage_check(struct sec_battery_info *battery)
{
	...
	if ((battery->status == POWER_SUPPLY_STATUS_FULL) &&
		...
		if ((value.intval < battery->pdata->full_condition_soc) &&
			(battery->voltage_now < (battery->pdata->recharge_condition_vcell - 40))) {
			sec_bat_set_charging_status(battery,
					POWER_SUPPLY_STATUS_CHARGING);
			dev_info(battery->dev,
	...
	if (sec_bat_check_recharge(battery)) {
		...
		sec_bat_wpc_charaging_preset(battery);
	...
	if (!battery->swelling_mode) {
		if (((battery->temperature >= swelling_high_block) ||
			...
			if (battery->voltage_now >= battery->pdata->swelling_drop_float_voltage) {
				battery->swelling_full_check_cnt = battery->pdata->full_check_count;
			}
...
#endif
			if (battery->swelling_mode == SWELLING_MODE_FULL)
				battery->swelling_mode = SWELLING_MODE_CHARGING;
		}
	}

	if ((en_swelling && !en_rechg) || reset_safty_timer) {
		if (reset_safty_timer)
			pr_info("%s : SAFETY TIME RESET (SWELLING MODE CHARING STOP!)\n", __func__);
		else
			pr_info("%s : SAFETY TIME RESET (SWELLING Exit!)\n", __func__);
		battery->expired_time = sec_bat_get_safty_time(battery);
		battery->prev_safety_time = 0;
	}
				...
				struct sec_battery_info *battery)
{
	...
	if ((battery->cable_type == SEC_BATTERY_CABLE_USB) ||
		(battery->cable_type == SEC_BATTERY_CABLE_USB_CDP)) {
			battery->temp_highlimit_threshold =
				battery->pdata->temp_highlimit_threshold_normal + 50;
			battery->temp_high_threshold =
				battery->pdata->temp_high_threshold_normal + 50;
	}
				...
				struct sec_battery_info *battery)
{
	...
	if (battery->pdata->usb_thermal_source && !battery->usb_temp_flag) {
		if (battery->prev_usb_temp == 0)
			battery->prev_usb_temp = battery->usb_temp;

		if (battery->usb_temp - battery->prev_usb_temp >= 50) {
			pr_info("%s: prev temp : %d, current temp : %d\n",
					__func__, battery->prev_usb_temp, battery->usb_temp);
...
#endif
#endif
		} else if (battery->usb_temp >= 700) {
			pr_info("%s: usb temp over 700 : %d\n",	__func__, battery->usb_temp);
			battery->usb_temp_flag = true;
#if defined(CONFIG_BATTERY_CISD)
#if defined(CISD_NOT_SUPPORT)
...
#endif
#endif
		} else if (battery->usb_temp - temp_value >= 200) {
			pr_info("%s: usb temp over than bat temp : %d\n", __func__, battery->usb_temp -  temp_value);
			battery->usb_temp_flag = true;
#if defined(CONFIG_BATTERY_CISD)
#if defined(CISD_NOT_SUPPORT)
...
#if defined(CONFIG_PREVENT_USB_CONN_OVERHEAT)
		if (battery->usb_temp_flag && !lpcharge)
	...
	if (battery->temp_highlimit_cnt >=
#if defined(CONFIG_PREVENT_USB_CONN_OVERHEAT)
	    battery->pdata->temp_check_count || battery->usb_temp_flag) {
#else
	    battery->pdata->temp_check_count) {
#endif
		battery->health = POWER_SUPPLY_HEALTH_OVERHEATLIMIT;
		...
		if ((battery->health == POWER_SUPPLY_HEALTH_GOOD) &&
			...
			if ((temp_value > battery->pdata->swelling_high_temp_recov) ||
				...
				if (battery->cooldown_mode == 0) {
					...
					if ((pre_health == POWER_SUPPLY_HEALTH_OVERHEAT) ||
					...
					struct sec_battery_info *battery)
{
	...
	case SEC_BATTERY_FULLCHARGED_SOC_CHGPSY:
		break;
				...
				struct sec_battery_info *battery)
{
	...
	case POWER_SUPPLY_STATUS_FULL:
		if (battery->expired_time == 0) {
			...
			if (is_wireless_type(battery->cable_type)) {
				value.intval = SEC_VOUT_OFF_ENTER;
				psy_do_property(battery->pdata->wireless_charger_name, set,
					POWER_SUPPLY_EXT_PROP_WPC_VOUT_OFF, value);
			}
				...
				struct sec_battery_info *battery)
{
	...
	case SEC_BATTERY_FULLCHARGED_SOC:
		if (battery->capacity >=
			(battery->charging_mode ==
			SEC_BATTERY_CHARGING_1ST ?
			battery->pdata->full_check_soc_1st :
			battery->pdata->full_check_soc_2nd)) {
			battery->full_check_cnt++;
			dev_info(battery->dev,
				"%s: Full Check SOC (%d)\n",
				__func__,
...
not_full_charged:
	...
	if (is_wireless_type(battery->cable_type) &&
		...
		if (battery->muic_cable_type == ATTACHED_DEV_WIRELESS_POWER_SHARE_MUIC) {
			if (battery->capacity == 100) {
				/* D2D set charging status to FULL if SOC reached 100 */

				pr_info("%s set charge status full on d2d charge\n", __func__);
				...
				struct sec_battery_info *battery)
{
	...
	case SEC_BATTERY_THERMAL_SOURCE_ADC:
		...
		battery->prev_usb_temp = battery->temperature;
		battery->temperature = value.intval;
				...
				struct sec_battery_info *battery)
{
	...
	sec_debug_set_extra_info_batt(battery->capacity, battery->voltage_now, battery->temperature, battery->current_now);
#endif
}
...
static void sec_bat_swelling_fullcharged_check(struct sec_battery_info *battery)
{
	...
	if ((value.intval == POWER_SUPPLY_STATUS_FULL) ||
		...
		pr_info("%s: Swelling mode full-charged check (%d) status(%d)\n",
			__func__, battery->swelling_full_check_cnt, value.intval);

		if ((value.intval == POWER_SUPPLY_STATUS_FULL) &&
			(battery->voltage_now >= battery->pdata->swelling_drop_float_voltage)) {
			battery->swelling_full_check_cnt += battery->pdata->swell_full_check_count;
		}
	} else
		battery->swelling_full_check_cnt = 0;

	if (battery->swelling_full_check_cnt >=	battery->pdata->swell_full_check_count) {
		battery->swelling_full_check_cnt = 0;
		battery->charging_mode = SEC_BATTERY_CHARGING_NONE;
		battery->is_recharging = false;
		battery->swelling_mode = SWELLING_MODE_FULL;
...
#if defined(CONFIG_FEATURE_DYNAMIC_SAFTY_TIMER)
...
static int sec_bat_check_cool_down(struct sec_battery_info *battery, int temperature)
{
	int skip_swell_chk = 0;
	int high_swell_exit = 0;
	int block_temp, recov_temp;
	union power_supply_propval val = {0, };
	int i;
	...
	if (battery->store_mode && (battery->capacity > STORE_MODE_CHARGING_MAX))
		return 0;

	if (battery->slate_mode && (battery->status == POWER_SUPPLY_STATUS_DISCHARGING))
		return 0;

	if ((battery->lpcharge_boost) && is_wireless_type(battery->cable_type)) {
		block_temp = battery->temp_high_threshold - 10;
		recov_temp = battery->pdata->swelling_high_temp_recov;
	} else if (battery->muic_cable_type == ATTACHED_DEV_WIRELESS_POWER_SHARE_MUIC) {
		block_temp = battery->pdata->swelling_power_share_high_temp_block;
		recov_temp = battery->pdata->swelling_power_share_high_temp_recov;
	} else {
		block_temp = battery->pdata->swelling_high_temp_block;
		recov_temp = battery->pdata->swelling_high_temp_recov;
	}

	pr_info("%s temp(curr:%d block:%d recv:%d) cool(%d)\n",
		__func__, temperature, block_temp, recov_temp, battery->cooldown_mode);

	if (temperature >= block_temp) {
		if (!battery->cooldown_mode) {
			...
			if (val.intval >= 1000) {
				battery->swelling_mode = SWELLING_MODE_CHARGING;
				sec_bat_set_charge(battery, SEC_BAT_CHG_MODE_CHARGING_OFF);
				val.intval = SEC_VOUT_OFF_ENTER;
				psy_do_property(battery->pdata->wireless_charger_name, set,
					POWER_SUPPLY_EXT_PROP_WPC_VOUT_OFF, val);
			...
			if (val.intval == ACOK_INPUT) {
				val.intval = SEC_VOUT_OFF_ENTER;
				psy_do_property(battery->pdata->wireless_charger_name, set,
					POWER_SUPPLY_EXT_PROP_WPC_VOUT_OFF, val);
			}
		}
	} else if (temperature <= recov_temp) {
		if (battery->cooldown_mode) {
			pr_info("%s vout on!!\n", __func__);
			battery->swelling_mode = SWELLING_MODE_NONE;
	...
	if (battery->cooldown_mode) {
		if (battery->muic_cable_type == ATTACHED_DEV_WIRELESS_POWER_SHARE_MUIC) {
			/* sometimes ping is read as vrect.
			 * So it is needed to check weather it is normal vrect
			 */
			for (i = 0;i < 3; i++) {
				psy_do_property(battery->pdata->wireless_charger_name, get,
					POWER_SUPPLY_PROP_ENERGY_AVG, val);
			...
			if (val.intval > 0) {
				...
				/* check vout off status from wireless charger */
				if (val.intval == 0) {
					pr_info("%s vout on!!\n", __func__);
					battery->swelling_mode = SWELLING_MODE_NONE;
	...
	if ((battery->cooldown_mode) || high_swell_exit)
		skip_swell_chk = 1;

	return skip_swell_chk;
}

static void sec_bat_wireless_darkzone_check(struct sec_battery_info *battery)
{
	int vout = 0, vrect, target_vout;
	int acok;
	union power_supply_propval val = {0, };
	struct timespec c_ts = {0, };

	if (battery->pdata->darkzone_expired_time < 0)
		return;

	c_ts = ktime_to_timespec(ktime_get_boottime());

	if (!is_wireless_type(battery->cable_type)) {
		battery->darkzone_ts = c_ts;
		return;
	}

	if (battery->slate_mode || battery->store_mode) {
		battery->darkzone_ts = c_ts;
		return;
	}

	psy_do_property(battery->pdata->wireless_charger_name, get,
		POWER_SUPPLY_EXT_PROP_DARKZONE_CHECK, val);
	if (val.intval == 0) {
		battery->darkzone_ts = c_ts;
		return;
	}
	...
	psy_do_property(battery->pdata->wireless_charger_name, get,
		POWER_SUPPLY_EXT_PROP_WPC_TARGET_VOUT, val);
	target_vout = val.intval;

	psy_do_property(battery->pdata->charger_name, get,
		POWER_SUPPLY_PROP_POWER_NOW, val);
	acok = val.intval;

	pr_info("%s acok(%d) vout(%d) vrect(%d) target_vout(%d) darkzone_time(%ld %d) chg_sts(%d) chg_mode(%d)\n",
		__func__, acok, vout, vrect, target_vout,
		battery->is_darkzone == 0 ? 0 : c_ts.tv_sec - battery->darkzone_ts.tv_sec,
		battery->pdata->darkzone_expired_time,
		battery->status, battery->charging_mode);
	...
	if (((vrect < target_vout) && (vout < target_vout)) &&
		...
		if (battery->is_darkzone == 0)
			battery->darkzone_ts = c_ts;

		battery->is_darkzone = 1;
		if ((c_ts.tv_sec - battery->darkzone_ts.tv_sec) >= battery->pdata->darkzone_expired_time) {
			val.intval = POWER_SUPPLY_HEALTH_UNDERVOLTAGE;
			psy_do_property(battery->pdata->wireless_charger_name, set,
				POWER_SUPPLY_PROP_HEALTH, val);

			pr_info("%s: Unsafe voltage \n", __func__);
			queue_delayed_work(battery->monitor_wqueue, &battery->darkzone_check_work, 0);
		} else {
			if (battery->capacity >= 95) {
				psy_do_property(battery->pdata->wireless_charger_name, set,
					POWER_SUPPLY_EXT_PROP_HALF_BRIDGE_MODE, val);
			}
		}
	} else {
		battery->darkzone_ts = c_ts;
		battery->is_darkzone = 0;
	}
}

static void sec_bat_darkzone_check_work(struct work_struct *work)
{
	struct sec_battery_info *battery = container_of(work,
		struct sec_battery_info, darkzone_check_work.work);
	...
	if (vrect == 0) {
		battery->is_darkzone = 0;
		val.intval = SEC_WIRELESS_PAD_NONE;
		psy_do_property("wireless", set,
				POWER_SUPPLY_PROP_ONLINE, val);
		val.intval = SEC_WIRELESS_PAD_NONE;
		psy_do_property(battery->pdata->charger_name, set,
			POWER_SUPPLY_EXT_PROP_WPC_ONLINE, val);
	}
}
				...
				struct work_struct *work)
{
	...
	struct timespec c_ts = {0, };
	union power_supply_propval val = {0, };
	unsigned int prev_swell_mode = battery->swelling_mode;
	bool skip_wpc_ap_mode_set = false;
	int wpc_online = 0;
	int skip_swell_chk;
	union power_supply_propval value = {0, };
	...
	if (battery->polling_in_sleep) {
		...
		if ((battery->status == POWER_SUPPLY_STATUS_DISCHARGING) &&
			(battery->ps_enable != true) && !battery->store_mode) {
...
#endif
	/* -1. usb chg on/off*/
	if (((battery->cable_type == POWER_SUPPLY_TYPE_USB) || (battery->cable_type == SEC_BATTERY_CABLE_USB_CDP))
	...
	if (!sec_bat_temperature_check(battery)) {
		if (battery->health == POWER_SUPPLY_HEALTH_OVERHEAT)
			sec_bat_check_cool_down(battery, battery->temperature);
		goto continue_monitor;
	}

#if defined(CONFIG_BATTERY_SWELLING)
	/* CP is activated or boot with charging, battery temperature can raise rapidly.
	 * If monitor_initial_count or short polling is enabled, cool down can be skiped.
	 * So sec_bat_check_cool_down func is called
	 * before monitor short routine in initial monitor
	 */
	skip_swell_chk = sec_bat_check_cool_down(battery, battery->temperature);
#endif

	/* monitor short routine in initial monitor */
	if (battery->pdata->monitor_initial_count || sec_bat_is_short_polling(battery)) {
		if (((battery->pdata->full_check_type == SEC_BATTERY_FULLCHARGED_SOC) ||
			(battery->pdata->full_check_type == SEC_BATTERY_FULLCHARGED_SOC_CHGPSY)) &&
			(battery->charging_mode == SEC_BATTERY_CHARGING_1ST) &&
			(battery->status == POWER_SUPPLY_STATUS_CHARGING) &&
	...
	if (skip_swell_chk == 0) {
		...
		if (prev_swell_mode != battery->swelling_mode) {
			if ((prev_swell_mode == SWELLING_MODE_NONE) &&
				(battery->swelling_mode == SWELLING_MODE_CHARGING)) {
				if (is_wireless_type(battery->cable_type)) {
					val.intval = WPC_MODE_NORMAL;
					psy_do_property(battery->pdata->wireless_charger_name, set,
						POWER_SUPPLY_EXT_PROP_WIRELESS_CHARGE_MONITOR, val);
					skip_wpc_ap_mode_set = true;
					val.intval = WPC_MODE_SWELL_ENTER;
					psy_do_property(battery->pdata->wireless_charger_name, set,
						POWER_SUPPLY_EXT_PROP_WIRELESS_CHARGE_MONITOR, val);
				}
			} else if ((prev_swell_mode == SWELLING_MODE_CHARGING) &&
					(battery->swelling_mode == SWELLING_MODE_NONE)) {
				if (is_wireless_type(battery->cable_type)) {
					val.intval = WPC_MODE_NORMAL;
					psy_do_property(battery->pdata->wireless_charger_name, set,
						POWER_SUPPLY_EXT_PROP_WIRELESS_CHARGE_MONITOR, val);
					skip_wpc_ap_mode_set = true;
					val.intval = WPC_MODE_SWELL_EXIT;
					psy_do_property(battery->pdata->wireless_charger_name, set,
						POWER_SUPPLY_EXT_PROP_WIRELESS_CHARGE_MONITOR, val);
				}
			}
	...
	if (is_wireless_type(battery->cable_type)) {
		if ((battery->swelling_mode == SWELLING_MODE_FULL) &&
			skip_wpc_ap_mode_set)
			skip_wpc_ap_mode_set = false;
	}
#else
...
continue_monitor:
	...
	if (battery->muic_cable_type != ATTACHED_DEV_JIG_UART_OFF_MUIC)
		sec_bat_set_charging_current(battery);
...
skip_current_monitor:
	...
	if (is_wireless_type(battery->cable_type)) {
		...
		if (!skip_wpc_ap_mode_set && battery->cooldown_mode == false) {
			val.intval = WPC_MODE_NORMAL;
			psy_do_property(battery->pdata->wireless_charger_name, set,
				POWER_SUPPLY_EXT_PROP_WIRELESS_CHARGE_MONITOR, val);
		}
	...
	pr_info("%s: Sts(%s), mode(%s), Health(%s), Cable(%d %s %d %d), T(%d), "
		"SOC(%d), V(%d), I(%d), siop(%d%%), sw(%s), slate(%d), store(%d)"
#if defined(CONFIG_AFC_CHARGER_MODE)
		", HV(%s), sleep_mode(%d)"
#endif
		", st(%lu)"
		...
		", cool(%d)"
		 ...
		 battery->cable_type,
		 battery->incompatible_wpc ? "INCOMPATIBLE" : sec_cable_type[battery->wire_status],
		 battery->muic_cable_type, wpc_online,
		 battery->temperature,
		 battery->capacity,
		 battery->voltage_now,
		 battery->current_now,
		 battery->siop_level,
		 sec_bat_swell_mode_str[battery->swelling_mode],
		 battery->slate_mode,
		 battery->store_mode
#if defined(CONFIG_AFC_CHARGER_MODE)
		...
		, battery->batt_cycle
#endif
		, lpcharge, battery->cooldown_mode
		 );
#if defined(CONFIG_ENG_BATTERY_CONCEPT)
	dev_info(battery->dev,
	...
	if ((battery->cable_type != SEC_BATTERY_CABLE_NONE) &&
		...
		dev_info(battery->dev,
			 ...
			 __func__, battery->capacity, STORE_MODE_CHARGING_MAX, STORE_MODE_CHARGING_MIN,battery->status, battery->store_mode);
		...
		if ((battery->capacity <= STORE_MODE_CHARGING_MIN) &&
			(battery->status == POWER_SUPPLY_STATUS_DISCHARGING)) {
			...
			if (is_wireless_type(battery->cable_type)) {
				...
				val.intval = 1;
				psy_do_property(battery->pdata->wireless_charger_name, set,
					POWER_SUPPLY_EXT_PROP_IS_RECHARGE, val);
			}
			...
			/* set default charge current to avoid entering half bridge mode */
			value.intval = battery->pdata->charging_current[battery->cable_type].fast_charging_current;
			psy_do_property(battery->pdata->charger_name, set,
				POWER_SUPPLY_PROP_CURRENT_NOW, value);
			pr_info("%s store mode chg current set to %dmA\n", __func__, value.intval);
			msleep(1000);
			value.intval = battery->pdata->store_mode_wpc_current;
			psy_do_property(battery->pdata->charger_name, set,
				POWER_SUPPLY_PROP_CURRENT_NOW, value);
		}
	}
...
skip_monitor:
	if (is_wireless_type(battery->cable_type)) {
		...
		if ((battery->status != POWER_SUPPLY_STATUS_DISCHARGING) &&
			(val.intval == WPC_OFF_PAD)) {
			pr_info("%s abnormal status wpc %d status %d\n",
				__func__, val.intval, battery->status);

			psy_do_property(battery->pdata->wireless_charger_name, set,
				POWER_SUPPLY_PROP_ONLINE, val);
		}

		sec_bat_wireless_darkzone_check(battery);
	}
...
static void sec_bat_cable_work(struct work_struct *work)
{
	...
	if (is_wireless_type(battery->cable_type)) {
		if (battery->status == POWER_SUPPLY_STATUS_DISCHARGING) {
			psy_do_property(battery->pdata->wireless_charger_name, get,
				POWER_SUPPLY_EXT_PROP_POWER_SHARING_CHARGE, val);
			if (val.intval == 0)
		...
		battery->darkzone_ts = ktime_to_timespec(ktime_get_boottime());
		...
		if (val.intval != ACOK_INPUT) {
			...
			for (i = 0; i < 15; i++) {
				psy_do_property(battery->pdata->charger_name, get,
					POWER_SUPPLY_PROP_POWER_NOW, val);
				pr_info("%s cnt(%d) acok(%d)\n", __func__, i, val.intval);
				if (val.intval == ACOK_INPUT) {
					/* to set charge enable after vout inserted */
					msleep(100);
					break;
				}
				msleep(100);
		...
		if (battery->lpcharge_boost) {
			val.intval = WPC_MODE_BOOT;
			psy_do_property(battery->pdata->wireless_charger_name, set,
				POWER_SUPPLY_EXT_PROP_WIRELESS_CHARGE_MONITOR, val);
		}

		pr_info("%s disable acokf irq\n", __func__);
		val.intval = 0;
		psy_do_property(battery->pdata->charger_name, set,
			POWER_SUPPLY_EXT_PROP_SET_VOUT_IRQ, val);

		battery->expired_time = sec_bat_get_safty_time(battery);
	} else {
		pr_info("%s enable acokf irq\n", __func__);
		val.intval = 1;
		psy_do_property(battery->pdata->charger_name, set,
			POWER_SUPPLY_EXT_PROP_SET_VOUT_IRQ, val);
	}
	...
	if (battery->cable_type == SEC_BATTERY_CABLE_NONE ||
		...
		sec_bat_change_default_current(battery, SEC_BATTERY_CABLE_TA,
			...
			battery->pdata->default_ta_charging_current);
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case BATT_TEMP:
		...
		case SEC_BATTERY_THERMAL_SOURCE_ADC:
			...
			break;
		case SEC_BATTERY_THERMAL_SOURCE_SEC_THERMISTOR:
			sec_therm_get_temp(TYPE_SEC_THREM_BATTERY, &value.intval);
			break;
		default:
			break;
		}
	...
	case CISD_DATA:
		{
			...
			size += snprintf(temp_buf + strlen(temp_buf), sizeof(temp_buf) - size,
				"%d", pcisd->data[CISD_DATA_RESET_ALG]);
	...
	case CISD_DATA_JSON:
		{
			...
			for (j = CISD_DATA_RESET_ALG + 1; j < CISD_DATA_MAX; j++)
				size += snprintf(temp_buf + strlen(temp_buf), sizeof(temp_buf) - size,
					",\"%s\":\"%d\"", cisd_data_str[j], pcisd->data[j]);
			i += scnprintf(buf + i, PAGE_SIZE - i, "%s\n", temp_buf);
		}
		break;
	case CISD_DATA_D_JSON:
		{
			...
			for (j = CISD_DATA_FULL_COUNT_PER_DAY + 1; j < CISD_DATA_MAX_PER_DAY; j++)
				size += snprintf(temp_buf + strlen(temp_buf), sizeof(temp_buf) - size,
					",\"%s\":\"%d\"", cisd_data_str_d[j-CISD_DATA_MAX], pcisd->data[j]);
	...
	case CISD_WC_DATA:
		{
			...
			while ((pad_data != NULL) && ((pad_data = pad_data->next) != NULL) &&
				...
				size += snprintf(temp_buf + strlen(temp_buf), sizeof(temp_buf) - size,
					" 0x%02x:%d", pad_data->id, pad_data->count);
			i += scnprintf(buf + i, PAGE_SIZE - i, "%s\n", temp_buf);
		}
		break;
	case CISD_WC_DATA_JSON:
		{
			...
			size += snprintf(temp_buf + strlen(temp_buf), sizeof(temp_buf) - size,
				"\"%s\":\"%d\"", PAD_INDEX_STRING, pcisd->pad_count);
	...
	case ACOK_COUNT:
		...
		break;
	case PDETB_COUNT:
		value.intval = DEBUG_INFO_PDETB_CNT;
		psy_do_property(battery->pdata->wireless_charger_name, get,
				POWER_SUPPLY_EXT_PROP_DEBUG_INFO, value);
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
			value.intval);
		break;
	case CHG_DEBUG_LIST:
		i += scnprintf(buf + i, PAGE_SIZE - i, "time,"
			"sts,health,"
			"chg_mode,vbat,"
			"I,soc,temp,wire_status,"
			"cable,muic,on_pad,"
			"store,slate,cycle,"
			"cool,swell,"
			"vrect,vout,"
			"wpc_vbat,wpc_inow,phm,"
			"tx_id,ap_mode,freq,"
			"acok_cnt,pdetb_cnt,"
			"lpchg,wpc_err,"
			"\n");
		break;
	case CHG_DEBUG_VALUE:
	{
		int wpc_online = 0;
		int phm_mode = 0;
		int wpc_vbat = 0, wpc_inow = 0;
		int vrect,vout,acok_count_val, pdetb_count_val,wc_freq;
		u8 tx_id,ap_mode;
		u64 wpc_err_sts;
		...
		psy_do_property(battery->pdata->charger_name, get,
				POWER_SUPPLY_EXT_PROP_DEBUG_INFO, value);
		acok_count_val = value.intval;

		value.intval = DEBUG_INFO_PDETB_CNT;
		psy_do_property(battery->pdata->wireless_charger_name, get,
				POWER_SUPPLY_EXT_PROP_DEBUG_INFO, value);
		pdetb_count_val = value.intval;

		psy_do_property(battery->pdata->wireless_charger_name, get,
			POWER_SUPPLY_EXT_PROP_WIRELESS_OP_FREQ, value);
		wc_freq = value.intval;

		psy_do_property(battery->pdata->wireless_charger_name, get,
			POWER_SUPPLY_EXT_PROP_WIRELESS_TX_ID, value);
		tx_id = (u8)value.intval;

		value.intval = DEBUG_INFO_WIRELESS_STATUS;
		psy_do_property(battery->pdata->wireless_charger_name, get,
			POWER_SUPPLY_EXT_PROP_DEBUG_INFO, value);
		ap_mode = (u8)value.intval;

		value.intval = DEBUG_INFO_WIRELESS_ERR_STATUS;
		psy_do_property(battery->pdata->wireless_charger_name, get,
				POWER_SUPPLY_EXT_PROP_DEBUG_INFO, value);
		wpc_err_sts = (u64)value.int64val;

		psy_do_property(battery->pdata->wireless_charger_name, get,
			POWER_SUPPLY_PROP_PRESENT, value);
		wpc_online = value.intval;

		psy_do_property(battery->pdata->wireless_charger_name, get,
			POWER_SUPPLY_EXT_PROP_WPC_VOUT_OFF, value);
		phm_mode = value.intval;

		psy_do_property(battery->pdata->wireless_charger_name, get,
			POWER_SUPPLY_PROP_CURRENT_AVG, value);
		wpc_inow = value.intval;

		psy_do_property(battery->pdata->wireless_charger_name, get,
			POWER_SUPPLY_PROP_VOLTAGE_AVG, value);
		wpc_vbat = value.intval;

		i += scnprintf(buf + i, PAGE_SIZE - i,
			"%s,%s,"	// battery->status, battery->health,
			"%s,%d,"	// battery->charging_mode, battery->voltage_now,
			"%d,%d,%d," // battery->current_now, battery->capacity, battery->temperature,
			"%s,"		// battery->wire_status,
			"%d,%d,%d,"	// battery->cable_type, battery->muic_cable_type, wpc_online
			"%d,%d,%d," // battery->store_mode, battery->slate_mode, battery->batt_cycle,
			"%d,%s,"	// battery->cooldown_mode, battery->swelling_mode
			"%d,%d,"	// vrect, vout
			"%d,%d,%d," // wpc_vbat, wpc_inow, phm
			"%x,%x,%d," // tx_id, ap_mode, wc_freq
			"%d,%d,"	// acok_count_val, pdetb_count_val
			"%d,%llX,"	// lpcharge. wpc_err_sts
			"\n",
			sec_bat_status_str[battery->status],
			sec_bat_health_str[battery->health],
			sec_bat_charging_mode_str[battery->charging_mode],
			battery->voltage_now,
			battery->current_now, battery->capacity, battery->temperature,
			sec_cable_type[battery->wire_status],
			battery->cable_type, battery->muic_cable_type, wpc_online,
			battery->store_mode, battery->slate_mode, battery->batt_cycle,
			battery->cooldown_mode,
			sec_bat_swell_mode_str[battery->swelling_mode],
			vrect, vout,
			wpc_vbat, wpc_inow, phm_mode,
			tx_id, ap_mode, wc_freq,
			acok_count_val, pdetb_count_val,
			lpcharge, wpc_err_sts);
		break;
	}
	case DEVICE_ID:
					...
					const char *buf, size_t count)
{
	...
	case BATT_EVENT_LCD:
		if (sscanf(buf, "%10d\n", &x) == 1) {
			...
			if (battery->pdata->single_charger_path == false) {
				get_monotonic_boottime(&ts);
				if (x) {
					battery->lcd_status = true;
				} else {
	...
	case BATT_TUNE_FAST_CHARGE_CURRENT:
		...
		if(x >= 0 && x <= 4000 ){
			battery->pdata->default_charging_current = x;
			battery->pdata->default_usb_charging_current = x;
			battery->pdata->default_ta_charging_current = x;
			for(i=0; i < SEC_BATTERY_CABLE_MAX; i++)
		...
		break;
	case BATT_TUNE_WIRED_FAST_CHARGE_CURRENT:
		sscanf(buf, "%10d\n", &x);
		pr_info("%s fast charge current = %d mA",__func__, x);
		if(x >= 0 && x <= 4000 ){
			battery->pdata->default_usb_charging_current = x;
	...
	case BATT_CURRENT_EVENT:
		break;
#if defined(CONFIG_TIZEN_SEC_KERNEL_ENG)
	case BATT_CURRENT_TEST:
		if (sscanf(buf, "%10d\n", &x) == 1) {
			value.intval = x;
			pr_info("BATT_CURRENT_TEST chg_current = %d\n", x);
			psy_do_property(battery->pdata->charger_name, set,
				POWER_SUPPLY_PROP_CURRENT_NOW, value);
			ret = count;
		}
	...
	case DEVICE_ID:{
		char device_id[33] ={0};
		if (sscanf(buf, "%32s\n", device_id) == 1) {
			battery->device_id = kzalloc(strlen(device_id)+1, GFP_KERNEL);
			strcpy(battery->device_id, device_id);
			pr_info("DEVICE_ID: %s", battery->device_id);
			ret = count;
		}
		break;
	}
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_HV_DISABLE:
			...
			break;
		case POWER_SUPPLY_EXT_PROP_OVERHEAT_HICCUP:
#if defined(CONFIG_PREVENT_USB_CONN_OVERHEAT)
			pr_info("%s: overheat hiccup recover!(%d)\n", __func__, val->intval);
			if (val->intval && battery->usb_temp_flag) {
...
#endif
			break;
		case POWER_SUPPLY_EXT_PROP_INCOMPATIBLE_WPC:
			if ((battery->incompatible_wpc == false) && (val->intval == 1))
				calc_safty_time = 1;

			battery->incompatible_wpc = val->intval;
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		if (is_hv_wireless_type(battery->cable_type) ||
			...
			if (battery->incompatible_wpc)
				val->intval = SEC_BATTERY_CABLE_INCOMPATIBLE_WPC;
			else {
				if (battery->muic_cable_type == ATTACHED_DEV_WIRELESS_POWER_SHARE_MUIC)
					val->intval = SEC_BATTERY_CABLE_WPC_POWER_SHARE;
				else
					val->intval = battery->cable_type;
			}
	...
	case POWER_SUPPLY_PROP_WPC_FREQ_STRENGTH:
		if (is_wireless_type(battery->cable_type)) {
			psy_do_property(battery->pdata->wireless_charger_name, get,
				POWER_SUPPLY_PROP_WPC_FREQ_STRENGTH, value);
			pr_info("%s : wpc op_freq strength(%d) muic_cable_type(%d)\n",
				__func__, value.intval, battery->muic_cable_type);
			val->intval = value.intval;
		} else {
			val->intval = 0;
		}
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_CHARGE_POWER:
			...
			break;
		case POWER_SUPPLY_EXT_PROP_SWELLING_MODE:
			val->intval = battery->swelling_mode;
			break;
		case POWER_SUPPLY_EXT_PROP_SLATE_MODE:
			val->intval = (int)battery->slate_mode;
			break;
		...
		case POWER_SUPPLY_EXT_PROP_BOOT_BOOST_CHARGE:
			...
			break;
		case POWER_SUPPLY_EXT_PROP_COOL_DOWN_MODE:
			val->intval = battery->cooldown_mode;
			break;
		case POWER_SUPPLY_EXT_PROP_DEVICE_ID:
			val->strval = battery->device_id;
			break;
		default:
			return -EINVAL;
		}
			       ...
			       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		psy_do_property(battery->pdata->wireless_charger_name, get,
			POWER_SUPPLY_PROP_PRESENT, value);
		val->intval = (value.intval == WPC_ON_PAD) ? 1 : 0;
		break;
	case POWER_SUPPLY_PROP_PRESENT:
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		pr_info("%s online wc_status = %d intval = %d\n",
			__func__, battery->wc_status, val->intval);
		...
		unsigned long action, void *data)
{
	...
	sec_bat_set_misc_event(battery, BATT_MISC_EVENT_UNDEFINED_RANGE_TYPE,
#if !defined(CONFIG_ENG_BATTERY_CONCEPT) && !defined(CONFIG_SEC_BATT_FACTORY)
	...
	if (!strcmp(cmd, "DETACH")) {
		if (is_wireless_type(battery->cable_type) &&
			...
			battery->incompatible_wpc = false;
			battery->cooldown_mode = false;
#if defined(CONFIG_BATTERY_CISD)
		...
		struct sec_battery_info *battery)
{
	...
	if (!np) {
		...
		ret = of_property_read_u32(np, "full_check_soc_1st", &pdata->full_check_soc_1st);
		if (ret) {
			pr_info("%s: full_check_soc_1st is Empty. Set default value 100\n", __func__);
			pdata->full_check_soc_1st = 100;
		}
		ret = of_property_read_u32(np, "full_check_soc_2nd", &pdata->full_check_soc_2nd);
		if (ret) {
			pr_info("%s: full_check_soc_2nd is Empty. Set default value 100\n", __func__);
			pdata->full_check_soc_2nd = 100;
		}
	...
	pdata->default_ta_charging_current = pdata->charging_current[SEC_BATTERY_CABLE_TA].fast_charging_current;
#ifdef CONFIG_SEC_BATT_FACTORY
	...
	ret = of_property_read_u32(np,
			   "battery,incompatible_expired_time", &temp);
	if (ret) {
		pr_info("incompatible_expired_time time is empty\n");
		pdata->incompatible_expired_time = 8 * 60 * 60;
	} else {
		pdata->incompatible_expired_time = (unsigned int) temp;
	}
	pdata->incompatible_expired_time *= 1000;

	ret = of_property_read_u32(np,
			   "battery,swell_expired_time", &temp);
	if (ret) {
		pr_info("swell_expired_time time is empty\n");
		pdata->swell_expired_time = 8 * 60 * 60;
	} else {
		pdata->swell_expired_time = (unsigned int) temp;
	}
	pdata->swell_expired_time *= 1000;
	...
	ret = of_property_read_u32(np, "battery,swell_full_check_count",
		&pdata->swell_full_check_count);
	if (ret) {
		pr_info("%s : Swell Full check count is Empty\n", __func__);
		pdata->swell_full_check_count = pdata->full_check_count;
	}
	...
	ret = of_property_read_u32(np, "battery,incompatible_wpc_current",
				   &pdata->incompatible_wpc_current);
	if (ret) {
		pr_info("%s: incompatible_wpc_current is Empty\n", __func__);
		pdata->incompatible_wpc_current = 75;
	}
	...
	ret = of_property_read_u32(np, "battery,swelling_power_share_high_temp_block",
				   &temp);
	pdata->swelling_power_share_high_temp_block = (int)temp;
	if (ret)
		pr_info("%s: swelling power share high temp block is Empty\n", __func__);

	ret = of_property_read_u32(np, "battery,swelling_power_share_high_temp_recov",
				   &temp);
	pdata->swelling_power_share_high_temp_recov = (int)temp;
	if (ret)
		pr_info("%s: swelling power share high temp recovery is Empty\n", __func__);
	...
	ret = of_property_read_u32(np, "battery,darkzone_expired_time",
			&pdata->darkzone_expired_time);
	if (ret) {
		pdata->darkzone_expired_time = -1;
		pr_err("%s: darkzone_expired_time is Empty. set to %d\n",
			 __func__, pdata->darkzone_expired_time);
	}
...
#endif
	p = of_get_property(np, "battery,tx_type", &len);
	if (p) {
		pdata->num_tx_type = len / sizeof(struct sec_bat_tx_type);
		pdata->wireless_tx_type = kzalloc(len, GFP_KERNEL);

		ret = of_property_read_u32_array(np, "battery,tx_type",
				 (u32 *)pdata->wireless_tx_type, len/sizeof(u32));
		if (ret) {
			pr_err("%s: failed to read battery,tx_type: %d\n", __func__, ret);
			if (pdata->wireless_tx_type)
				kfree(pdata->wireless_tx_type);
			pdata->wireless_tx_type = NULL;
			pdata->num_tx_type = 0;
		} else {
			for (i = 0; i < pdata->num_tx_type; i++)
				pr_info("wireless_tx_type tx_id = 0x%02x powerchg_current_hold = %d safety_timer(%d)\n",
					pdata->wireless_tx_type[i].tx_id,
					pdata->wireless_tx_type[i].chg_current,
					pdata->wireless_tx_type[i].safety_timer);
		}
	}
...
static void sec_bat_parse_mode_dt(struct sec_battery_info *battery)
{
	...
	if (battery->store_mode) {
		ret = of_property_read_u32(np, "battery,store_wpc_current",
			&pdata->store_mode_wpc_current);
		if (ret) {
			pr_info("%s: store_wpc_current is Empty\n", __func__);
			pdata->store_mode_wpc_current = 75;
		}

		pr_info("%s pdata->store_mode_wpc_current = %d\n",
			__func__, pdata->store_mode_wpc_current);
...
static void sec_bat_lpcharge_work(struct work_struct *work)
{
	...
	if (is_wireless_type(battery->cable_type)) {
		...
		if (battery->cooldown_mode == false) {
			...
			value.intval = WPC_MODE_NORMAL;
			psy_do_property(battery->pdata->wireless_charger_name, set,
				POWER_SUPPLY_EXT_PROP_WIRELESS_CHARGE_MONITOR, value);
		}
	}

	wake_unlock(&battery->lpcharge_wake_lock);
}
...
static int sec_battery_probe(struct platform_device *pdev)
{
...
#endif
	wake_lock_init(&battery->lpcharge_wake_lock, WAKE_LOCK_SUSPEND,
			"sec-lpcharge_wake_lock");
	...
	INIT_DELAYED_WORK(&battery->darkzone_check_work, sec_bat_darkzone_check_work);
	...
	wake_lock(&battery->lpcharge_wake_lock);
	battery->lpcharge_boost = 1;
	queue_delayed_work(battery->monitor_wqueue,
		&battery->lpcharge_work, msecs_to_jiffies(55000));
...
#if defined(CONFIG_STORE_MODE) && !defined(CONFIG_SEC_BATT_FACTORY)
	battery->store_mode = true;
	sec_bat_parse_mode_dt(battery);
#endif
	...
	if (value.intval == SEC_BATTERY_CABLE_NONE) {
		if (chg_value.intval == POWER_SUPPLY_STATUS_CHARGING) {
			for (i = 0; i < 30; i++) {
				msleep(100);
				psy_do_property("battery", get,
					POWER_SUPPLY_PROP_ONLINE, value);
...
#endif
	wake_lock_destroy(&battery->lpcharge_wake_lock);
...
static void sec_battery_complete(struct device *dev)
{
	...
	if (battery->status == POWER_SUPPLY_STATUS_DISCHARGING) {
		if (battery->capacity > value.intval) {
			/* max dynamic soc range is max soc value - full_condition_soc value */
			if ((battery->capacity - value.intval) >=
				(100 - battery->pdata->full_condition_soc)) {
				psy_do_property(battery->pdata->fuelgauge_name, set,
					POWER_SUPPLY_EXT_PROP_BATTERY_CAPACITY_UPDATE, value);
				pr_info("%s BATTERY_CAPACITY_UPDATE\n", __func__);
			}
		}
	}
