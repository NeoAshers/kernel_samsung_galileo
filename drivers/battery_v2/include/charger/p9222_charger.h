...
#define P9222_TX_DATA_COM_TRY_SEND			0x06

#define P9222_TX_DATA_VAL_TRY_SEND_BT_ADDRESS		0xff
...
struct p9222_charger_platform_data {
	...
	int num_of_bt_support;
	u32 *tx_id_with_bt_support;
...
struct p9222_charger_data {
	...
	struct wake_lock wpc_tx_id_check_lock;
	...
	struct wake_lock phm_free_lock;
	struct delayed_work phm_free_work;

	struct wake_lock cs100_lock;
	struct delayed_work cs100_work;

	struct wake_lock unsafe_voltage_chk_lock;
	...
	struct wake_lock align_check_lock;
	struct delayed_work align_check_work;
	int d2d_vout_buf[P9222_OP_FREQ_CHECK_BUF_NUM];
	int d2d_vrect_buf[P9222_OP_FREQ_CHECK_BUF_NUM];
	int d2d_align_chk_cnt;
	int d2d_align_detach_cnt;
	int d2d_vout_strength;
	int vout_avr;
	int op_freq_offset;
	int align_check_start;
	int max_freq_strength_cnt;
	struct timespec align_check_star_time;
	...
	int irq_wpc_det_enabled;
	int irq_wpc_det_wakeup_enabled;
	int phm_set_fail;

	/*debug info*/
	int pdetb_count;
	unsigned int uno_off_cnt;
	unsigned int under_voltage_cnt;
	unsigned int ot_cnt;
	unsigned int wtd_err_cnt;
	unsigned int low_iout_cnt;

	int low_current_detected;
	struct timespec low_current_detect_time;

	int curr_acok_cnt;
	int prev_acok_cnt;

	struct timespec uno_off_ts;
