...
const char* new_node_name = "bcm47755B1";
...
static int __init gps_bcm477x_init(void)
{
	...
	if (of_property_read_string(np, "gps-node-name", &bcm477x.name))
		bcm477x.name = (char *)matched_np->data;

#if defined(CONFIG_PULSE)
	if (system_rev == 4)
		bcm477x.name = (char *)new_node_name;

	pr_info("[SSP_GPS] system_rev = %d, name = %s\n", system_rev, bcm477x.name);
#endif
