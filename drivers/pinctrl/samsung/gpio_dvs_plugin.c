...
const static struct of_device_id secgpio_dvs_dt_match[] = {
	{ .compatible = "samsung,exynos9110-secgpio-dvs",
		.data = (void *)&exynos9110_secgpio_dvs },
	{ },
};
	...
	char __user *buffer, size_t count, loff_t *ppos)
{
	...
	struct gpio_dvs_private_data* gpio_data = file->private_data;
#ifdef CONFIG_OF
	const struct of_device_id *match;
	struct device_node *node;
#endif

	if (*ppos < 0 || !count || gpio_data == NULL)
		return -EINVAL;
	...
	if (*ppos == 0) {
		...
		if (dev && dev->platform_data) {
			...
			gpio_data->buf = kmalloc(buf_size, GFP_KERNEL);
			if (!gpio_data->buf)
				return -ENOMEM;
			ret = show_gpio_readable(gdvs, gpio_data->buf, ret);
		} else {
			gpio_data->buf = kmalloc(SIZE_PER_LINE, GFP_KERNEL);
			if (!gpio_data->buf)
				return -ENOMEM;
			ret = snprintf(gpio_data->buf, SIZE_PER_LINE, "%s", \
				"Can not retrive GPIO DVS information\n");
		}

		if (ret <= count) {
			size_for_copy = ret;
			gpio_data->rest_size = 0;
		} else {
			size_for_copy = count;
			gpio_data->rest_size = ret - size_for_copy;
		}
	} else {
		if (gpio_data->rest_size <= count) {
			size_for_copy = gpio_data->rest_size;
			gpio_data->rest_size = 0;
		} else {
			size_for_copy = count;
			gpio_data->rest_size -= size_for_copy;
		}
	}

	if (size_for_copy >  0) {
		int offset = (int) *ppos;
		if (copy_to_user(buffer, gpio_data->buf + offset , size_for_copy)) {
			if (gpio_data->buf) {
				kfree(gpio_data->buf);
				gpio_data->buf = NULL;
			}
...
static int gpio_dvs_open_gpio_state(struct inode* inode, struct file* file) {
	struct gpio_dvs_private_data* gpio_data = kmalloc(sizeof(struct gpio_dvs_private_data), GFP_KERNEL);
...
static int gpio_dvs_release_gpio_state(struct inode* inode, struct file* file) {
	struct gpio_dvs_private_data* gpio_data = file->private_data;
...
static const struct  file_operations gpio_dvs_fops = {
	.owner = THIS_MODULE,
	.open = gpio_dvs_open_gpio_state,
	.release = gpio_dvs_release_gpio_state,
