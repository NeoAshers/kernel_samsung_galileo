	...
	if (ws->event_count) {
		...
		deleted_ws.emon_total_time =
			ktime_add(deleted_ws.emon_total_time,
				  ws->emon_total_time);
		deleted_ws.batr_total_time =
			ktime_add(deleted_ws.batr_total_time,
				  ws->batr_total_time);
#endif
...
#ifdef CONFIG_ENERGY_MONITOR
        deleted_ws.emon_active_count += ws->emon_active_count;
        deleted_ws.emon_wakeup_count += ws->emon_wakeup_count;
        deleted_ws.batr_active_count += ws->batr_active_count;
        deleted_ws.batr_wakeup_count += ws->batr_wakeup_count;
#endif
...
void wakeup_source_add(struct wakeup_source *ws)
{
	...
	ws->batr_last_time = ws->last_time;
#endif
...
static void wakeup_source_activate(struct wakeup_source *ws)
{
	...
	ws->last_time = ktime_get();
#ifdef CONFIG_ENERGY_MONITOR
	ws->emon_active_count++;
	ws->emon_last_time = ws->last_time;
	ws->batr_active_count++;
	ws->batr_last_time = ws->last_time;
#endif
...
static void wakeup_source_report_event(struct wakeup_source *ws)
{
	...
	if (events_check_enabled) {
		ws->wakeup_count++;
#ifdef CONFIG_ENERGY_MONITOR
		ws->emon_wakeup_count++;
		ws->batr_wakeup_count++;
#endif
	}
...
#ifdef CONFIG_ENERGY_MONITOR
	ws->emon_total_time = ktime_add(ws->emon_total_time,
							ktime_sub(now, ws->emon_last_time));
	ws->emon_last_time = now;
	ws->batr_total_time = ktime_add(ws->batr_total_time, duration);
	ws->batr_last_time = now;
#endif
...
void pm_get_large_wakeup_count(int type,
				struct emon_wakeup_source *ws_array,  size_t n)
{
	int i;
	struct wakeup_source *ws;
	unsigned long emon_active_count;
	...
	for (i = 0; i < n; i++) {
		...
		strncpy(ws_array[i].name, "no_ws", strlen("no_ws") + 1);
		ws_array[i].emon_active_count = 0;
		ws_array[i].emon_wakeup_count = 0;
	}

	i = 0;
	rcu_read_lock();
	list_for_each_entry_rcu(ws, &wakeup_sources, entry) {
		if (ws->emon_wakeup_count == 0)
			continue;

		emon_active_count = ws->emon_active_count;
		emon_wakeup_count = ws->emon_wakeup_count;

		pr_debug("%s: name=%s active=%d active_count=%ld wakeup_count=%ld\n",
			__func__, ws->name, ws->active, emon_active_count, emon_wakeup_count);

		if (i < n) {
			memcpy(ws_array[i].name, ws->name, WS_NAME_SIZE-1);
			ws_array[i].emon_active_count = emon_active_count;
			ws_array[i].emon_wakeup_count = emon_wakeup_count;
			i++;
			if (i == n)
				sort(&ws_array[0],
					n,
					sizeof(struct emon_wakeup_source),
					pm_emon_wakeup_count_cmp_func, NULL);
		} else {
			if (emon_wakeup_count > ws_array[n-1].emon_wakeup_count) {
				memcpy(ws_array[n-1].name, ws->name, WS_NAME_SIZE-1);
				ws_array[n-1].emon_active_count = emon_active_count;
				ws_array[n-1].emon_wakeup_count = emon_wakeup_count;
				sort(&ws_array[0],
					n,
					sizeof(struct emon_wakeup_source),
					pm_emon_wakeup_count_cmp_func, NULL);
		...
		if (type != ENERGY_MON_TYPE_DUMP) {
			ws->emon_active_count = 0;
			ws->emon_wakeup_count = 0;
		}
	}
	rcu_read_unlock();
...
void pm_get_large_wakeup_sources_batr(
				struct emon_wakeup_source *ws_array,  size_t n)
{
	int i;
	struct wakeup_source *ws;
	ktime_t batr_total_time;
	ktime_t active_time;
	ktime_t zero = ktime_set(0, 0);
	ktime_t now;
	...
	list_for_each_entry_rcu(ws, &wakeup_sources, entry) {
		if (!ws->active &&
			ktime_to_ms(ws->batr_total_time) == ktime_to_ms(zero))
			continue;

		now = ktime_get();
		batr_total_time = ws->batr_total_time;
		if (ws->active) {
			active_time = ktime_sub(now, ws->batr_last_time);
			batr_total_time = ktime_add(batr_total_time, active_time);
		}
		pr_debug("%s: name=%s active=%d total_time=%lld\n",
			__func__, ws->name, ws->active, ktime_to_ms(batr_total_time));

		if (i < n) {
			memcpy(ws_array[i].name, ws->name, WS_NAME_SIZE-1);
			ws_array[i].emon_total_time = batr_total_time;
			i++;
			if (i == n)
				sort(&ws_array[0],
					n,
			...
			if (ktime_compare(batr_total_time,
					ws_array[n-1].emon_total_time) == 1) {
				memcpy(ws_array[n-1].name, ws->name, WS_NAME_SIZE-1);
				ws_array[n-1].emon_total_time = batr_total_time;
				sort(&ws_array[0],
					n,
					sizeof(struct emon_wakeup_source),
					pm_emon_cmp_func, NULL);
