		...
		int fd, u64 *va_pages, u64 *flags, u32 padding)
{
	...
	if (IS_ERR_OR_NULL(dma_buf)) {
		/* MALI_SEC_DEBUG */
		dev_err(kctx->kbdev->dev, "Failed to get from %d", fd);
		goto no_buf;
	}
	...
	if (!dma_attachment) {
		/* MALI_SEC_DEBUG */
		dev_err(kctx->kbdev->dev, "Failed to attach");
		goto no_attachment;
	}
	...
	*va_pages = (PAGE_ALIGN(dma_buf->size) >> PAGE_SHIFT) + padding;
	if (!*va_pages) {
		/* MALI_SEC_DEBUG */
		dev_err(kctx->kbdev->dev, "No va pages");
		goto bad_size;
	}
	...
	*flags &= ~BASE_MEM_SAME_VA;
	...
	if (!reg) {
		/* MALI_SEC_DEBUG */
		dev_err(kctx->kbdev->dev, "No va region");
		goto no_region;
	}
	...
	if (IS_ERR_OR_NULL(reg->gpu_alloc)) {
		/* MALI_SEC_DEBUG */
		dev_err(kctx->kbdev->dev, "Failed to alloc");
		goto no_alloc_obj;
	}
	...
	if (kbase_update_region_flags(kctx, reg, *flags) != 0) {
		/* MALI_SEC_DEBUG */
		dev_err(kctx->kbdev->dev, "Failed to update region");
		goto invalid_flags;
	}
		...
		u64 *flags)
{
	...
	if (!reg) {
		/* MALI_SEC_DEBUG */
		dev_err(kctx->kbdev->dev, "No va region from type[%d]", type);
		goto no_reg;
	}
	...
	if (*flags & (BASE_MEM_SAME_VA | BASE_MEM_NEED_MMAP)) {
		...
		*gpu_va <<= PAGE_SHIFT;
		...
		if (kbase_gpu_mmap(kctx, reg, 0, *va_pages, 1) != 0) {
			/* MALI_SEC_DEBUG */
			dev_err(kctx->kbdev->dev, "Fail to mmap");
			goto no_gpu_va;
		}
		/* return real GPU VA */
		*gpu_va = reg->start_pfn << PAGE_SHIFT;
		...
		if (kbase_add_va_region(kctx, reg, 0, *va_pages, 1) != 0) {
			/* MALI_SEC_DEBUG */
			dev_err(kctx->kbdev->dev, "Fail to add va region");
			goto no_gpu_va;
		}
		/* return real GPU VA */
		*gpu_va = reg->start_pfn << PAGE_SHIFT;
