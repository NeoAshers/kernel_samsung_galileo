		...
		size_t nr_pages, size_t align)
{
	...
	if (!tmp) {
		/* MALI_SEC_DEBUG */
		dev_err(kctx->kbdev->dev, "Failed to find/insert va region");
		err = -ENOMEM;
	}
...
int kbase_gpu_mmap(struct kbase_context *kctx, struct kbase_va_region *reg, u64 addr, size_t nr_pages, size_t align)
{
	...
	if (err) {
		/* MALI_SEC_DEBUG */
		dev_err(kctx->kbdev->dev, "Failed to add va region");
		return err;
	}
	...
	if (reg->gpu_alloc->type == KBASE_MEM_TYPE_ALIAS) {
		...
		if (err) {
			/* MALI_SEC_DEBUG */
			dev_err(kctx->kbdev->dev, "Failed to insert pages");
			goto bad_insert;
		}
