				...
				struct drm_file *file_priv)
{
	...
	add_gem_queue_work(current->comm);
#endif
	...
	if (sec_debug_get_debug_level()) {
		if (tbm_gem_obj->buffer) {
			...
			DRM_INFO("%s:sz[%d]f[0x%x]h[%d]obj[%p]to[%d]\n",
				"ga", (int)args->size, args->flags, args->handle,
				obj, gv_gem_alloc_size);
		}
	}
#endif
	return 0;
