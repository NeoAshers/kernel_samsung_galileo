...
int tdm_irq_enable_vblank(struct drm_device *drm_dev, unsigned int crtc)
{
	...
	case TDM_CRTC_PRIMARY:
		...
		hrtimer_start(&tdm_priv->virtual_vblank,
...
static void tdm_irq_disable_vblank(struct drm_device *drm_dev, unsigned int crtc)
{
	struct tgm_drv_private *dev_priv = drm_dev->dev_private;
	struct tdm_private *tdm_priv = dev_priv->tdm_priv;
