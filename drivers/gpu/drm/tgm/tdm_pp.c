		...
		u32 fmt, struct tdm_sz *sz)
{
	...
	case DRM_FORMAT_YVU444:
		ofs[0] = (uint64_t)sz->hsize * (uint64_t)sz->vsize;
		ofs[1] = ofs[2] = ofs[0] >> 2;
	...
	case DRM_FORMAT_XRGB8888:
		ofs[0] = (uint64_t)sz->hsize * (uint64_t)sz->vsize << 2;
		if (*base[0]) {
			if (size[0] < ofs[0])
				goto err_info;
		}
		...
		struct tdm_pp_queue_buf *qbuf)
{
	...
	if (pp_is_m2m_cmd(property->cmd)) {
		if (property->type & PP_EVENT_DRIVEN) {
			...
			ret = pp_start_property(ppdrv, c_node, true);
			if (ret) {
		...
		struct drm_file *file)
{
	...
	case PP_CTRL_PLAY:
		...
		mutex_lock(&ppdrv->drv_lock);
		ret = pp_start_property(ppdrv, c_node, false);
		if (ret) {
			DRM_INFO("%s:failed to start property:id[%d]\n"
...
int pp_start_property(struct tdm_ppdrv *ppdrv,
		struct tdm_pp_cmd_node *c_node, bool used_lock)
{
	struct tdm_pp_mem_node *m_node;
