...
int tdm_get_irq(void)
{
	return tgm_irq;
}

void tdm_set_irq(int irq)
{
	tgm_irq = irq;
}
...
static void (*tdm_reset_block)(void);
void tdm_register_reset_block(void (*callback)(void))
{
	tdm_reset_block = callback;
}
...
#ifndef CONFIG_DRM_TDM_FRAME_DONE
		...
		struct tdm_control_dpms *req, struct drm_file *file)
{
	...
	if (req->type == DPMS_EVENT_DRIVEN) {
		...
		dpms_work->event = e;
#ifdef CONFIG_DISPLAY_EARLY_DPMS
		if (tdm_priv->early_dpms_force)
			dpms_work->early_dpms = false;
		else
			...
			void *data, struct drm_file *file)
{
	...
	tdm_priv->early_dpms_force = true;
#endif

	ret = tdm_handle_dpms(tdm_priv, req, file);

	DRM_INFO("[%s][%d][%s][%u]ret[%d]\n", __func__, req->dpms,
		req->type ? "ASYNC" : "SYNC", tdm_priv->vblank_count, ret);

#ifdef CONFIG_DISPLAY_EARLY_DPMS
	tdm_priv->early_dpms_force = false;
#endif
...
static void tdm_free_frame_done_event(struct drm_pending_event *event)
{
	if (event)
		kfree(event);
}

static struct drm_file *tdm_f_sync_file = NULL;
int tdm_set_frame_done(struct drm_device *dev,
			void *data, struct drm_file *file)
{
	tdm_f_sync_file = file;

	DRM_INFO("Use frame done event\n");
...
{
	struct tdm_send_frame_done_event *e = NULL;
	unsigned long flags;

	if (!tdm_f_sync_file || req_num < 0)
		return 0;
	...
	if (!e) {
		DRM_ERROR("failed to allocate event.\n");
		spin_lock_irqsave(&dev->event_lock, flags);
		tdm_f_sync_file->event_space += sizeof(e->event);
		spin_unlock_irqrestore(&dev->event_lock, flags);
		return -1;
	}

	e->event.base.type = TDM_FRAME_DONE_EVENT;
	e->event.base.length = sizeof(e->event);
	e->event.crtc_id = TDM_CRTC_PRIMARY;
	e->event.num = req_num;

	e->base.event = &e->event.base;
	e->base.file_priv = tdm_f_sync_file;
	e->base.destroy =  tdm_free_frame_done_event;
			...
			unsigned long cmd, void *_data)
{
	...
	case DISPLAY_EARLY_DPMS_MODE_SET: {
		...
		if (!tdm_priv->early_dpms && dpms == DRM_MODE_DPMS_OFF && tdm_priv->dpms_working == false)
			tdm_priv->early_dpms = early_dpms;

		tdm_priv->dpms_working = true;
	}
		break;
...
int tdm_init(struct drm_device *drm_dev)
{
	...
	ret = tdm_irq_init(drm_dev);
	if (ret)
		DRM_ERROR("failed to tdm_irq_init.\n");
