...
static char manufacturer_string[MAX_STR_SIZE];
static char product_string[MAX_STR_SIZE];
static char serial_string[MAX_STR_SIZE];
				   ...
				   const char *buf, size_t size)
{
	...
	pr_info("acm_instances_store: acm_instances = %d\n", config->instances);
			       ...
			       struct usb_composite_dev *cdev)
{
	...
	net = rndis_opts->net;
	/* remove the log message. security review request.
	...
	*/
	...
	config->org_ctl_inf_bInterfaceClass =
		rndis_opts->ctl_intf->bInterfaceClass;
	config->org_ctl_inf_bInterfaceSubClass =
		rndis_opts->ctl_intf->bInterfaceSubClass;
	config->org_ctl_inf_bInterfaceProtocol =
					...
					const char *buf, size_t size)
{
	...
	return snprintf(config->manufacturer, MAX_STR_SIZE, "%s", buf);
}
			       ...
			       struct usb_configuration *c)
{
	...
	if (c->bConfigurationValue == USB_CONFIGURATION_1) {
		...
		dev_err(&smdev->dev, "Not supported configuration(%d)\n",
...
#if defined(CONFIG_USB_NOTIFY_LAYER) && defined(CONFIG_USB_NOTIFIER)
			    ...
			    const char *buff, size_t size)
{
	...
	if (!o_notify) {
		dev_err(pdev, "There is no otg_notify!!\n");
		return -ENODEV;
	}
	send_otg_notify(o_notify, NOTIFY_EVENT_VBUS, enabled);
	dev_info(pdev, "Forcely NOTIFY_EVENT_VBUS issue(%d)!!\n", enabled);
	ret = size;
#else
	dev_info(pdev, "There is no handler for usb phy ctl!!\n");
	ret = -ENODEV;
#endif
...
#define DESCRIPTOR_ATTR(field, format_string, base)	\
static ssize_t								\
...
field ## _store(struct device *dev, struct device_attribute *attr,	\
		const char *buf, size_t size)	\
{	\
	return snprintf(buffer, MAX_STR_SIZE, "%s", buf);	\
}	\
static DEVICE_ATTR(field, S_IRUGO | S_IWUSR, field ## _show, field ## _store);
...
static int slp_multi_usb_bind(struct usb_composite_dev *cdev)
{
	...
	ret = slp_multi_init_functions(smdev);
	if (ret)
		goto err_multi_usb_bind;
...
slp_multi_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *ctrl)
{
	...
	if (smdev) {
		...
		list_for_each_entry(av_f, &smdev->available_functions,
			...
			/* Check again it was added configuration or not */
			list_for_each_entry(en_f,
				&smdev->funcs_fconf, fconf_list) {
				if (av_f == en_f) {
					en_f_config |= USB_CONFIGURATION_1;
					break;
				}
			}
			list_for_each_entry(en_f,
				&smdev->funcs_sconf, sconf_list) {
				if (av_f == en_f) {
					en_f_config |= USB_CONFIGURATION_2;
					break;
				}
			}
			if (en_f_config) {
				value = av_f->ctrlrequest(av_f, cdev, ctrl);
				if (value >= 0)
					break;
			}
		}
				...
				struct slp_multi_platform_data *pdata)
{
	...
	smdev->id = err;
	smdev->ustate = USB_STATE_NOTATTACHED;
	smdev->mtp_mode = pdata->mtp_mini_mode;
	...
	while ((f = *functions++)) {
		for (i = 0; i < pdata->nfuncs; i++)
			if ((!f->used) &&
				(!strcmp(pdata->enable_funcs[i].name,
				f->name))) {
...
static int slp_multi_probe(struct platform_device *pdev)
{
	...
	struct slp_multi_platform_data *pdata;
	struct slp_multi_fn *enable_funcs;
	const char *en_func_str;
	int i, err;
	...
	if (np) {
		...
		if (of_find_property(np, "slp,slp_multi-enable-funcs", NULL)) {
			...
			enable_funcs = devm_kzalloc(&pdev->dev,
				pdata->nfuncs * sizeof(struct slp_multi_fn),
					GFP_KERNEL);
			if (!enable_funcs) {
				dev_err(&pdev->dev, "unable to allocate platform data\n");
				return -ENOMEM;
			...
			for (i = 0; i < pdata->nfuncs; i++) {
				err = of_property_read_string_index(pdev->dev.of_node,
					"slp,slp_multi-enable-funcs",
						i, &en_func_str);
				if (err) {
					dev_err(&pdev->dev, "Can't get %dth str from DTS\n", i);
					continue;
				}

				enable_funcs[i].name = devm_kzalloc(&pdev->dev,
					(strlen(en_func_str) + 1), GFP_KERNEL);
				if (!enable_funcs[i].name)
					return -ENOMEM;

				strncpy(enable_funcs[i].name,
					en_func_str, strlen(en_func_str));
		...
		if (of_find_property(np, "slp,slp_multi-mtp-mini-mode", NULL))
			pdata->mtp_mini_mode = true;
	} else {
	...
	/*
	 * The Tizen gadget driver disconnects the gadget
	 * on bind and expects the gadget to stay disconnected until
	 * it calls usb_gadget_connect when userspace is ready.
	 * Internally, UDC core calls usb_gadget_connect automatically
	 * when the gadget_driver binded to UDC in usb_composite_probe
	 * So re-set by false to avoid enabling the pullup
	 * before userspace is ready.
	 */
	if (smdev->cdev && smdev->cdev->gadget)
		usb_udc_vbus_handler(smdev->cdev->gadget, false);
	...
	dev_info(&pdev->dev, "usb_mode driver, version:%s init OK\n",
		 USB_MODE_VERSION);
	return 0;
 err_comp_prb:
	list_del(&smdev->list);
