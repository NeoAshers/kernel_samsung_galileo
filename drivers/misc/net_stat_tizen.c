...
struct net_stat_ndev_stat {
	...
	unsigned up;
	unsigned down;
	ktime_t total_up_time;
	ktime_t last_up_time; /* time stamp for total up time calc */
	...
	for (i = 0; i < net_stat.num_registered_ndev; i++) {
		...
		if (stat->ndev) {
			if (stat->ndev->reg_state <= NETREG_REGISTERED) {
				...
				total_up_time = stat->total_up_time;
				if (stat->up - stat->down > 0) {
					ktime_t now, active_time;

					now = ktime_get_boottime();
					active_time = ktime_sub(now, stat->last_up_time);
					total_up_time = ktime_add(total_up_time, active_time);
				}
				...
				seq_printf(m, "%-12s  "
							...
							"%-12llu  %-12llu\n",
							stat->ndev->name,
							stat->up, ktime_to_ms(total_up_time),
							stat->down,
			     ...
			     unsigned long event, void *ptr)
{
	...
	case NETDEV_UP:
		stat->up++;
		now = ktime_get_boottime();
		stat->last_up_time = now;
		break;
	case NETDEV_DOWN:
		stat->down++;
		now = ktime_get_boottime();
		duration = ktime_sub(now, stat->last_up_time);
		stat->total_up_time = ktime_add(stat->total_up_time, duration);
		break;
	default:
		break;
	}
...
#ifdef CONFIG_ENERGY_MONITOR
int net_stat_tizen_get_stat(struct net_stat_tizen_emon *emon_stat,
	size_t n)
{
	struct net_stat_ndev_stat *stat;
	struct rtnl_link_stats64 temp;
	...
	for (i = 0; i < n; i++) {
		...
		emon_stat[i].up_time = total_up_time;
		...
		emon_stat[i].bt_active_time = stat->bt_stat.active_time;
		emon_stat[i].bt_tx_time = stat->bt_stat.tx_time;
		emon_stat[i].bt_rx_time = stat->bt_stat.rx_time;

		emon_stat[i].scan_time = stat->wifi_stat.total_scan_time;
...
int net_stat_tizen_get_stat_emon(int type,
 	struct net_stat_tizen_emon *emon_stat, size_t n)
{
	...
	for (i = 0; i < n; i++) {
		...
		total_up_time = ktime_sub(stat->total_up_time, stat->emon_up_time);
		if (emon_stat[i].state > 0) {
			ktime_t now, active_time;
			...
			if (type != ENERGY_MON_TYPE_DUMP) {
				stat->total_up_time = ktime_add(stat->total_up_time, active_time);
				stat->last_up_time = now;
			}
		}
		emon_stat[i].up_time = total_up_time;
		...
		if (type != ENERGY_MON_TYPE_DUMP) {
			...
			stat->emon_up_time = stat->total_up_time;
