...
static long misc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GNSS_IOCTL_FAULT:
		gif_err("%s: GNSS_IOCTL_FAULT\n", iod->name);

		if (!gc->ops.gnss_force_set_fault) {
			gif_err("%s: !gc->ops.gnss_force_set_fault\n", iod->name);
			return -EINVAL;
		}
		gc->ops.gnss_force_set_fault(gc);
		return 0;
		...
		unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case GNSS_IOCTL_LOAD_FIRMWARE:
		gif_info_limited("%s: GNSS_IOCTL_LOAD_FIRMWARE (32-bit)\n", iod->name);
		return parsing_load_firmware32(iod, realarg);
	case GNSS_IOCTL_READ_FIRMWARE:
		gif_info_limited("%s: GNSS_IOCTL_READ_FIRMWARE (32-bit)\n", iod->name);
