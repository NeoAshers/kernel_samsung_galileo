...
struct gnss_data {
	...
	char *rfic_regulator_name;
	struct regulator *rfic_regulator;
};
...
#define GNSS_DELAY_SET_RFIC 2000 /* 2ms */
...
#define gif_info_limited(fmt, ...) \
		printk_ratelimited(KERN_INFO "%s: " pr_fmt(fmt), __func__, ##__VA_ARGS__)
