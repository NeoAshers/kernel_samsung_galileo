...
static struct gnss_data *gnss_if_parse_dt_pdata(struct device *dev)
{
	...
	pdata->gpio_rfic_reset = of_get_named_gpio(dev->of_node, "gpio_rfic_reset", 0);
	if (gpio_is_valid(pdata->gpio_rfic_reset)) {
		int err;

		gif_info("gpio_rfic_reset: %d\n", pdata->gpio_rfic_reset);
		err = gpio_request(pdata->gpio_rfic_reset, "gpio_rfic_reset");
		if (err) {
			gif_err("Unable to request gpio_request: %d\n", err);
			gpio_free(pdata->gpio_rfic_reset);
		} else {
			gpio_direction_output(pdata->gpio_rfic_reset, 0);
		}
	}

	of_property_read_string(dev->of_node, "rfic_regulator_name",
				(const char **)&pdata->rfic_regulator_name);
	gif_info("rfic_regulator_name: %s\n", pdata->rfic_regulator_name);
...
static int gnss_probe(struct platform_device *pdev)
{
	...
	if (pdata->rfic_regulator_name) {
		pdata->rfic_regulator = devm_regulator_get_optional(dev, pdata->rfic_regulator_name);
		if (!IS_ERR(pdata->rfic_regulator)) {
			int ret;

			/* Output is on, and its mode changed by TCXOEN */
#ifdef CONFIG_SOC_EXYNOS9110
			ret = s2m_ldo15_set_mode(2);
			if (ret) {
				gif_err("rfic regulator set_mode failed\n");
				goto free_iod;
			}
			gif_info("s2m_ldo15_set_mode ret = %d\n", ret);
#endif
			gif_info("enable rfic_regulator %s\n", pdata->rfic_regulator_name);
			ret = regulator_enable(pdata->rfic_regulator);
			if (ret) {
				gif_err("rfic regulator enable failed\n");
				goto free_iod;
			}
		} else {
			gif_err("rfic regulator initialization failed\n");
			goto free_iod;
		}
	}
