...
static irqreturn_t kepler_wdt_isr(int irq, void *arg)
{
	...
	spin_lock_irqsave(&gc->wdt_irq_lock, flags);
	if (gc->wdt_irq_active) {
		disable_irq_nosync(gc->wdt_irq);
		gc->wdt_irq_active = false;
	}
	spin_unlock_irqrestore(&gc->wdt_irq_lock, flags);
...
static int kepler_hold_reset(struct gnss_ctl *gc)
{
	...
	if (gpio_is_valid(gc->gnss_data->gpio_rfic_reset)) {
		gpio_set_value(gc->gnss_data->gpio_rfic_reset, 0);
		udelay(GNSS_DELAY_SET_RFIC);
		gif_info("set gpio_rfic_reset 0\n");
	}
...
static int kepler_force_set_fault(struct gnss_ctl *gc)
{
	struct io_device *iod = gc->iod;

	gif_err("%s+++\n", __func__);

	gif_info("Force Trigger fault\n");
	if (!wake_lock_active(&gc->gc_fault_wake_lock))
		wake_lock_timeout(&gc->gc_fault_wake_lock, HZ);
...
static int kepler_release_reset(struct gnss_ctl *gc)
{
	...
	if (gpio_is_valid(gc->gnss_data->gpio_rfic_reset)) {
		gpio_set_value(gc->gnss_data->gpio_rfic_reset, 1);
		udelay(GNSS_DELAY_SET_RFIC);
		gpio_set_value(gc->gnss_data->gpio_rfic_reset, 0);
		udelay(GNSS_DELAY_SET_RFIC);
		gpio_set_value(gc->gnss_data->gpio_rfic_reset, 1);
		udelay(GNSS_DELAY_SET_RFIC);
		gif_info("set gpio_rfic_reset 5us\n");
	}
	...
	spin_lock_irqsave(&gc->wdt_irq_lock, flags);
	if (!gc->wdt_irq_active) {
		enable_irq(gc->wdt_irq);
		gc->wdt_irq_active = true;
	}
	spin_unlock_irqrestore(&gc->wdt_irq_lock, flags);
...
static int kepler_power_on(struct gnss_ctl *gc)
{
	...
	if (gpio_is_valid(gc->gnss_data->gpio_rfic_reset)) {
		gpio_direction_output(gc->gnss_data->gpio_rfic_reset, 0);
		udelay(GNSS_DELAY_SET_RFIC);
		gpio_set_value(gc->gnss_data->gpio_rfic_reset, 1);
		udelay(GNSS_DELAY_SET_RFIC);
		gpio_set_value(gc->gnss_data->gpio_rfic_reset, 0);
...
static int kepler_resume(struct gnss_ctl *gc)
{
	...
	if (gc->pmu_ops->check_mif_req) {
		if (gc->pmu_ops->check_mif_req() == 1) {
			gif_info("Trigger fault to clear mif request\n");
			if (!wake_lock_active(&gc->gc_fault_wake_lock))
				wake_lock_timeout(&gc->gc_fault_wake_lock, HZ);
...
static void gnss_get_ops(struct gnss_ctl *gc)
{
	...
	gc->ops.gnss_force_set_fault = kepler_force_set_fault;
}
...
int init_gnssctl_device(struct gnss_ctl *gc, struct gnss_data *pdata)
{
	...
	gc->wdt_irq = irq;
	ret = devm_request_irq(&pdev->dev, gc->wdt_irq, kepler_wdt_isr, 0,
			  "kepler_wdt_handler", gc);
	if (ret) {
		gif_err("Request irq fail - kepler_wdt_isr(%d)\n", ret);
		return ret;
	}
	spin_lock_init(&gc->wdt_irq_lock);
	enable_irq_wake(irq);
	disable_irq(gc->wdt_irq);
	gc->wdt_irq_active = false;
