...
int get_cp_status_read(struct device *dev, char *buf)
{
#define CP_STAT_ADDR	0x11860038

	int cnt;
	void __iomem *reg;

	reg = devm_ioremap(dev, CP_STAT_ADDR, SZ_64);

	cnt = sprintf(buf, "CP_STAT: 0x%08x\n", __raw_readl(reg));

	devm_iounmap(dev, reg);
		...
		struct device_attribute *attr, char *buf)
{
	...
	cnt += get_cp_status_read(dev, buf + cnt);
	return cnt;
}
							...
							long long *raw_val, int check_level, int caller_type)
{
	...
	if (caller_type == SLEEP_MONITOR_CALL_SUSPEND) {
		for (i = 0; i < modem_mon_idx; i++) {
			...
			tx_bytes += ndev->stats.tx_bytes - modem_smon[i].tx_bytes;
...
static int modem_probe(struct platform_device *pdev)
{
	...
	if (shm_get_cplog_flag()) {
		ret = security_request_cp_ram_logging();
		if (ret)
			goto free_iod;
	}
#endif
