				  ...
				  unsigned int out, unsigned int count)
{
	...
	if (unlikely(space < count)) {
		if (cp_online(mc)) {
			mif_err_limited("%s: %d NOSPC %s_TX{qsize:%d in:%d out:%d space:%d len:%d}\n",
				ld->name, mld->nr_busy_cnt, dev->name, qsize,
				in, out, space, count);
		}
		return -ENOSPC;
	}
	...
	for (i = 0; i < MAX_SIPC_MAP; i++) {
		...
		if (unlikely(ret < 0)) {
			if (ret == -EBUSY || ret == -ENOSPC) {
				...
				mask |= msg_mask(dev);
				if (mld->nr_busy_cnt > 500) {
					shmem_forced_cp_crash(mld, CRASH_REASON_MIF_TX_ERR,
						"tx_frames_to_dev error/nr_busy_cnt");
				}
				continue;
			} else {
				shmem_forced_cp_crash(mld, CRASH_REASON_MIF_TX_ERR,
...
static int rx_net_frames_from_zerocopy_adaptor(struct sbd_ring_buffer *rb, int budget)
{
	int rcvd = 0;
	struct link_device *ld = rb->ld;
	struct mem_link_device *mld = ld_to_mem_link_device(ld);
	...
	num_frames = min_t(unsigned int, zerocopy_adaptor_usage(zdptr), budget);
#else
	num_frames = zerocopy_adaptor_usage(zdptr);
#endif
...
int mem_netdev_poll(struct napi_struct *napi, int budget)
{
	...
	if (rb->zerocopy)
		rcvd = rx_net_frames_from_zerocopy_adaptor(rb, budget);
	else
...
#else
			if (rb->zerocopy)
				rcvd = rx_net_frames_from_zerocopy_adaptor(rb, 0);
			else
...
#else
	mif_err("mode=%u, param2=0x%lx, param3=0x%lx, cp_base_addr=0x%lx\n",
