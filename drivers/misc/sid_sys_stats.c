	...
	if (uid >= 5000) {
		...
		if (special_usid == -1) {
			...
			if (!err) {
				if (!strcmp(ctx, "User")) {
					special_usid = usid;
					pr_info("%s: found special_usid: %d\n",
	...
	hash_for_each_possible(sid_entry->pid_entries, pid_entry, hash, key) {
		if (key == pid_entry->key) {
			pr_debug("%s: exist_pid_entry: %s %u: %s %u",
				__func__, task->group_leader->comm, key, pid_entry->comm, pid_entry->key);
			return pid_entry;
		}
	}

	pr_debug("%s: no_pid_entry: %s %u\n", __func__, task->group_leader->comm, key);
		...
		struct task_struct *task)
{
	...
	key = full_name_hash(NULL, task->group_leader->comm, strlen(task->group_leader->comm));
	...
	pid_entry->pid = pid;
	get_task_comm(pid_entry->comm, task->group_leader);
	pid_entry->key = key;
...
static int sid_cputime_show(struct seq_file *m, void *v)
{
	...
	do_each_thread(temp, task) {
		...
		if ((usid < 5000 && sid != 5) || usid == special_usid)
...
#ifdef CONFIG_ENERGY_MONITOR
static int sid_emon_is_whitelist(struct pid_entry *entry)
{
	/* hard code irq name for now, need to get from device tree */
	if (strncmp(entry->comm, "launchpad-loade", sizeof("launchpad-loade")) == 0)
		return 1;
...
int get_sid_cputime(struct sid_sys_stats *stat_array, size_t n)
{
	struct sid_entry *sid_entry = NULL;
	struct task_struct *task, *temp;
	cputime_t profile_time = 0;
	...
	hash_for_each(sid_hash_table, bkt, sid_entry, hash) {
		cputime_t total_utime = sid_entry->utime + sid_entry->active_utime;
		cputime_t total_stime = sid_entry->stime + sid_entry->active_stime;
		cputime_t total_time = total_utime + total_stime;
		u32 permil = (u32)div_u64(total_time, profile_time);
...
#ifdef CONFIG_PID_SYS_STATS
int get_sid_cputime_emon(int type, struct sid_sys_stats *sid_array,
			struct sid_sys_stats *pid_array, size_t n)
{
	...
	hash_for_each(sid_hash_table, bkt, sid_entry, hash) {
		...
		hash_for_each(sid_entry->pid_entries, bkt_pid, pid_entry, hash) {
			cputime_t total_pid_utime;
			cputime_t total_pid_stime;
			cputime_t total_pid_time;
			u32 permil;

			if (sid_emon_is_whitelist(pid_entry))
				continue;

			total_pid_utime = pid_entry->utime + pid_entry->active_utime -
								pid_entry->emon_utime;
			total_pid_stime = pid_entry->stime + pid_entry->active_stime -
								pid_entry->emon_stime;
			total_pid_time = total_pid_utime + total_pid_stime;
			permil = (u32)div_u64(total_pid_time, profile_time);
...
#else
int get_sid_cputime_emon(int type,
