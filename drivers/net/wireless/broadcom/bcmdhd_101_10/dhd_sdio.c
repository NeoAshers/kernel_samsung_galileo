...
#if defined(CONFIG_BCM43012) && defined(SOFTAP_FIRMWARE_430131)
	{
		extern int op_mode;
		if ((op_mode == DHD_FLAG_HOSTAP_MODE) &&
			...
			(bus->sih->chiprev == 1) &&
			(bus->fw_path != NULL)) {
			int softap_fw_len = strlen(SOFTAP_FIRMWARE_430131);

			strncpy(bus->fw_path, SOFTAP_FIRMWARE_430131, softap_fw_len);
			bus->fw_path[softap_fw_len] = '\0';

			DHD_ERROR(("%s, 430131 sofap fw(%s)\n", __FUNCTION__, bus->fw_path));
		}
	}
#endif
...
#define BCM430131_SUFFIX "_c0"
static int concate_revision_bcm43013(dhd_bus_t *bus, char *fw_path, char *nv_path)
{
	uint32 chip_ver;
	char chipver_tag[10] = {0, };
	...
	chip_ver = bus->sih->chiprev;
	if (chip_ver == 2) {
		DHD_ERROR(("----- CHIP 430132 -----\n"));
	} else if (chip_ver == 1) {
		DHD_ERROR(("----- CHIP 430131 -----\n"));
		strncat(chipver_tag, BCM430131_SUFFIX, strlen(BCM430131_SUFFIX));
	} else {
		DHD_ERROR(("----- Unknown chip version, ver=%x -----\n", chip_ver));
		return -1;
	}

	strncat(fw_path, chipver_tag, strlen(chipver_tag));
	strncat(nv_path, chipver_tag, strlen(chipver_tag));

	DHD_ERROR(("%s : fw_path = %s, nv_path = %s\n",
...
concate_revision(dhd_bus_t *bus, char *fw_path, char *nv_path)
{
	...
	case BCM43454_CHIP_ID:
		...
		break;
	case BCM43012_CHIP_ID:
		res = concate_revision_bcm43013(bus, fw_path, nv_path);
...
bool dhd_is_430131(struct dhd_bus *bus, char *suffix)
{
	if ((bus->sih->chip == BCM43012_CHIP_ID) &&
		(bus->sih->chiprev == 1)) {
		strncpy(suffix, BCM430131_SUFFIX, strlen(BCM430131_SUFFIX));
		return TRUE;
	}
