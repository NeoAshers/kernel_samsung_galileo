...
#elif defined(BCM43012_CHIP_) || defined(BCM43013_CHIP)
...
extern bool dhd_is_430131(struct dhd_bus *bus, char *suffix);
...
dhd_set_blob_support(dhd_pub_t *dhdp, char *fw_path)
{
	...
	{
		char suffix[10] = {0, };
		char blob_path_430131[100] = {0, };

		if (dhd_is_430131(dhdp->bus, suffix)) {
			int left;
			strncpy(blob_path_430131, filepath, sizeof(blob_path_430131)-1);

			left = sizeof(blob_path_430131) - strlen(blob_path_430131);
			if (left <= strlen(suffix)) {
				DHD_ERROR(("%s, check blob_path(%s)\n",
					__FUNCTION__, blob_path_430131));
				return;
			}
			strncat(blob_path_430131, suffix, strlen(suffix));

			filepath = blob_path_430131;
		}
		DHD_ERROR(("%s blob file path(%s)\n", __FUNCTION__, filepath));
	}
#endif /* CONFIG_BCM43012 */
