...
static void exynos_pd_power_on_pre(struct exynos_pm_domain *pd)
{
	...
	pd->last_time = ktime_get();
	pd->start_on_time = pd->last_time;
...
static void exynos_pd_power_off_post(struct exynos_pm_domain *pd)
{
	...
	pd->last_time = ktime_get();
	delta = ktime_sub(pd->last_time, pd->start_on_time);
	pd->total_on_time = ktime_add(pd->total_on_time, delta);
...
static int exynos_pd_power_on(struct generic_pm_domain *genpd)
{
	...
	DEBUG_PRINT_INFO(EXYNOS_PD_PREFIX"%s(%s)+\n", __func__, pd->name);
