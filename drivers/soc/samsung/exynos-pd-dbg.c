...
static void exynos_pd_dbg_summary_show(struct generic_pm_domain *genpd)
{
	...
	total_on_time = pd->total_on_time;
	if (genpd->status == GPD_STATE_ACTIVE) {
		ktime_t now = ktime_get();

		active_time = ktime_sub(now, pd->last_time);
		total_on_time = ktime_add(total_on_time, active_time);
	}

	pr_info("[GENPD] : %-30s [GPD_STATUS] : %-15s %lld %lld\n",
			genpd->name, gpd_status_lookup[genpd->status],
			ktime_to_ms(total_on_time), ktime_to_ms(pd->last_time));
