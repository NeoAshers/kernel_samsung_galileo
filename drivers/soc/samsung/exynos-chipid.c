...
const char *soc_ap_id;
EXPORT_SYMBOL(soc_ap_id);
...
static void __init exynos_chipid_get_chipid_info(void)
{
	...
	exynos_soc_info.chip_id = (val >> data->chip_id_bit) & EXYNOS_REV_MASK;
...
static ssize_t chipid_evt_ver_show(struct kobject *kobj,
			         struct kobj_attribute *attr, char *buf)
{
	if (exynos_soc_info.chip_id == 1)
		return snprintf(buf, 14, "EVT0.2\n");
	else if (exynos_soc_info.revision == 0)
