...
extern void update_frames_and_vsync(unsigned int *frames, unsigned int *vsync);
#endif

#if defined(CONFIG_SLP_BUSY_LEVEL)
int cpu_busy_level_enable = 1;
unsigned long high_cpu_load_boot_completed;
#endif

int sample_count = 0;
int sample_count_busy = 0;

struct delayed_work work_detect_busy_level;
struct workqueue_struct *work_detect_busy_level_queue;

void work_detect_busy_level_function(struct work_struct *work)
{
	if (sample_count_busy *100 / sample_count >= cpu_busy_level_threshold) {
		cpu_busy_level = TARGET_BUSY_STATE;
		busy_level_time_trigger[BUSY_LEVEL_TRIGGER_CPU_LOAD] = busy_state_get_time_occur();
		update_cpu_busy_level(BUSY_LEVEL_TRIGGER_CPU_LOAD);
		cancel_delayed_work(&work_detect_busy_level);
	} else {
		pr_info("[B_LEVEL] escape %2d %3d\n", sample_count_busy, sample_count);
	}
...
void store_cpu_load(unsigned int cpu_load[])
{
...
#if 0
	static unsigned long interval_time, before_time, curr_time;

	curr_time= cpu_clock(UINT_MAX);

	interval_time = (unsigned long) curr_time - before_time;
	interval_time = interval_time/1000000;
...
#else
		cpu_load_freq_history[cnt].cpu_load[j] = cpu_load[j] * 100 / 1024;
#endif
	}
...
#if defined(CONFIG_FRAME_DROP_DETECTOR)
	update_frames_and_vsync(&cpu_load_freq_history[cnt].frames, &cpu_load_freq_history[cnt].vsync);
#endif
	...
	if ((high_cpufreq >= 1000000)
		&& (high_usage >= 90)) {
		if (!delayed_work_pending(&work_detect_busy_level)) {
			queue_delayed_work(work_detect_busy_level_queue, &work_detect_busy_level, msecs_to_jiffies(5 * TIME_CHECK_DELAY));
			sample_count_busy++;
		} else {
			sample_count_busy++;
		}
	...
	if (sample_count_busy) sample_count++;
#endif
...
#if defined(CONFIG_SLP_BUS_CLK_CHECK_LOAD)
{
	...
	if (performance_monitor_en == 1)
		performance_monitor_manager(cnt);
#endif
...
#if defined(CONFIG_FRAME_DROP_DETECTOR)
			, cpu_load_freq_history_view[cnt].frames
			, cpu_load_freq_history_view[cnt].vsync
#endif
