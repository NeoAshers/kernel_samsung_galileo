...
static struct itmon_rpathinfo rpathinfo[] = {
	/* Data BUS */
	{0,	"CPU",		"CPU_MEM0",	GENMASK(0, 0),	0},

	{0,	"MODEM0",	"CP_MEM0",	GENMASK(2, 0),	0},
	{1,	"ABOX",		"CP_MEM0",	GENMASK(2, 0),	0},
	{2,	"MODEM1",	"CP_MEM0",	GENMASK(2, 0),	0},
	{3,	"WLBT",		"CP_MEM0",	GENMASK(2, 0),	0},
	{4,	"DIT",		"CP_MEM0",	GENMASK(2, 0),	0},

	{0,	"ISP",		"RT_MEM0",	GENMASK(2, 0),	0},
	{1,	"DPU",		"RT_MEM0",	GENMASK(2, 0),	0},
	{2,	"ABOX",		"RT_MEM0",	GENMASK(2, 0),	0},
	{3,	"MODEM1",	"RT_MEM0",	GENMASK(2, 0),	0},
	{4,	"WLBT",		"RT_MEM0",	GENMASK(2, 0),	0},
	{5,	"GNSS",		"RT_MEM0",	GENMASK(2, 0),	0},
	{6,	"MODEM0",	"RT_MEM0",	GENMASK(2, 0),	0},
	{7,	"DIT",		"RT_MEM0",	GENMASK(2, 0),	0},

	{0,	"M_NRT0",	"NRT_MEM0",	GENMASK(0, 0),	0},
	{1,	"SIREX",	"NRT_MEM0",	GENMASK(0, 0),	0},

	{0,	"APM",		"S_NRT0",	GENMASK(3, 0),	0},
	{1,	"CHUB",		"S_NRT0",	GENMASK(3, 0),	0},
	{2,	"VTS",		"S_NRT0",	GENMASK(3, 0),	0},
	{3,	"WLBT",		"S_NRT0",	GENMASK(3, 0),	0},
	{4,	"COREX",	"S_NRT0",	GENMASK(3, 0),	0},
	{5,	"CSSYS",	"S_NRT0",	GENMASK(3, 0),	0},
	{6,	"DIT",		"S_NRT0",	GENMASK(3, 0),	0},
	{7,	"FSYS",		"S_NRT0",	GENMASK(3, 0),	0},
	{8,	"G3D",		"S_NRT0",	GENMASK(3, 0),	0},
	{9,	"GNSS",		"S_NRT0",	GENMASK(3, 0),	0},
	{10,	"MFCMSCL",	"S_NRT0",	GENMASK(3, 0),	0},
	{11,	"MODEM1",	"S_NRT0",	GENMASK(3, 0),	0},

	/* Peri BUS */
	{0,	"CPU",		"PERI",		GENMASK(4, 0),	0},
	{1,	"ABOX",		"PERI",		GENMASK(4, 0),	0},
	{2,	"GNSS",		"PERI",		GENMASK(4, 0),	0},
	{3,	"ISP",		"PERI",		GENMASK(4, 0),	0},
	{4,	"MFCMSCL",	"PERI",		GENMASK(4, 0),	0},
	{5,	"MODEM0",	"PERI",		GENMASK(4, 0),	0},
	{6,	"MODEM1",	"PERI",		GENMASK(4, 0),	0},
	{7,	"VTS",		"PERI",		GENMASK(4, 0),	0},
	{8,	"WLBT",		"PERI",		GENMASK(4, 0),	0},
	{9,	"APM",		"PERI",		GENMASK(4, 0),	0},
	{10,	"CHUB",		"PERI",		GENMASK(4, 0),	0},
	{11,	"COREX",	"PERI",		GENMASK(4, 0),	0},
	{12,	"CSSYS",	"PERI",		GENMASK(4, 0),	0},
	{13,	"DIT",		"PERI",		GENMASK(4, 0),	0},
	{14,	"DPU",		"PERI",		GENMASK(4, 0),	0},
	{15,	"FSYS",		"PERI",		GENMASK(4, 0),	0},
	{16,	"G3D",		"PERI",		GENMASK(4, 0),	0},
};
...
static struct itmon_masterinfo masterinfo[] = {
	...
	/* BLK_ISP */
	{"ISP",		0,				"CSIS",			GENMASK(2, 1)},
	{"ISP",		BIT(1),				"ISP",			GENMASK(2, 1)},
	{"ISP",		BIT(2),				"MCSC",			GENMASK(2, 1)},
	{"ISP",		BIT(1) | BIT(2),		"VRA",			GENMASK(2, 1)},

	/* BLK_FSYS */
	{"FSYS",	0,				"MMC_EMBD",		GENMASK(1, 0)},
	{"FSYS",	BIT(0),				"MMC_CARD",		GENMASK(1, 0)},
	{"FSYS",	BIT(1),				"USB20DRD",		GENMASK(1, 0)},

	/* BLK_APM */
	{"APM",		0,				"GREBE",		GENMASK(0, 0)},
	{"APM",		BIT(0),				"PEM",			GENMASK(0, 0)},

	/* BLK_CHUB */
	{"CHUB",	BIT(0),				"CM4_SHUB",		GENMASK(2, 0)},
	{"CHUB",	BIT(1),				"PDMA_SHUB",		GENMASK(2, 0)},

	/* BLK_MIF */
	{"COREX",	0,				"RTIC",			GENMASK(1, 0)},
	{"COREX",	BIT(0),				"SSS",			GENMASK(1, 0)},
	...
	/* BLK_MFCMSCL */
	{"MFCMSCL",	0,				"MFC",			GENMASK(2, 1)},
	{"MFCMSCL",	BIT(0),				"JPEG",			GENMASK(2, 1)},
	{"MFCMSCL",	BIT(1),				"MCSC(POLY)",		GENMASK(2, 1)},
	{"MFCMSCL",	BIT(0) | BIT(1),		"M2M(BI)",		GENMASK(2, 1)},

	/* BLK_MODEM0 */
	{"MODEM0",	0,				"UCPUM",		GENMASK(6, 0)},
	{"MODEM0",	BIT(0),				"DMA0",			GENMASK(6, 0)},
	{"MODEM0",	BIT(0) | BIT(3),		"DMA1",			GENMASK(6, 0)},
	{"MODEM0",	BIT(0) | BIT(4),		"DMA2",			GENMASK(6, 0)},
	{"MODEM0",	BIT(0) | BIT(2),		"LCPUM",		GENMASK(6, 0)},
	{"MODEM0",	BIT(2),				"LMAC",			GENMASK(6, 0)},
	{"MODEM0",	BIT(1),				"CSXAP",		GENMASK(6, 0)},
	{"MODEM0",	BIT(0) | BIT(1),		"DATAMOVER",		GENMASK(6, 0)},
	{"MODEM0",	BIT(0) | BIT(1) | BIT(4),	"BAYES",		GENMASK(6, 0)},
	{"MODEM0",	BIT(0) | BIT(1) | BIT(3),	"LOGGER",		GENMASK(6, 0)},
	{"MODEM0",	BIT(1) | BIT(2),		"HARQMOVERtoL2",	GENMASK(6, 0)},

	/* BLK_MODEM1 */
	{"MODEM1",	0,				"UCPUM",		GENMASK(6, 0)},
	{"MODEM1",	BIT(0),				"DMA0",			GENMASK(6, 0)},
	{"MODEM1",	BIT(0) | BIT(3),		"DMA1",			GENMASK(6, 0)},
	{"MODEM1",	BIT(0) | BIT(4),		"DMA2",			GENMASK(6, 0)},
	{"MODEM1",	BIT(0) | BIT(2),		"LCPUM",		GENMASK(6, 0)},
	{"MODEM1",	BIT(2),				"LMAC",			GENMASK(6, 0)},
	{"MODEM1",	BIT(1),				"CSXAP",		GENMASK(6, 0)},
	{"MODEM1",	BIT(0) | BIT(1),		"DATAMOVER",		GENMASK(6, 0)},
	{"MODEM1",	BIT(0) | BIT(1) | BIT(4),	"BAYES",		GENMASK(6, 0)},
	{"MODEM1",	BIT(0) | BIT(1) | BIT(3),	"LOGGER",		GENMASK(6, 0)},
	{"MODEM1",	BIT(1) | BIT(2),		"HARQMOVERtoL2",	GENMASK(6, 0)},

	/* BLK_WLBT */
	{"WLBT",	0,				"CR4",			GENMASK(5, 0)},
	{"WLBT",	BIT(0),				"XDMA",			GENMASK(5, 0)},
	{"WLBT",	BIT(1),				"ENC_DMA",		GENMASK(5, 0)},
...
static struct itmon_nodeinfo data_path[] = {
	{M_NODE, "ABOX",		0x12013000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "APM",			0x12023000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "CHUB",		0x12033000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "COREX",		0x12043000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "CPU",			0x12003000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "CSSYS",		0x12053000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "DIT",			0x12063000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "DPU",			0x12073000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "FSYS",		0x12083000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "G3D",			0x12093000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "GNSS",		0x120A3000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "ISP",			0x120B3000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "MFCMSCL",		0x120C3000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "MODEM0",		0x120D3000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "MODEM1",		0x120E3000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "VTS",			0x120F3000, NULL, 0,	   false, false,  true, true, false},
	{M_NODE, "WLBT",		0x12103000, NULL, 0,	   false, false,  true, true, false},
	{S_NODE, "CPU_MEM0",		0x12113000, NULL, TMOUT,   true, false,  true, true, false},
	{S_NODE, "CP_MEM0",		0x12133000, NULL, TMOUT,   true, false,  true, true, false},
	{S_NODE, "PERI",		0x12153000, NULL, TMOUT,   true, false,  true, true, false},
	{S_NODE, "RT_MEM0",		0x12123000, NULL, TMOUT,   true, false,  true, true, false},
	{S_NODE, "S_NRT0",		0x12143000, NULL, TMOUT,   true, false,  true, true, false},
};

/* trex_d_nrt is sorted by INT_VEC_DEBUG_INTERRUPT_VECTOR_TABLE bits */
static struct itmon_nodeinfo trex_d_nrt[] = {
	{M_NODE, "M_NRT0",		0x12613000, NULL, 0,	   false, false, true, true, false},
	{M_NODE, "SIREX",		0x12603000, NULL, 0,	   false, false, true, true, false},
	{S_NODE, "NRT_MEM0",		0x12623000, NULL, TMOUT,   true, false, true, true, false},
};

/* peri_path is sorted by INT_VEC_DEBUG_INTERRUPT_VECTOR_TABLE bits */
static struct itmon_nodeinfo peri_path[] = {
	{M_NODE, "M_PERI",		0x12403000, NULL, 0,	   false, false, true, true, false},
	{S_NODE, "APMP",		0x12413000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "CHUBP",		0x12523000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "COREP_SFR",		0x12453000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "COREP_TREX",		0x12443000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "CPU_CL0P",		0x12423000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "CSSYS",		0x12433000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "DISPAUDP",		0x12463000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "FSYSP",		0x12473000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "G3DP",		0x12483000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "GICP",		0x12493000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "GNSSP",		0x124A3000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "ISPP",		0x124B3000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "MFCMSCLP",		0x124C3000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "MIF0P",		0x124D3000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "MODEMP",		0x124E3000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "PERIP",		0x124F3000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "SIREXP_MEM0",		0x12503000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "VTSP",		0x12533000, NULL, TMOUT,   true,  false, true, true, false},
	{S_NODE, "WLBTP",		0x12513000, NULL, TMOUT,   true,  false, true, true, false},
};

static struct itmon_nodegroup nodegroup[] = {
	{40, "TREX_D_CORE",	0x123F3000, NULL, data_path, ARRAY_SIZE(data_path), BUS_DATA},
	{42, "TREX_D_NRT",	0x127F3000, NULL, trex_d_nrt, ARRAY_SIZE(trex_d_nrt), BUS_DATA},
	{41, "TREX_P_CORE",	0x125F3000, NULL, peri_path, ARRAY_SIZE(peri_path), BUS_PERI},
};
					...
					unsigned int trans_type)
{
	...
	if (!strncmp(traceinfo->port, "CPU", strlen("CPU"))) {
		...
		if (traceinfo->master && traceinfo->target_addr == INVALID_REMAPPING &&
			...
			pr_info("ITMON Master is CP, Crash occur\n");
	...
	} else if (!strncmp(traceinfo->port, "CHUB", strlen("CHUB"))) {
		pr_info("[SSP] Debug CHUB ITMON skips: %s detected\n", traceinfo->port);
		pdata->err_cnt = 0;
		//panic("[SSP] Debug Kernel Panic: CHUB ITMON \n");
	} else if (!strncmp(traceinfo->port, "GNSS", strlen("GNSS"))) {
		pdata->err_cnt = 0;
		pr_info("ITMON detects GNSS tries to access 0x%lX, skips panic\n",
				traceinfo->target_addr);
	} else {
		pr_info("ITMON detects port:%s, target_addr:0x%lX\n",
				traceinfo->port, traceinfo->target_addr);
	}
}
