...
int get_name_from_pid(char *task_name, int pid)
{
	...
	if (p_task) {
			...
			strncpy(last_task_info.comm, p_task->comm,
				strlen(p_task->comm) > sizeof(last_task_info.comm)?
				sizeof(last_task_info.comm) : strlen(p_task->comm));
	}
	rcu_read_unlock();
	...
	if (found == 0) {
		if(search_killed_task(pid, task_name) >= 0) {
			...
			strncpy(last_task_info.comm, task_name, strlen(task_name) > sizeof(last_task_info.comm)?
				sizeof(last_task_info.comm) : strlen(task_name));
		} else {
