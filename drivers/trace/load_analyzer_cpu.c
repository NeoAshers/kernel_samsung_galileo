...
unsigned int period_check_idle = 3000;
unsigned int framedrop_threshold = 50;
unsigned int iowait_threshold = 10;
unsigned int allocstall_threshold = 20;
unsigned int cpu_busy_level_threshold = 90;
...
static u64 user_old[CPU_NUM], nice_old[CPU_NUM], system_old[CPU_NUM];
static u64 idle_old[CPU_NUM], iowait_old[CPU_NUM], irq_old[CPU_NUM], softirq_old[CPU_NUM];
unsigned long busy_level_time_trigger[BUSY_LEVEL_NUMBER_TRIGGER];
struct time_in_state_busy_level_diff time_in_state_diff[MAX_CPU_STATE];
#endif

int update_time_in_state_diff(void)
{
	int i = 0;
	...
	for (i = 0; i < stats->state_num; i++) {
		time_in_state_diff[i].cpu_freq = stats->freq_table[i];
		time_in_state_diff[i].diff = jiffies_to_msecs(stats->time_in_state[i]) - time_in_state_diff[i].last_time_in_state;
		time_in_state_diff[i].last_time_in_state = jiffies_to_msecs(stats->time_in_state[i]);
	}
...
int check_valid_range(unsigned int cpu, unsigned int start_cnt,
		unsigned int end_cnt, unsigned int *start_cnt_cpu_busy)
{
	int ret = 0;
	unsigned long long t1, t2;
...
#if defined(CONFIG_SLP_BUSY_LEVEL)
			if(cpu_busy_level) {
			/* when cpu busy, many task stored to cpu_task_history,
			  * it make buffer overflow. so we break when buffer full, do not check anymore */
				load_cnt = get_index(load_cnt, cpu_load_history_num, 1);
				*start_cnt_cpu_busy = load_cnt;
				pr_info("[LA] CPU[%d] in busy mode, check_running in range [%d~%d] \n",cpu,load_cnt,last_load_cnt);
			} else
#endif
			{
				overflow = 1;
		...
		char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (*ppos == 0) {
		...
		ret +=  scnprintf(buf + ret, count - ret, cpu_load_freq_menu);
	}
	...
	char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (*ppos == 0) {
		...
		ret +=  scnprintf(buf + ret, count - ret, cpu_bus_load_freq_menu);
	}
		...
		int ret, int *ret_check_valid)
{
	...
	unsigned int start_cnt_cpu_busy = 0;
	...
	*ret_check_valid = 0;
	...
	for (i = 0; i < CPU_NUM ; i++) {
		...
		cpu = i;
		check_valid_data = check_valid_range(cpu, start_cnt, end_cnt,&start_cnt_cpu_busy);
		...
		if(start_cnt_cpu_busy!= 0)
			start_cnt = start_cnt_cpu_busy;
		...
		if (end_cnt == start_cnt+1) {
			ret += snprintf(buf + ret, buf_size - ret,
					"[%d] TOTAL SECTION TIME = %ld[ms]\n\n", end_cnt, (unsigned long)t);
		} else {
			ret += snprintf(buf + ret, buf_size - ret,
					"[%d~%d] TOTAL SECTION TIME = %ld[ms]\n\n",
		...
		char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (*ppos == 0) {
		...
		for (i = 0; i < CPU_NUM ; i++) {
			...
			cpu = i;
			ret_check_valid = check_valid_range(cpu, start_cnt, end_cnt,&start_cnt_cpu_busy);
...
#if defined(CONFIG_FRAME_DROP_DETECTOR)
		if (check_valid_range(0, start_cnt, end_cnt, &start_cnt_cpu_busy) == 0) {
			ret += snprintf(buf + ret, buf_size - ret,
					"##################################"
				" FRAMERATE ############################\n");

			if (start_cnt + 1 <= end_cnt) {
				for (i = start_cnt + 1; i <= end_cnt; i++){
					total_frames += cpu_load_freq_history_view[i].frames;
					total_vsync += cpu_load_freq_history_view[i].vsync;
				}
			} else {
				for (i = start_cnt + 1; i < cpu_load_history_num; i++){
					total_frames += cpu_load_freq_history_view[i].frames;
					total_vsync += cpu_load_freq_history_view[i].vsync;
				}
				for (i = 0; i <= end_cnt; i++){
			...
			frames_per_sec = total_frames * 1000 / (unsigned long) t;
			ret +=  snprintf(buf + ret, buf_size - ret, "%d fps (%d/%2d) \n\n", frames_per_sec, total_frames,total_vsync);
		}
#endif
...
#if defined(CONFIG_FRAME_DROP_DETECTOR)
static struct framedrop_info last_framedrop_info;

static int calculate_framerate(int *frames, int *vsync)
{
	int start_cnt = 0, end_cnt = 0, cnt = 0, i = 0;
	int range_num = 20;
	unsigned int total_frames = 0;
	unsigned int total_vsync = 0;
	...
	*vsync = total_vsync;
	...
	return (total_vsync > 0)? (total_frames * 100 / total_vsync) : -1;
}
#endif

#if defined(CONFIG_SLP_BUSY_LEVEL)
unsigned long busy_state_get_time_occur(void)
{
	unsigned long long t;
...
static void work_detect_busy_state_function(struct work_struct *work)
{
#if defined(CONFIG_FRAME_DROP_DETECTOR)
	int framerate = 0;
	int frame, vsync;

	if ((framerate = calculate_framerate(&frame, &vsync)) == -1)
		return;

	if ((framerate < framedrop_threshold) && (vsync > 30)) {
		pr_info("[B_LEVEL] framedrop - %d %d %d (rate/frame/vsync)\n",
				framerate, frame, vsync);

		last_framedrop_info.frame = frame;
		last_framedrop_info.vsync = vsync;
		last_framedrop_info.framerate = framerate;

		cpu_busy_level = TARGET_BUSY_STATE;
		busy_level_time_trigger[BUSY_LEVEL_TRIGGER_FRAME_DROP] = busy_state_get_time_occur();

		update_cpu_busy_level(BUSY_LEVEL_TRIGGER_FRAME_DROP);
	}
#endif
}

static DECLARE_DELAYED_WORK(work_detect_busy_state, work_detect_busy_state_function);

static int touch_event_callback(struct notifier_block *nb,
         unsigned long event, void *data)
{
	...
	if (event == TOUCH_EVENT_RELEASE) {
		time_end = data;
		if ((time_end != NULL) && (time_begin != NULL) && (is_press == 1)) {
			if (jiffies_to_msecs(*time_end - *time_begin) >= PRESS_RELEASE_THRESHOLD)
				schedule_delayed_work(&work_detect_busy_state, msecs_to_jiffies(TIME_CHECK_DELAY));
		}
		is_press = 0;
	}
...
int get_meminfo_proc(char *buf, int buf_size, int ret)
{
	struct sysinfo i;
	long cached;
	long available;
	...
	ret +=  snprintf(buf + ret, buf_size - ret, "MemTotal:\t%lu\n", i.totalram << (PAGE_SHIFT - 10));
	ret +=  snprintf(buf + ret, buf_size - ret, "MemFree:\t%lu\n", i.freeram << (PAGE_SHIFT - 10));
	ret +=  snprintf(buf + ret, buf_size - ret, "MemAvailable:\t%lu\n", available << (PAGE_SHIFT - 10));
	ret +=  snprintf(buf + ret, buf_size - ret, "Buffers:\t%lu\n", i.bufferram << (PAGE_SHIFT - 10));
	ret +=  snprintf(buf + ret, buf_size - ret, "Cached:\t\t%lu\n", cached << (PAGE_SHIFT - 10));
	ret +=  snprintf(buf + ret, buf_size - ret, "SwapCached:\t%lu\n",
						total_swapcache_pages() << (PAGE_SHIFT - 10));
	ret +=  snprintf(buf + ret, buf_size - ret, "Slab:\t\t%lu\n", (global_page_state(NR_SLAB_RECLAIMABLE) +
						global_page_state(NR_SLAB_UNRECLAIMABLE)) << (PAGE_SHIFT - 10));
	ret +=  snprintf(buf + ret, buf_size - ret, "SReclaimable:\t%lu\n",
						global_page_state(NR_SLAB_RECLAIMABLE) << (PAGE_SHIFT - 10));
	ret +=  snprintf(buf + ret, buf_size - ret, "SUnreclaim:\t%lu\n\n",
						global_page_state(NR_SLAB_UNRECLAIMABLE) << (PAGE_SHIFT - 10));
...
static u32 period_in_msec = 5000;
#define DIFF_VALUE_MSEC(val)	((val[i]) && ((val[i]) > (val##_old[i]))) ? \
		(cputime_to_usecs(val[i] - val##_old[i]) / 1000) : 0
int last_cpu_load_get_cpu_stat(char *buf, int buf_size, int ret)
{
	int i;

	ret += snprintf(buf + ret, buf_size - ret, "6. [CPU STATUS]\n");
	ret += snprintf(buf + ret, buf_size - ret,
				"[SLUG]        user      nice    system    idle    iowait    irq    softirq\n");
	for_each_possible_cpu(i) {

		ret += snprintf(buf + ret, buf_size - ret,
				"[SLUG] cpu%d: %5u     %5u     %5u   %5u     %5u  %5u      %5u\n",
				i, DIFF_VALUE_MSEC(user),
				DIFF_VALUE_MSEC(nice),
				DIFF_VALUE_MSEC(system),
				DIFF_VALUE_MSEC(idle),
				DIFF_VALUE_MSEC(iowait),
				DIFF_VALUE_MSEC(irq),
				DIFF_VALUE_MSEC(softirq));
	}
...
void get_cpu_stat(void)
{
	...
	if (jiffies_to_msecs(jiffies - time_old) >= period_in_msec) {
		for_each_possible_cpu(i) {
			user_old[i] = user[i];
			nice_old[i] = nice[i];
			system_old[i] = system[i];
			idle_old[i] = idle[i];
			iowait_old[i] = iowait[i];
			irq_old[i] = irq[i];
			softirq_old[i] = softirq[i];
...
#if defined(CONFIG_SLP_CHECK_BUS_LOAD)
int saved_range_num = 10;
#define LAST_CPU_LOAD_PRINT_RANGE	(10)
int saved_max_list_num = 10;
static int last_cpu_load_read_sub(char *buf, int buf_size)
{
...
#if defined(CONFIG_SLP_BUSY_LEVEL)
	ret +=  snprintf(buf + ret, buf_size - ret, "Trigger reason: ");
	if (busy_level_time_trigger[BUSY_LEVEL_TRIGGER_CPU_LOAD]) {
		ret +=  snprintf(buf + ret, buf_size - ret, "High cpu load - Timestamp (s): %lu.%lu, ",
			busy_level_time_trigger[BUSY_LEVEL_TRIGGER_CPU_LOAD] / 1000,
			busy_level_time_trigger[BUSY_LEVEL_TRIGGER_CPU_LOAD] % 1000);
		busy_level_time_trigger[BUSY_LEVEL_TRIGGER_CPU_LOAD] = 0;
	}
	if (busy_level_time_trigger[BUSY_LEVEL_TRIGGER_IOWAIT]) {
		ret +=  snprintf(buf + ret, buf_size - ret, "Iowait - Timestamp (s): %lu.%lu, ",
			busy_level_time_trigger[BUSY_LEVEL_TRIGGER_IOWAIT] / 1000,
			busy_level_time_trigger[BUSY_LEVEL_TRIGGER_IOWAIT] % 1000);
		busy_level_time_trigger[BUSY_LEVEL_TRIGGER_IOWAIT] = 0;
	}
	if (busy_level_time_trigger[BUSY_LEVEL_TRIGGER_ALLOCSTALL]) {
		ret +=  snprintf(buf + ret, buf_size - ret, "MemAllocstall - Timestamp (s): %lu.%lu, ",
			busy_level_time_trigger[BUSY_LEVEL_TRIGGER_ALLOCSTALL] / 1000,
			busy_level_time_trigger[BUSY_LEVEL_TRIGGER_ALLOCSTALL] % 1000);
		busy_level_time_trigger[BUSY_LEVEL_TRIGGER_ALLOCSTALL] = 0;
	}
#if defined(CONFIG_FRAME_DROP_DETECTOR)
	if (busy_level_time_trigger[BUSY_LEVEL_TRIGGER_FRAME_DROP]) {
		ret +=  snprintf(buf + ret, buf_size - ret, "Framedrop - Timestamp (s): %lu.%lu\n",
			busy_level_time_trigger[BUSY_LEVEL_TRIGGER_FRAME_DROP] / 1000,
			busy_level_time_trigger[BUSY_LEVEL_TRIGGER_FRAME_DROP] % 1000);
		busy_level_time_trigger[BUSY_LEVEL_TRIGGER_FRAME_DROP] = 0;
	}
#endif
#endif
	...
	do {
		...
		ret = show_cpu_bus_load_freq_sub(cnt, LAST_CPU_LOAD_PRINT_RANGE, buf, (buf_size - ret) ,ret);
		ret +=  snprintf(buf + ret, buf_size - ret, "\n");

		ret +=  snprintf(buf + ret, buf_size - ret, "4. [CPU TASK INFO]\n");
		ret = check_running_buf_print(saved_max_list_num, buf, (buf_size - ret), ret, &ret_check_valid);
#if defined(CONFIG_FRAME_DROP_DETECTOR)
		ret += snprintf(buf + ret, buf_size - ret,"5. [FRAMERATE]\n");
		...
		if (start_cnt <= end_cnt) {
			...
			for (i = start_cnt; i < cpu_load_history_num; i++) {
				total_frames += cpu_load_freq_history_view[i].frames;
				total_vsync += cpu_load_freq_history_view[i].vsync;
			}
		...
		frames_per_sec = total_frames * 100 / total_vsync;
		ret +=  snprintf(buf + ret, buf_size - ret, "%d %% (%d/%2d)\n\n", frames_per_sec, total_frames,total_vsync);

#endif

#if defined(CONFIG_SLP_BUSY_LEVEL)
		ret = last_cpu_load_get_cpu_stat(buf, (buf_size-ret) , ret);
		ret = get_meminfo_proc(buf, (buf_size - ret), ret);
#endif
		ret += snprintf(buf + ret, buf_size - ret,"8. [CPU DIFF]\n");
		ret += snprintf(buf + ret, buf_size - ret, "%d [", time_in_state_diff[0].time_diff_busy_occur);
		for (i = 0; time_in_state_diff[i].cpu_freq; i++) {
			ret += snprintf(buf + ret, buf_size - ret, "%d ", time_in_state_diff[i].diff);
		}
		ret += snprintf(buf + ret, buf_size - ret, "]\n");
...
static DEFINE_MUTEX(load_read_mutex);
...
#if defined(CONFIG_KSWAPD_WAKEUP_DEBUGS)
static ssize_t kswapd_wakeup_debugs_write(struct file *file,
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	int mode = 0;
	char buf[32];
...
static DEFINE_MUTEX(kswapd_read_mutex);
static ssize_t kswapd_wakeup_debugs_read(struct file *file,
		char __user *buffer, size_t count, loff_t *ppos)
{
	unsigned int size_for_copy = 0;
	char *buf = NULL;
	int buf_size = 40;
	buf = vmalloc(buf_size);
	mutex_lock(&kswapd_read_mutex);

	if(*ppos != 0) {
		vfree(buf);
		mutex_unlock(&kswapd_read_mutex);
		return 0;
	}

	size_for_copy += sprintf(buf, "Count: %lu - Time<ms>: %d\n", kswapd_count, jiffies_to_msecs(run_time));
...
static DEFINE_MUTEX(idle_time_check);
static ssize_t check_idle_time_write(struct file *file,
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (value_read > 0) {
		period_check_idle = value_read;
		pr_info("%s: [BUSY_LEVEL] read idle check period: %u\n", __FUNCTION__, value_read);
	} else
		return -EINVAL;
...
static ssize_t check_idle_time_read(struct file *file,
		char __user *buffer, size_t count, loff_t *ppos)
{
	unsigned int size_for_copy = 0;
	char *buf = NULL;
	int buf_size = 40;
	buf = vmalloc(buf_size);
	mutex_lock(&idle_time_check);

	if(*ppos != 0) {
		vfree(buf);
		mutex_unlock(&idle_time_check);
		return 0;
	}

	size_for_copy += sprintf(buf, "Current period: %u\n", period_check_idle);
...
static DEFINE_MUTEX(busy_level_init);
static ssize_t threshold_cpubusy_write(struct file *file,
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	unsigned int value_read = 0;
	char buf[32];
	...
	if (value_read > 0) {
		cpu_busy_level_threshold = value_read;
		pr_info("%s: [BUSY_LEVEL] read cpu busy threshold: %u\n", __FUNCTION__, value_read);
	} else
		return -EINVAL;
...
static ssize_t threshold_cpubusy_read(struct file *file,
		char __user *buffer, size_t count, loff_t *ppos)
{
	unsigned int size_for_copy = 0;
	char *buf = NULL;
	int buf_size = 64;
	buf = vmalloc(buf_size);
	mutex_lock(&busy_level_init);

	if(*ppos != 0) {
		vfree(buf);
		mutex_unlock(&busy_level_init);
		return 0;
	}

	size_for_copy += sprintf(buf, "cpu busy threshold: %u\n",
		cpu_busy_level_threshold);
...
static ssize_t threshold_framedrop_write(struct file *file,
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	unsigned int value_read = 0;
	char buf[32];
	...
	if (value_read > 0) {
		framedrop_threshold = value_read;
		pr_info("%s: [BUSY_LEVEL] read framedop threshold: %u\n", __FUNCTION__, value_read);
	} else
		return -EINVAL;
...
static ssize_t threshold_framedrop_read(struct file *file,
		char __user *buffer, size_t count, loff_t *ppos)
{
	unsigned int size_for_copy = 0;
	char *buf = NULL;
	...
	size_for_copy += sprintf(buf, "framedrop: %u\n",
		framedrop_threshold);
...
static ssize_t threshold_iowait_write(struct file *file,
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	unsigned int value_read = 0;
	char buf[32];
	...
	if (value_read > 0) {
		iowait_threshold = value_read;
		pr_info("%s: [BUSY_LEVEL] read iowait threshold: %u\n", __FUNCTION__, value_read);
	} else
		return -EINVAL;
...
static ssize_t threshold_iowait_read(struct file *file,
		char __user *buffer, size_t count, loff_t *ppos)
{
	unsigned int size_for_copy = 0;
	char *buf = NULL;
	...
	size_for_copy += sprintf(buf, "iowait: %u\n",
		 iowait_threshold);
...
static ssize_t threshold_memstall_write(struct file *file,
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	unsigned int value_read = 0;
	char buf[32];
	...
	if (value_read > 0) {
		allocstall_threshold= value_read;
		pr_info("%s: [BUSY_LEVEL] read allocstall threshold: %u\n", __FUNCTION__, value_read);
	} else
		return -EINVAL;
...
static ssize_t threshold_memstall_read(struct file *file,
		char __user *buffer, size_t count, loff_t *ppos)
{
	unsigned int size_for_copy = 0;
	char *buf = NULL;
	...
	size_for_copy += sprintf(buf, "allocstall: %u\n",
		 allocstall_threshold);
...
#if defined(CONFIG_KSWAPD_WAKEUP_DEBUGS)
static const struct file_operations kswapd_wakeup_debugs_fops = {
	.owner = THIS_MODULE,
	.read  = kswapd_wakeup_debugs_read,
	.write = kswapd_wakeup_debugs_write
};
#endif

#if defined(CONFIG_SLP_BUSY_LEVEL)
static struct notifier_block touch_event_notifier = {
    .notifier_call = touch_event_callback,
};
#endif

static const struct file_operations threshold_cpubusy_fops = {
	.owner = THIS_MODULE,
	.read  = threshold_cpubusy_read,
	.write = threshold_cpubusy_write,
};

static const struct file_operations threshold_framedrop_fops = {
	.owner = THIS_MODULE,
	.read  = threshold_framedrop_read,
	.write = threshold_framedrop_write,
};

static const struct file_operations threshold_iowait_fops = {
	.owner = THIS_MODULE,
	.read  = threshold_iowait_read,
	.write = threshold_iowait_write,
};

static const struct file_operations threshold_memstall_fops = {
	.owner = THIS_MODULE,
	.read  = threshold_memstall_read,
	.write = threshold_memstall_write,
};

static const struct file_operations check_idle_time_fops = {
	.owner = THIS_MODULE,
	.read  = check_idle_time_read,
	.write = check_idle_time_write,
};
...
void debugfs_cpu_bus(struct dentry *d)
{
...
#if defined(CONFIG_KSWAPD_WAKEUP_DEBUGS)
	if (!debugfs_create_file("kswapd_wakeup_debugs", 0666, d, NULL, &kswapd_wakeup_debugs_fops))
		pr_err("%s : debugfs_create_file, error\n", "kswapd_wakeup_debugs");
#endif
	...
	if (!debugfs_create_file("threshold_cpubusy", 0606, d, NULL, &threshold_cpubusy_fops))
		pr_err("%s : debugfs_create_file, error\n", "threshold_cpubusy");

	if (!debugfs_create_file("threshold_framedrop", 0606, d, NULL, &threshold_framedrop_fops))
		pr_err("%s : debugfs_create_file, error\n", "threshold_framedrop");

	if (!debugfs_create_file("threshold_iowait", 0606, d, NULL, &threshold_iowait_fops))
		pr_err("%s : debugfs_create_file, error\n", "threshold_iowait");

	if (!debugfs_create_file("threshold_memstall", 0606, d, NULL, &threshold_memstall_fops))
		pr_err("%s : debugfs_create_file, error\n", "threshold_memstall");

	if (!debugfs_create_file("check_idle_time", 0606, d, NULL, &check_idle_time_fops))
		pr_err("%s : debugfs_create_file, error\n", "check_idle_time");
