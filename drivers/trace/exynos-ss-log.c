...
static void exynos_ss_print_last_irq(int cpu)
{
	...
	if (atomic_read(&ess_idx.irq_log_idx[cpu]) < 0) {
		pr_info("%-16s: No information.\n", ">>> last irq");
		return;
	}
...
static void exynos_ss_print_last_task(int cpu)
{
	...
	if (atomic_read(&ess_idx.task_log_idx[cpu]) < 0) {
		pr_info("%-16s: No information.\n", ">>> last task");
		return;
	}
...
static void exynos_ss_print_last_work(int cpu)
{
	...
	if (atomic_read(&ess_idx.work_log_idx[cpu]) < 0) {
		pr_info("%-16s: No information.\n", ">>> last work");
		return;
	}
...
static void exynos_ss_print_last_cpuidle(int cpu)
{
	...
	if (atomic_read(&ess_idx.cpuidle_log_idx[cpu]) < 0) {
		pr_info("%-16s: No information.\n", ">>> last cpuidle");
		return;
	}
...
static void exynos_ss_print_freqinfo(void)
{
	...
	pr_info("\n<freq info>\n");
	if (atomic_read(&ess_idx.freq_log_idx) < 0) {
	...
	for (i = 0; i < ESS_FLAG_END; i++) {
		...
		if (freq_name && ess_freq_name[i] && strncmp(freq_name, ess_freq_name[i], strlen(ess_freq_name[i]))) {
