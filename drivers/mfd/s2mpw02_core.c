...
static char pmic_block[10];
static struct dentry *s2mpw02_reg_dump;
...
static ssize_t s2mpw02_reg_dump_read(struct file *file, char __user *user_buf,
					size_t count, loff_t *ppos)
{
	...
	if (!strncmp(pmic_block, "common", sizeof("common")))
		i2c = dbg_s2mpw02->i2c;
	else if (!strncmp(pmic_block, "pm", sizeof("pm")))
		i2c = dbg_s2mpw02->pmic;
	else if (!strncmp(pmic_block, "rtc", sizeof("rtc")))
		i2c = dbg_s2mpw02->rtc;
	else if (!strncmp(pmic_block, "codec", sizeof("codec")))
		return -EINVAL;
	else if (!strncmp(pmic_block, "charger", sizeof("charger")))
		i2c = dbg_s2mpw02->charger;
	else if (!strncmp(pmic_block, "fg", sizeof("fg")))
		i2c = dbg_s2mpw02->fuelgauge;
	else
		return -EINVAL;

	for (i = 0; i < 255; i++) {
		ret = s2mpw02_read_reg(i2c, i, val + i);
		if (ret)
			return ret;
	}

	ret = snprintf(buf, sizeof(buf), "[%s block]\n", pmic_block);
	...
	for (i = 0; i < 256; i += j) {
		...
		for (j = 0; j < 0x10; j++) {
			if (i + j == 256)
				break;
			ret += snprintf(buf + ret, sizeof(buf) - ret, "%02x ", *(val + i + j));
		}
	}
	ret += snprintf(buf + ret, sizeof(buf) - ret, "\n");
...
static ssize_t s2mpw02_reg_dump_write(struct file *file, const char __user *user_buf,
					size_t count, loff_t *ppos)
{
	char buf[20];
	ssize_t len, argc;
...
static const struct file_operations s2mpw02_reg_dump_fops = {
	.open = simple_open,
	.read = s2mpw02_reg_dump_read,
	.write = s2mpw02_reg_dump_write,
	.llseek = default_llseek,
};
#endif
				...
				const struct i2c_device_id *dev_id)
{
	...
	s2mpw02_reg_dump = debugfs_create_file("reg_dump", 0644, s2mpw02_root, NULL, &s2mpw02_reg_dump_fops);
#endif
...
#ifdef CONFIG_DEBUG_FS
	debugfs_remove_recursive(s2mpw02_reg_dump);
