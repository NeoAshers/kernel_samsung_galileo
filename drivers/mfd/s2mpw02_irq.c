...
//	u8 param1, param2, param3, param4, param5;
	...
	/* [DEBUG] irq pin is low without any irq source */
	if (irq_src == 0x00) {
		gpa0_con = readl(s2mpw02->mem_base);
		pr_info("%s: gpa0_con 0x%x\n", __func__, gpa0_con);
	}
...
int s2mpw02_irq_init(struct s2mpw02_dev *s2mpw02)
{
	...
	s2mpw02->mem_base = ioremap(GPIO_ALIVE_BASE + GPA0_CON_OFFSET, SZ_32);
	if (s2mpw02->mem_base == NULL)
