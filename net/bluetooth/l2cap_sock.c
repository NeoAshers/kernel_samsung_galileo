...
extern const char* l2cap_get_psm_string(__le16 psm);
extern const char* l2cap_get_cid_string(__le16 cid);
#endif
...
#ifdef CONFIG_TIZEN_WIP
	BT_INFO("[%s(%d)] sk: 0x%p, PSM: 0x%04x(%s), CID: 0x%04x(%s), bdaddr_type: 0x%02x(%s)", __FUNCTION__, __LINE__,
			sk,
			__le16_to_cpu(la.l2_psm), l2cap_get_psm_string(__le16_to_cpu(la.l2_psm)),
			__le16_to_cpu(la.l2_cid), l2cap_get_cid_string(__le16_to_cpu(la.l2_cid)),
			la.l2_bdaddr_type,
			(la.l2_bdaddr_type == BDADDR_BREDR)?"BREDR":
			(la.l2_bdaddr_type == BDADDR_LE_PUBLIC)?"LE_PUBLIC":
			(la.l2_bdaddr_type == BDADDR_LE_RANDOM)?"LE_RANDOM":"Not Defined"
	);
#endif
...
#ifdef CONFIG_TIZEN_WIP
	BT_INFO("[%s(%d)] sk: 0x%p, PSM: 0x%04x(%s), CID: 0x%04x(%s), bdaddr_type: 0x%02x(%s), backlog: %d", __FUNCTION__, __LINE__,
		sk,
		__le16_to_cpu(chan->psm), l2cap_get_psm_string(__le16_to_cpu(chan->psm)),
		chan->scid, l2cap_get_cid_string(chan->scid),
		chan->src_type,
		(chan->src_type == BDADDR_BREDR)?"BREDR":
		(chan->src_type == BDADDR_LE_PUBLIC)?"LE_PUBLIC":
		(chan->src_type == BDADDR_LE_RANDOM)?"LE_RANDOM":"Not Defined",
		backlog
	);
#else
...
#ifdef CONFIG_TIZEN_WIP
	if (nsk) {
		chan = l2cap_pi(nsk)->chan;
		if(chan)
		{
			BT_INFO("[%s(%d)] sk: 0x%p(timeo: %ld), new sk: 0x%p, PSM: 0x%04x(%s), CID: 0x%04x(%s), bdaddr_type: 0x%02x(%s)", __FUNCTION__, __LINE__,
					sk, timeo, nsk,
					__le16_to_cpu(chan->psm), l2cap_get_psm_string(__le16_to_cpu(chan->psm)),
					chan->scid, l2cap_get_cid_string(chan->scid),
					chan->src_type,
				 ...
				 char __user *optval, unsigned int optlen)
{
	...
	case BT_LE_CONN_PARAM: {
		...
		if (conn->hcon->le_conn_interval >= param.min &&
			conn->hcon->le_conn_interval <= param.max &&
			conn->hcon->le_conn_latency == param.latency) {
			BT_ERR("no need to update");
			err = -EBADRQC;
			break;
		}
