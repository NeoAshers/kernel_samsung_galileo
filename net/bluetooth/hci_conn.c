...
static void hci_conn_cleanup(struct hci_conn *conn)
{
	...
	if (test_bit(HCI_CONN_PARAM_REMOVAL_PEND, &conn->flags)) {
		hci_conn_params_del(conn->hdev, &conn->dst, conn->dst_type);
		/* Check MGMT_OP_UNPAIR_DEVICE event pending */
		mgmt_unpair_complete(hdev, 0);
	}
#endif
...
#ifdef CONFIG_TIZEN_WIP
	BT_INFO("%s(%d): %02X-%02X(type:%s) handle(0x%4.4x) acl_num(%d) le_num(%d) le_num_slave(%d)", __FUNCTION__, __LINE__,
		conn->dst.b[1], conn->dst.b[0],
		(conn->type == ACL_LINK)?"ACL":
		(conn->type == LE_LINK)?"LE":
		(conn->type == SCO_LINK)?"SCO":
		(conn->type == ESCO_LINK)?"eSCO":"Unknown",
		conn->handle, hdev->conn_hash.acl_num, hdev->conn_hash.le_num, hdev->conn_hash.le_num_slave);
#endif
...
static void hci_acl_create_connection(struct hci_conn *conn)
{
	...
	if (ie) {
		if (inquiry_entry_age(ie) <= INQUIRY_ENTRY_AGE_MAX) {
			...
			BT_INFO("[%s] Inquiry information is very old ", __FUNCTION__);
		}
...
#ifdef CONFIG_TIZEN_WIP
	BT_INFO("%s(%d): %02X-%02X(type:%s) acl(%d) le(%d) le_slave(%d)", __FUNCTION__, __LINE__,
			conn->dst.b[1],conn->dst.b[0],
			(conn->type == ACL_LINK)?"ACL":
			(conn->type == LE_LINK)?"LE":
			(conn->type == SCO_LINK)?"SCO":
			(conn->type == ESCO_LINK)?"eSCO":"Unknown",
			hdev->conn_hash.acl_num,hdev->conn_hash.le_num, hdev->conn_hash.le_num_slave);
#endif
