...
void hci_req_add_le_scan_disable(struct hci_request *req)
{
	...
	BT_INFO("[%s(%d)] scan: %s, filter_dup: %s", __FUNCTION__, __LINE__,
				(cp.enable)?"Enable":"Disable",
				(cp.filter_dup)?"Enable":"Disable");
#endif
...
#ifdef CONFIG_TIZEN_WIP
	BT_INFO("[%s(%d)] send: LE_SET_ADV_ENABLE(%s)", __FUNCTION__, __LINE__, (enable)?"true":"false");
#endif
...
#ifdef CONFIG_TIZEN_WIP
			BT_INFO("[%s(%d)] hci_disconn_cmd(handle: 0x%4.4x reason: 0x%2.2x)", __FUNCTION__, __LINE__, conn->handle, reason);
#endif
