...
static int clean_up_hci_state(struct hci_dev *hdev)
{
	...
	if (test_bit(HCI_ISCAN, &hdev->flags) ||
		...
		u8 scan = 0x00;
#ifdef CONFIG_TIZEN_WIP
		BT_INFO("[%s(%d)] send: WRITE_SCAN_ENABLE: 0x%2.2x(%s)", __FUNCTION__, __LINE__,
					scan,
					(scan == SCAN_DISABLED)?"DISABLED":
					(scan == SCAN_INQUIRY)?"INQUIRY":
					(scan == SCAN_PAGE)?"PAGE":
					(scan == (SCAN_INQUIRY | SCAN_PAGE))?"INQUIRY&PAGE":"Unknown"
		);
#endif
...
#ifdef CONFIG_TIZEN_WIP
void mgmt_unpair_complete(struct hci_dev *hdev, u8 status)
{
	struct mgmt_pending_cmd *cmd;

	cmd = pending_find(MGMT_OP_UNPAIR_DEVICE, hdev);
	if (!cmd)
		return;

	BT_ERR("Pending found : MGMT_OP_UNPAIR_DEVICE");
...
#ifdef CONFIG_TIZEN_WIP
	/* release refcnt for pairing timeout */
	hci_conn_drop(conn);
#endif
...
#ifdef CONFIG_TIZEN_WIP
		BT_INFO("[%s(%d)] type: %s, interval: %d slots, window: %d slots, own_address_type: 0x%02x, filter_policy: 0x%02x", __FUNCTION__, __LINE__,
					(param_cp.type == LE_SCAN_PASSIVE)?"Passive":"Active",
					cpu_to_le16(param_cp.interval),
					cpu_to_le16(param_cp.window),
					param_cp.own_address_type,
					param_cp.filter_policy);
#endif
...
#ifdef CONFIG_TIZEN_WIP
		BT_INFO("[%s(%d)] scan: %s, filter_dup: %s", __FUNCTION__, __LINE__,
					(enable_cp.enable)?"Enable":"Disable",
					(enable_cp.filter_dup)?"Enable":"Disable");
#endif
...
{
	struct mgmt_pending_cmd *cmd;
	struct mgmt_rp_le_read_phys rp;

	BT_DBG("%s status %u", hdev->name, status);

	cmd = pending_find(MGMT_OP_LE_READ_PHYS, hdev);
	if (!cmd) {
	...
	if (status) {
		mgmt_cmd_status(cmd->sk, hdev->id, MGMT_OP_LE_READ_PHYS,
			   mgmt_status(status));
	}
	...
	mgmt_cmd_complete(cmd->sk, hdev->id, MGMT_OP_LE_READ_PHYS, 0,
			     &rp, sizeof(rp));
...
static int le_read_phy(struct sock *sk, struct hci_dev *hdev, void *data,
			u16 len)
{
	struct mgmt_cp_le_read_phys *cp = data;
	struct mgmt_pending_cmd *cmd;
	struct hci_conn *conn;
	int err = 0;

	if (!hdev_is_powered(hdev)) {
		err = mgmt_cmd_status(sk, hdev->id, MGMT_OP_LE_READ_PHYS,
				 MGMT_STATUS_NOT_POWERED);
		goto unlock;
	}
	...
	if (pending_find(MGMT_OP_LE_READ_PHYS, hdev)) {
		err = mgmt_cmd_status(sk, hdev->id, MGMT_OP_LE_READ_PHYS,
				 MGMT_STATUS_BUSY);
		goto unlock;
	}

	cmd = mgmt_pending_add(sk, MGMT_OP_LE_READ_PHYS, hdev, data, len);
	if (!cmd) {
		err = -ENOMEM;
		goto unlock;
	}
	...
	conn = hci_conn_hash_lookup_ba(hdev, LE_LINK, &cp->bdaddr);
	if (!conn) {
		mgmt_cmd_status(sk, hdev->id, MGMT_OP_LE_READ_PHYS,
				MGMT_STATUS_NOT_CONNECTED);
		mgmt_pending_remove(cmd);
		goto unlock;
	}
...
static int le_set_default_phy(struct sock *sk, struct hci_dev *hdev, void *data,
			u16 len)
{
	struct mgmt_cp_le_set_default_phys *cp = data;
	struct mgmt_pending_cmd *cmd;
	int err = 0;
	...
	if (!hdev_is_powered(hdev)) {
		err = mgmt_cmd_status(sk, hdev->id, MGMT_OP_LE_SET_DEFAULT_PHYS,
				 MGMT_STATUS_NOT_POWERED);
		goto unlock;
	}
	...
	if (pending_find(MGMT_OP_LE_SET_DEFAULT_PHYS, hdev)) {
		err = mgmt_cmd_status(sk, hdev->id, MGMT_OP_LE_SET_DEFAULT_PHYS,
				 MGMT_STATUS_BUSY);
		goto unlock;
	}

	cmd = mgmt_pending_add(sk, MGMT_OP_LE_SET_DEFAULT_PHYS, hdev, data, len);
	if (!cmd) {
		err = -ENOMEM;
		goto unlock;
	}

	err = hci_le_set_default_phy(hdev, cp->all_phy, cp->tx_phys, cp->rx_phys);
	if (!err) {
		err = mgmt_cmd_status(sk, hdev->id, MGMT_OP_LE_SET_DEFAULT_PHYS,
				 MGMT_STATUS_INVALID_PARAMS);
		mgmt_pending_remove(cmd);
	...
	err = mgmt_cmd_complete(sk, hdev->id, MGMT_OP_LE_SET_DEFAULT_PHYS, 0,
				NULL, 0);
...
static int le_set_phy(struct sock *sk, struct hci_dev *hdev, void *data,
			u16 len)
{
	struct mgmt_cp_le_set_phys *cp = data;
	struct mgmt_rp_le_set_phys rp;
	struct mgmt_pending_cmd *cmd;
	struct hci_conn *conn;
	int err = 0;
	...
	if (!hdev_is_powered(hdev)) {
		err = mgmt_cmd_status(sk, hdev->id, MGMT_OP_LE_SET_PHYS,
				 MGMT_STATUS_NOT_POWERED);
		goto unlock;
	}
	...
	if (pending_find(MGMT_OP_LE_SET_PHYS, hdev)) {
		err = mgmt_cmd_status(sk, hdev->id, MGMT_OP_LE_SET_PHYS,
				 MGMT_STATUS_BUSY);
		goto unlock;
	}

	cmd = mgmt_pending_add(sk, MGMT_OP_LE_SET_PHYS, hdev, data, len);
	if (!cmd) {
		err = -ENOMEM;
		goto unlock;
	}
	...
	conn = hci_conn_hash_lookup_ba(hdev, LE_LINK, &cp->bdaddr);
	if (!conn) {
		mgmt_cmd_status(sk, hdev->id, MGMT_OP_LE_SET_PHYS,
				MGMT_STATUS_NOT_CONNECTED);
		mgmt_pending_remove(cmd);
		goto unlock;
	}

	err = hci_le_set_phy(conn, cp->tx_phys, cp->rx_phys, cp->phy_opt);
	if (!err) {
		err = mgmt_cmd_status(sk, hdev->id, MGMT_OP_LE_SET_PHYS,
				 MGMT_STATUS_INVALID_PARAMS);
		mgmt_pending_remove(cmd);
	...
	rp.status = 0;
	mgmt_cmd_complete(sk, hdev->id, MGMT_OP_LE_SET_PHYS,
...
void mgmt_le_batching_failed(struct hci_dev *hdev, u8 status)
{
	...
	cmd = pending_find(MGMT_OP_DELETE_LE_BATCHING_INST, hdev);
	if (cmd) {
		mgmt_cmd = MGMT_OP_DELETE_LE_BATCHING_INST;
		goto done;
	}
...
{
	struct mgmt_pending_cmd *cmd;
	struct mgmt_rp_get_le_batching_buffer rp;
	...
	if (status) {
		mgmt_cmd_status(cmd->sk, hdev->id, MGMT_OP_GET_LE_BATCHING_BUFFER,
			   mgmt_status(status));
	}
	...
	mgmt_cmd_complete(cmd->sk, hdev->id, MGMT_OP_GET_LE_BATCHING_BUFFER, 0,
			     &rp, sizeof(rp));

	mgmt_pending_remove(cmd);
}

static int get_le_batching_buffer(struct sock *sk, struct hci_dev *hdev,
		void *data, u16 len)
{
	struct mgmt_pending_cmd *cmd;
	struct hci_request req;
	struct hci_cp_get_le_batching_buffer cp_en = { 0, };
	int err;
	...
	if (pending_find(MGMT_OP_GET_LE_BATCHING_BUFFER, hdev) ||
		...
		pending_find(MGMT_OP_DELETE_LE_BATCHING_INST, hdev)) {
		BT_DBG("%s", hdev->name);
		err = mgmt_cmd_status(sk, hdev->id, MGMT_OP_GET_LE_BATCHING_BUFFER,
				MGMT_STATUS_BUSY);
		goto unlocked;
	...
	cmd = mgmt_pending_add(sk, MGMT_OP_GET_LE_BATCHING_BUFFER, hdev, NULL, 0);
	if (!cmd) {
		BT_DBG("%s", hdev->name);
		err = -ENOMEM;
		goto unlocked;
	...
	BT_DBG("Get LE batching buffer");

	hci_req_add(&req, HCI_OP_LE_BATCHING, sizeof(cp_en), &cp_en);
...
{
	struct mgmt_pending_cmd *cmd;
	struct mgmt_rp_set_le_batching_param rp;
	...
	if (status) {
		mgmt_cmd_status(cmd->sk, hdev->id, MGMT_OP_SET_LE_BATCHING_PARAM,
			   mgmt_status(status));
	}
	...
	mgmt_cmd_complete(cmd->sk, hdev->id, MGMT_OP_SET_LE_BATCHING_PARAM, 0,
			     &rp, sizeof(rp));

	mgmt_pending_remove(cmd);
}

static int set_le_batching_param(struct sock *sk, struct hci_dev *hdev,
		void *data, u16 len)
{
	struct mgmt_pending_cmd *cmd;
	struct hci_request req;
	struct mgmt_cp_set_le_batching_param *cp = data;
	struct hci_cp_set_le_batching_param cp_en = { 0, };
	...
	conn = hci_conn_hash_lookup_ba(hdev, LE_LINK, &cp->bdaddr);
	if (!conn) {
		err = mgmt_cmd_complete(sk, hdev->id, MGMT_OP_SET_LE_BATCHING_PARAM,
				  MGMT_STATUS_NOT_CONNECTED,
				  NULL, 0);
		goto unlocked;
	}
	cp_en.subcode = LE_BATCHING_SUBCODE_SET_PARAM;
	cp_en.handle = conn->handle;
	...
	cmd = mgmt_pending_add(sk, MGMT_OP_SET_LE_BATCHING_PARAM, hdev, cp, sizeof(*cp));
	if (!cmd) {
		BT_DBG("%s", hdev->name);
		err = -ENOMEM;
		goto unlocked;
	...
	BT_DBG("Set LE batching param [0x%2.2X 0x%4.4X 0x%4.4X 0x%4.4X]",
			cp_en.handle, cp_en.cid,
			cp_en.pkt_th, cp_en.timeout);
...
void mgmt_set_le_batching_enable_complete(struct hci_dev *hdev, u8 status)
{
	struct mgmt_pending_cmd *cmd;
	struct mgmt_rp_set_le_batching_enable rp;
	...
	if (status) {
		mgmt_cmd_status(cmd->sk, hdev->id, MGMT_OP_SET_LE_BATCHING_ENABLE,
			   mgmt_status(status));
	}
	...
	mgmt_cmd_complete(cmd->sk, hdev->id, MGMT_OP_SET_LE_BATCHING_ENABLE, 0,
				 &rp, sizeof(rp));

	mgmt_pending_remove(cmd);
}

static int set_le_batching_enable(struct sock *sk, struct hci_dev *hdev,
		void *data, u16 len)
{
	struct mgmt_pending_cmd *cmd;
	struct hci_request req;
	struct mgmt_cp_set_le_batching_enable *cp = data;
	struct hci_cp_set_le_batching_enable cp_en = { 0, };
	struct hci_conn *conn = NULL;
	int err;
	...
	conn = hci_conn_hash_lookup_ba(hdev, LE_LINK, &cp->bdaddr);
	if (!conn) {
		err = mgmt_cmd_complete(sk, hdev->id, MGMT_OP_SET_LE_BATCHING_ENABLE,
				  MGMT_STATUS_NOT_CONNECTED,
				  NULL, 0);
		goto unlocked;
	}
	cp_en.subcode = LE_BATCHING_SUBCODE_SET_ENABLE;
	cp_en.handle = conn->handle;
	cp_en.cid = L2CAP_CID_ATT;
	...
	cmd = mgmt_pending_add(sk, MGMT_OP_SET_LE_BATCHING_ENABLE, hdev, cp, sizeof(*cp));
	if (!cmd) {
		BT_DBG("%s", hdev->name);
		err = -ENOMEM;
		goto unlocked;
	...
	BT_DBG("%sable LE batching",
		(cp->enable == 0x01) ? "En" : "Dis");
...
void mgmt_delete_le_batching_inst_complete(struct hci_dev *hdev, u8 status)
{
	struct mgmt_pending_cmd *cmd;
	...
	if (status) {
		mgmt_cmd_status(cmd->sk, hdev->id, MGMT_OP_DELETE_LE_BATCHING_INST,
			   mgmt_status(status));
	}

	mgmt_cmd_complete(cmd->sk, hdev->id, MGMT_OP_DELETE_LE_BATCHING_INST,
				status, NULL, 0);

	mgmt_pending_remove(cmd);
}

static int delete_le_batching_inst(struct sock *sk, struct hci_dev *hdev,
		void *data, u16 len)
{
	struct mgmt_pending_cmd *cmd;
	struct hci_request req;
	struct mgmt_cp_delete_le_batching_inst *cp = data;
	struct hci_cp_delete_le_batching_inst cp_en = { 0, };
	struct hci_conn *conn = NULL;
	int err;
	...
	if (!hdev_is_powered(hdev)) {
		err = mgmt_cmd_status(sk, hdev->id, MGMT_OP_DELETE_LE_BATCHING_INST,
				MGMT_STATUS_NOT_POWERED);
		goto unlocked;
	}
	...
	conn = hci_conn_hash_lookup_ba(hdev, LE_LINK, &cp->bdaddr);
	if (!conn) {
		err = mgmt_cmd_complete(sk, hdev->id, MGMT_OP_DELETE_LE_BATCHING_INST,
				  MGMT_STATUS_NOT_CONNECTED,
				  NULL, 0);
		goto unlocked;
	}
	cp_en.subcode = LE_BATCHING_SUBCODE_DEL_INSTANCE;
	cp_en.handle = conn->handle;
	cp_en.cid = L2CAP_CID_ATT;
	...
	cmd = mgmt_pending_add(sk, MGMT_OP_DELETE_LE_BATCHING_INST, hdev, cp, sizeof(*cp));
	if (!cmd) {
		BT_DBG("%s", hdev->name);
		err = -ENOMEM;
		goto unlocked;
...
static const struct hci_mgmt_handler tizen_mgmt_handlers[] = {
	...
	{ le_read_phy,		   MGMT_LE_SET_READ_PHYS_SIZE},
	{ le_set_default_phy,	   MGMT_OP_LE_SET_DEFAULT_PHYS_SIZE},
	{ le_set_phy,		   MGMT_LE_SET_PHY_SIZE},
	{ get_le_batching_buffer,	   MGMT_GET_LE_BATCHING_BUFFER_SIZE},
	{ set_le_batching_param,		   MGMT_SET_LE_BATCHING_PARAM_SIZE},
	{ set_le_batching_enable,		   MGMT_SET_LE_BATCHING_ENABLE_SIZE},
	{ delete_le_batching_inst,		   MGMT_DELETE_LE_BATCHING_INST_SIZE},
};
#endif
...
#ifdef CONFIG_TIZEN_WIP
void mgmt_le_phy_updated(struct hci_dev *hdev, u8 status,
		bdaddr_t *bdaddr, u8 tx_phys, u8 rx_phys)
{
	struct mgmt_ev_le_phy_updated ev;

	bacpy(&ev.addr, bdaddr);
	ev.status = status;
	ev.tx_phys = tx_phys;
	ev.rx_phys = rx_phys;

	mgmt_event(MGMT_EV_LE_PHY_UPDATED, hdev, &ev, sizeof(ev), NULL);
}
...
{
	char buf[512];
	struct mgmt_ev_dbfw_plus *ev = (void *) buf;
	...
	ev->event_code = evt_code;
	memcpy(ev->data, data, len - sizeof(u8));

	mgmt_event(MGMT_EV_DBFW_PLUS, hdev, buf, len, NULL);
}

void mgmt_alf_info(struct hci_dev *hdev, struct sk_buff *skb)
{
	struct mgmt_ev_alf *ev = (void *) skb->data;
	struct mgmt_ev_alf mgmt_ev;

	mgmt_ev.handle = cpu_to_le16(ev->handle);
	mgmt_ev.link_status = ev->link_status;
	mgmt_ev.packet_count = cpu_to_le16(ev->packet_count);
	mgmt_ev.retx_count = cpu_to_le16(ev->retx_count);

	BT_INFO("[%s(%d)] handle(%04x) status(%d) pk_count(%d), retx(%d)",
				__FUNCTION__, __LINE__, mgmt_ev.handle, mgmt_ev.link_status,
				mgmt_ev.packet_count, mgmt_ev.retx_count);

	mgmt_event(MGMT_EV_ALF, hdev, &mgmt_ev, sizeof(struct mgmt_ev_alf), NULL);
}
...
void mgmt_hardware_error(struct hci_dev *hdev, u8 err_code)
{
	...
	BT_INFO("[%s(%d)] Bluetooth will recover soon,..", __FUNCTION__, __LINE__);
