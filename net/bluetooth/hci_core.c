...
/* Disable Secure connection implementation now */
	...
	if (hdev->commands[35] & 0x20) {
		...
		if (hdev->le_features[1] & HCI_LE_LE_PHY_2M) {

			/* If the controller supports LE 2M PHY feature,
			 * set 2M by default.
			 */
			cp.tx_phys = 0x02;
			cp.rx_phys = 0x02;
...
#ifdef CONFIG_TIZEN_WIP
	{
		/* DBFW+ RSSI by NOC Enable */
		struct hci_cp_dbfw_plus_rssi_by_noc cp;
		cp.subcommand = DBFW_PLUS_RSSI_NOC_EVENT_CODE;
		cp.enable = 0x01;
		hci_req_add(req, HCI_OP_DBFW_PLUS,
				 sizeof(cp), &cp);
	}

#if defined(CONFIG_NOBLESSE)
	/* Set EWP	*/
	{
		/* EWP Module flags. This indicates modules that
		 * are to be EWP enabled in the controller.
		 */
#define EWP_MODULE_CUST_SPECIFIC (1 << 0)	/* Customer specific */
#define EWP_MODULE_SYSTEM (1 << 1)			/* System */
#define EWP_MODULE_RF (1 << 2)				/* Radio module */
#define EWP_MODULE_COEX (1 << 3)			/* BT-WiFi coex */
#define EWP_MODULE_HCE_DEBUG (1 << 31)		/* Health check */

		/* EWP Feature flags. This indicates features  that
		 * are to be EWP enabled in the controller.
		 */
#define EWP_FEATURE_ID_TRACE (1 << 0)         /* Generic trace */
#define EWP_FEATURE_ID_STRING_TRACE (1 << 1)  /* String trace */
#define EWP_FEATURE_ID_HC (1 << 2)            /* Health check */
#define EWP_FEATURE_ID_STRUCT_TRACE (1 << 3)  /* Struct trace */
#define EWP_FEATURE_ID_MEM_EXP_TRACE (1 << 4) /* Memory trace*/
#define EWP_FEATURE_ID_BT_REALTIME (1 << 5)   /* BT realtime trace */
#define EWP_FEATURE_ID_OS_REALTIME (1 << 6)   /* BT OS trace */

		struct hci_cp_enable_ewp cp;
		unsigned int ewp_mod_mask =
				EWP_MODULE_CUST_SPECIFIC | EWP_MODULE_SYSTEM |
				EWP_MODULE_RF | EWP_MODULE_COEX |
				EWP_MODULE_HCE_DEBUG;

		unsigned int ewp_features_mask =
				EWP_FEATURE_ID_TRACE | EWP_FEATURE_ID_STRING_TRACE |
				EWP_FEATURE_ID_HC | EWP_FEATURE_ID_STRUCT_TRACE |
				EWP_FEATURE_ID_MEM_EXP_TRACE |
				EWP_FEATURE_ID_BT_REALTIME |
				EWP_FEATURE_ID_OS_REALTIME;

		memset(&cp, 0x00, sizeof(struct hci_cp_enable_ewp));
		cp.subcommand = 0x01;
		...
		/* EWP module mask */
		cp.params[1] = ewp_mod_mask&0xff;
		cp.params[2] = (ewp_mod_mask>>8)&0xff;
		cp.params[3] = (ewp_mod_mask>>16)&0xff;
		cp.params[4] = (ewp_mod_mask>>24)&0xff;

		/* EWP feature mask */
		cp.params[9] = ewp_features_mask&0xff;
		cp.params[10] = (ewp_features_mask>>8)&0xff;
		cp.params[11] = (ewp_features_mask>>16)&0xff;
		cp.params[12] = (ewp_features_mask>>24)&0xff;

		/* Logging Level */
		cp.params[13] = 0x01;	// critical + warning

		/* Periodicity at which HC Engine will run */
		cp.params[14] = 0x02;	// 2 sec
		cp.params[15] = 0x00;

		hci_req_add(req, HCI_OP_EWP, sizeof(cp), &cp);
	}
#endif	// defined(CONFIG_NOBLESSE)
#endif
...
#else
	BT_INFO("[%s(%d)] We need memory dump from Bluetooth controller. Bluez will recv 'MGMT_EV_HARDWARE_ERROR' instead of 'MGMT_EV_TX_TIMEOUT_ERROR'.", __FUNCTION__, __LINE__);
	hci_reset_dev(hdev);
#endif
}
...
static void hci_tx_work(struct work_struct *work)
{
	...
	BT_DBG("[%s] %s acl %d sco %d le %d", __FUNCTION__, hdev->name,
			hdev->acl_cnt, hdev->sco_cnt, hdev->le_cnt);
