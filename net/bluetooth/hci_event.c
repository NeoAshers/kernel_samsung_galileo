...
#ifdef CONFIG_TIZEN_WIP
	BT_INFO("[%s(%d)] recv: WRITE_SCAN_ENABLE(status: 0x%2.2x)", __FUNCTION__, __LINE__, status);
#else
	BT_DBG("%s status 0x%2.2x", hdev->name, status);
#endif
...
#ifdef CONFIG_TIZEN_WIP
	BT_INFO("[%s(%d)] recv: LE_SET_ADV_ENABLE(status: 0x%2.2x)", __FUNCTION__, __LINE__, status);
#else
	BT_DBG("%s status 0x%2.2x", hdev->name, status);
#endif
...
static void hci_cc_le_batching(struct hci_dev *hdev,
					struct sk_buff *skb)
{
	...
	case LE_BATCHING_SUBCODE_GET_BUFFER:
		mgmt_get_le_batching_buffer_complete(hdev,
					rp2->status, rp2->buf_size);
		break;
	case LE_BATCHING_SUBCODE_SET_PARAM:
		mgmt_set_le_batching_param_complete(hdev, rp->status);
		break;
	case LE_BATCHING_SUBCODE_SET_ENABLE:
		mgmt_set_le_batching_enable_complete(hdev, rp->status);
		break;
	case LE_BATCHING_SUBCODE_DEL_INSTANCE:
		mgmt_delete_le_batching_inst_complete(hdev, rp->status);
		break;
	default:
		BT_DBG("%s subcode 0x%2.2x", hdev->name, rp->subcode);
		break;
	}
}
#endif
				 ...
				 hci_req_complete_skb_t *req_complete_skb)
{
	...
	*status = skb->data[sizeof(*ev)];
	...
	case HCI_OP_LE_READ_PHY:
		...
		break;
	case HCI_OP_LE_BATCHING:
		hci_cc_le_batching(hdev, skb);
		break;
#endif
	default:
...
#ifdef CONFIG_TIZEN_WIP
	BT_INFO("[%s(%d)] %s status: 0x%2.2x, new role: %s(0x%2.2x)", __FUNCTION__, __LINE__,
									hdev->name, ev->status,
...
#ifdef CONFIG_TIZEN_WIP
		/* The handle of NOC is 2 bytes(12 Bits meaningful)
		 * and 4 Bits is used for RSSI
		 */
		handle = __le16_to_cpu(info->handle) & 0xFFF;
#else
...
#else
		/* Chip Crash */
		BT_INFO("[%s] Hardware Error due to chip crash(0xcc)", __func__);
		mgmt_hardware_error(hdev, 0xCC);
#endif
	}
}
...
static void hci_vendor_specific_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	case SEC_BRCM_DBFW_PLUS_DUMP_EVENT: {
		...
		if((event_code == DBFW_PLUS_LINK_LOSS_INFO_EVENT_CODE) || (event_code == DBFW_PLUS_LINK_LOSS_CLOCK_INFO_EVENT_CODE) || (event_code == DBFW_PLUS_LINK_LOSS_PAGE_SLOTS_EVENT_CODE))
		{
			BT_INFO("SEC_BRCM_DBFW_PLUS_DUMP_EVENT: 0x%02x(%s)", event_code,
									(event_code == DBFW_PLUS_LINK_LOSS_INFO_EVENT_CODE)?"Link Loss Info":
									(event_code == DBFW_PLUS_LINK_LOSS_CLOCK_INFO_EVENT_CODE)?"Link Loss Clock Info":
									(event_code == DBFW_PLUS_LINK_LOSS_PAGE_SLOTS_EVENT_CODE)?"Link Loss Page Slots":
									(event_code == DBFW_PLUS_LINK_LOSS_LMP_TRACE_EVENT_CODE)?"Link Loss LMP Trace":
		...
		{
			BT_ERR_RATELIMITED("SEC_BRCM_DBFW_PLUS_DUMP_EVENT: 0x%02x(%s)", event_code,
										(event_code == DBFW_PLUS_LINK_LOSS_INFO_EVENT_CODE)?"Link Loss Info":
										(event_code == DBFW_PLUS_LINK_LOSS_CLOCK_INFO_EVENT_CODE)?"Link Loss Clock Info":
										(event_code == DBFW_PLUS_LINK_LOSS_PAGE_SLOTS_EVENT_CODE)?"Link Loss Page Slots":
										(event_code == DBFW_PLUS_LINK_LOSS_LMP_TRACE_EVENT_CODE)?"Link Loss LMP Trace":
	...
	case SEC_BRCM_ALF_EVENT:

		/* A2DP link Feedback */
		mgmt_alf_info(hdev, skb);
		break;
