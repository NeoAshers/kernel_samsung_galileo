...
#define PAD_RETENTION_VTS_OPTION		(0x4438)
#define VTS_CPU_STATUS				(0x4428)
#define VTS_CPU_STATUS_STATUS_MASK		(0x00000001)
#define VTS_CPU_STANDBY				(0x3814)
#define VTS_CPU_STANDBY_STANDBYWFI_MASK		(0x10000000)
#define VTS_CPU_OPTION				(0x3818)
#define VTS_CPU_OPTION_USE_STANDBYWFI_MASK	(0x00010000)
#define VTS_CPU_RESET_OPTION			(0x442C)
#define VTS_CPU_RESET_OPTION_ENABLE_CPU_MASK	(0x10000000)
#define VTS_LOGIC_CLAMP_OPTION			(0x44A0)
#define VTS_LOGIC_CLAMP_OPTION_INIT_WAKEUP_MASK	(0x10000000)
#define VTS_LOGIC_CLAMP_OPTION_AUTO_WAKEUP_MASK	(0x20000000)
#define VTS_LOGIC_CLAMP_STATUS			(0x449C)
#define VTS_LOGIC_CLAMP_STATUS_STATUS_MASK	(0x00000001)
#define VTS_CTRL				(0x0078)
#define VTS_CTRL_START_ENABLE_MASK		(0x00000200)
#define VTS_CTRL_START_DONE_MASK		(0x00000100)
#define VTS_OSC_CON2_OSC			(0x0B08)
#define VTS_OSC_CON2_OSC_MAS_REQ_RCO_MASK	(0x00000002)
/* For VTS TZPC Configuration */
...
#define VTS_TRIGGERED_TIMEOUT_MS (1000)
#undef VTS_SLEEP_TRIGGER

#define USE_WAKEUP_KEY_EVENT

/* For only external static functions */
static struct vts_data *p_vts_data;

#ifdef USE_WAKEUP_KEY_EVENT
...
#if defined(CONFIG_SOC_EXYNOS9810) || defined(CONFIG_SOC_EXYNOS9110)
static u32 vts_set_baaw(void __iomem *sfr_base, u64 base, u32 size)
{
	...
	/*
	void __iomem *baaw_soc;

	baaw_soc = ioremap(0x10420000, SZ_4K);
	// set BAAW SoC Security config
	writel(0x11000, baaw_soc + 0x00);
	writel(0x11100, baaw_soc + 0x04);
	writel(0x11000, baaw_soc + 0x08);
	writel(0x80000003, baaw_soc + 0x0C);
	// set BAAW SoC SFR config
	writel(0x11100, baaw_soc + 0x10);
	writel(0x11200, baaw_soc + 0x14);
	writel(0x40000, baaw_soc + 0x18);
	writel(0x80000003, baaw_soc + 0x1C);
	// set BAAW SoC SRAM config
	writel(0x11200, baaw_soc + 0x20);
	writel(0x112d0, baaw_soc + 0x24);
	writel(0x0, baaw_soc + 0x28);
	writel(0x80000003, baaw_soc + 0x2C);
	iounmap(baaw_soc);
	*/
...
static void vts_pmu_init(void)
{
	unsigned int status = 0;
	void __iomem *vts_cmu;
	int i = 1000;

	/*
	 * ISO Release
	 * Write : LOGIC_CLAMP_VTS_OPTION(0x1186_44a0), bit[28] , 0x1
	 * Read Till : LOGIC_CLAMP_VTS_STATUS(0x1186_449c), bit[0], 0x1

	 * Reset release (__START_ENABLE)
	 * Write : VTS_CTRL(0x1186_0078), bit[9] , 0x1
	 * Read Till : VTS_CTRL(0x1186_0078), bit[8], 0x1

	 * Clear (__START_ENABLE)
	 * Write : VTS_CTRL(0x1186_0078), bit[9] , 0x0
	 * Write : LOGIC_CLAMP_VTS_OPTION(0x1186_44a0), bit[29] , 0x1
	 * Write : LOGIC_CLAMP_VTS_OPTION(0x1186_44a0), bit[28] , 0x0

	 * RCO request enable
	 * Write : OSC_CON2_OSC(0x1186_0b08), bit[1] , 0x0
	 *
	 * VTS AUTOCLKGATING ENABLE
	 * Write : VTS_CMU_VTS_CONTROLLER_OPTION(0x1110_0800), bit[29:28] , 0x3
	 */

	exynos_pmu_update(VTS_LOGIC_CLAMP_OPTION,
			VTS_LOGIC_CLAMP_OPTION_INIT_WAKEUP_MASK,
			VTS_LOGIC_CLAMP_OPTION_INIT_WAKEUP_MASK);
	/* wait for VTS LOGIC CLAMP in STATUS SFR */
	do {
		exynos_pmu_read(VTS_LOGIC_CLAMP_STATUS, &status);
	} while (i-- && !(status & VTS_LOGIC_CLAMP_STATUS_STATUS_MASK));
	pr_info("%s : VTS_LOGIC_CLAMP_STATUS = %x\n", __func__, status);

	exynos_pmu_update(VTS_CTRL,
			VTS_CTRL_START_ENABLE_MASK,
			VTS_CTRL_START_ENABLE_MASK);
	/* wait for VTS START DONE STATUS in CONTROL SFR */
	i = 1000;
	do {
		exynos_pmu_read(VTS_CTRL, &status);
	} while (i-- && !(status & VTS_CTRL_START_DONE_MASK));
	pr_info("%s : VTS_CTRL = %x\n", __func__, status);

	exynos_pmu_update(VTS_CTRL,
			VTS_CTRL_START_ENABLE_MASK, 0);

	exynos_pmu_update(VTS_LOGIC_CLAMP_OPTION,
			VTS_LOGIC_CLAMP_OPTION_AUTO_WAKEUP_MASK,
			VTS_LOGIC_CLAMP_OPTION_AUTO_WAKEUP_MASK);

	exynos_pmu_update(VTS_LOGIC_CLAMP_OPTION,
			VTS_LOGIC_CLAMP_OPTION_INIT_WAKEUP_MASK, 0);

	exynos_pmu_update(VTS_OSC_CON2_OSC,
			VTS_OSC_CON2_OSC_MAS_REQ_RCO_MASK, 0);

	vts_cmu = ioremap(0x11100000, SZ_4K);
	writel(0x30000000, vts_cmu + 0x800);
	iounmap(vts_cmu);
}

int vts_blk_pmu_init(void)
{
	unsigned long EXYNOS_VTS = 3;
	int result = 0;

	vts_pmu_init();

	result = exynos_smc(SMC_CMD_CONN_IF,
			(EXYNOS_VTS << 32) | EXYNOS_SET_CONN_TZPC,
			0, 0);
	if (result < 0)
		pr_err("Failed to set VTS TZPC \n");
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_info(dev, "VTS RUNTIME, %s , %d, count = %d\n",
	...
	if (vcrecognize_mode < VTS_VOICE_TRIGGER_MODE ||
		...
		mutex_lock(&data->ipc_mutex);

		if (vcrecognize_mode <= VTS_SENSORY_TRIGGER_MODE) {
			dev_info(dev, "VTS RUNTIME ON, %s , %d\n", __func__, __LINE__);
		...
		if (!vcrecognize_start &&
			...
			dev_info(dev, "VTS RUNTIME off - 0, %s , %d, count = %d\n",
			             __func__, __LINE__, atomic_read(&dev->power.usage_count));
			rpm_ret = pm_runtime_put_sync(dev);
			dev_info(dev, "VTS RUNTIME off - 1, %s , %d, rpm_ret = %d\n",
			             __func__, __LINE__, rpm_ret);
		}
		mutex_unlock(&data->ipc_mutex);
	}
...
err_ipcmode:
	if (pm_runtime_active(dev) && (vcrecognize_start ||
		data->exec_mode & (0x1 << vcrecognize_mode) ||
		data->voicerecog_start & (0x1 << data->active_trigger))) {
		dev_info(dev, "VTS RUNTIME off, %s , %d\n", __func__, __LINE__);
...
static void send_wakeup_key_event(void)
{
    unsigned int keycode = KEY_VOICE_WAKEUP;

    pr_info("ABOX recognition: Raise key event (%d)\n", keycode);
...
static int vts_suspend(struct device *dev)
{
	...
	if (data->vts_ready) {
        struct snd_ctl_elem_value ucontrol;
        ucontrol.value.integer.value[0] = VTS_VT_ALWAYS_ON_MODE_OFF;
        if (data->card) {
            result = set_vtsvoicerecognize_mode(
            snd_soc_card_get_kcontrol(data->card,
                "VTS VoiceRecognization Mode"), &ucontrol);
        }
    }
...
static int vts_runtime_resume(struct device *dev)
{
	...
	if (result < 0) {
		dev_err(dev, "Failed to enable the VTS RCO clock\n");
		goto error_clk;
	}
#endif
...
// vts_svoice_model mode changed 666
static const BIN_ATTR(vts_svoice_model, (S_IRUGO | S_IWUGO),
        vts_svoice_model_read,
        vts_svoice_model_write,
        VTS_MODEL_SVOICE_BIN_MAXSZ);
...
#if !defined(VTS_SLEEP_TRIGGER)
        dev_dbg(dev, "VTS RUNTIME SUSPEND PREPARE %s , usage_count = %d\n",
		   __func__, atomic_read(&dev->power.usage_count));
		if (data->vts_ready) {
		    struct snd_ctl_elem_value ucontrol;
			int result = 0;
			ucontrol.value.integer.value[0] = VTS_VT_ALWAYS_ON_MODE_OFF;
			dev_info(dev, "%s, SUSPEND PREPARE\n", __func__);
			if (data->card) {
			    result = set_vtsvoicerecognize_mode(
			    snd_soc_card_get_kcontrol(data->card,
			       "VTS VoiceRecognization Mode"), &ucontrol);
			    if (result) {
			        dev_err(dev, "Failed to suspend VTS\n");
					return NOTIFY_BAD;
				}
			}
		}
...
static int samsung_vts_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_SOC_EXYNOS8895) || defined(CONFIG_SOC_EXYNOS9110)
    ...
    if (init_input_device() < 0) {
        dev_err(dev, "%s: input device init failed\n", __func__);
        goto error;
    }
#endif
