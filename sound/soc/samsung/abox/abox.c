...
#define abox_log_dev_info(prefix, fmt, args...)					\
	abox_log_info("[abox]" prefix fmt, ##args)
#define dev_info(dev, fmt, args...)						\
	abox_log_dev_info("(%d)", fmt, dev->id, ##args)
/*	abox_log_dev_info("[%s(%d)%d]", fmt, __func__, __LINE__, dev->id, ##args) */

#define abox_log_dev_err(prefix, fmt, args...)					\
	abox_log_err("[abox]" prefix fmt, ##args)
#define dev_err(dev, fmt, args...)						\
	abox_log_dev_err("(%d)", fmt, dev->id, ##args)

#define abox_log_dev_warn(prefix, fmt, args...)					\
	abox_log_warn("[abox]" prefix fmt, ##args)
#define dev_warn(dev, fmt, args...)						\
	abox_log_dev_warn("(%d)", fmt, dev->id, ##args)
...
#define GPIO_MODE_ABOX_SYS_PWR_REG		(0x1434)
#define PAD_RETENTION_ABOX_OPTION		(0x4188)
#define ABOX_MAGIC				(0x0814)
#define ABOX_MAGIC_VALUE			(0xAB0CAB0C)
#define ABOX_CPU_CONFIGURATION			(0x4174)
#define ABOX_CPU_LOCAL_PWR_CFG			(0x00000001)
#define ABOX_CPU_STATUS				(0x4178) /* CPU_RESET_DISPAUD_STATUS */
#define ABOX_CPU_STATUS_STATUS_MASK		(0x00000001)
#define ABOX_CPU_STANDBY			(0x3804)
#define ABOX_CPU_STANDBY_WFE_MASK		(0x20000000)
#define ABOX_CPU_STANDBY_WFI_MASK		(0x10000000)
#define ABOX_CPU_OPTION				(0x417C) /* CPU_RESET_DISPAUD_OPTION */
#define ABOX_CPU_OPTION_ENABLE_CPU_MASK		(0x10000000)
#endif
...
void *abox_addr_to_kernel_addr(struct abox_data *data, unsigned int addr)
{
	...
	else if (addr >= IOVA_DRAM_FIRMWARE && addr < IOVA_VSS_FIRMWARE)
		...
		unsigned int width)
{
	...
	if (data->uaif_max_div <= 32) {
		if ((rate % 44100) == 0)
			audif_rate = (unsigned long)(((rate > 176400) ? 352800 : 176400) *
					width * 2);
		else
			audif_rate = (unsigned long)(((rate > 192000) ? 384000 : 192000) *
					width * 2);
		...
		if ((rate % 44100) == 0)
			audif_rate = (unsigned long)(352800 * clk_width * clk_channels);
		else
			audif_rate = (unsigned long)(384000 * clk_width * clk_channels);

		if (audif_rate < rate * width * channels)
			audif_rate = (unsigned long)(rate * width * channels);
	}

	if (abox_test_quirk(data, ABOX_QUIRK_LSI_CODEC)) {
		dev_dbg(dev, "use lsi codec\n");
		audif_rate = 24576000;
	}
...
static int abox_cmpnt_probe(struct snd_soc_component *component)
{
	...
	snd_soc_component_init_regmap(component, data->regmap);
...
static bool abox_readable_reg_9610(struct device *dev, unsigned int reg)
{
	switch (reg) {
	case ABOX_UAIF_CTRL0(4):
	case ABOX_UAIF_CTRL1(4):
		return false;
	default:
		return abox_readable_reg(dev, reg);
	}
}

static bool abox_writeable_reg_9610(struct device *dev, unsigned int reg)
{
	switch (reg) {
	case ABOX_UAIF_CTRL0(4):
	case ABOX_UAIF_CTRL1(4):
...
static bool abox_writeable_reg_9110(struct device *dev, unsigned int reg)
{
	switch (reg) {
	case ABOX_UAIF_CTRL0(3):
	case ABOX_UAIF_CTRL1(3):
...
static bool abox_timer_readable_reg_9110(struct device *dev, unsigned int reg)
{
	return abox_timer_readable_reg(dev, reg);
}

static bool abox_timer_writeable_reg_9110(struct device *dev, unsigned int reg)
{
	return abox_timer_readable_reg(dev, reg);
}
...
bool abox_cpu_gear_idle(struct device *dev, struct abox_data *data, void *id)
{
	...
	for (request = data->cpu_gear_requests;
			...
			&& request->id;
			request++) {
		if (request->id == data->gear_lock_request.id) {
			dev_info(dev, "%s(%d) 0x%lx 0x%lx %d\n",
					__func__, __LINE__,
					(unsigned long)data->gear_lock_request.id,
					(unsigned long)request->id,
					data->gear_lock_request.value);

			if (data->gear_lock_request.value < ABOX_CPU_GEAR_MIN)
				return false;
		}
		if (id == request->id) {
		...
		const void *id, unsigned int gear)
{
	...
	dev_info(dev, "%s:%s(id:%p level:%u)\n",
			"req_cpu_gear",
			dev->kobj.name ? dev->kobj.name : "NIL",
			id, gear);
...
static void abox_download_extra_firmware(struct abox_data *data)
{
	...
	for (ext_fw = data->firmware_extra; ext_fw - data->firmware_extra <
		...
		memcpy(base + ext_fw->offset, ext_fw->firmware->data,
				ext_fw->firmware->size);
		dev_dbg(dev, "%s is downloaded at area %u offset %u\n",
				ext_fw->name, ext_fw->area, ext_fw->offset);
	}
}
#else
...
static void abox_wait_suspend_done(struct device *dev, struct abox_data *data)
{
	unsigned int reg_val = 0;
	unsigned int reg_val_old = 0;
	void __iomem *suspend_ack = data->sram_base + (data->sram_size - 0x8);
	u64 timeout = local_clock() + ABOX_SUSPEND_TIMEOUT_NS;

	while (readl(suspend_ack) != ABOX_SUSPEND_MAGIC) {
		if (local_clock() <= timeout) {
			mdelay(1);

			reg_val = readl(suspend_ack);
			if (reg_val != reg_val_old) {
				dev_info(dev, "during %s(0x%x)\n", __func__, reg_val);
				reg_val_old = reg_val;
			}
			continue;
		}
...
static int abox_disable(struct device *dev)
{
	...
	if (state != CALLIOPE_DISABLED) {
		...
		abox_wait_suspend_done(dev, data);
	}
...
static int abox_runtime_suspend(struct device *dev)
{
    ...
    abox_dbg_dump_simple(dev, p_abox_data, "runtime_suspend data");
...
static int samsung_abox_probe(struct platform_device *pdev)
{
	...
	data->timer_base = devm_request_and_map_byname(pdev, "timer",
			NULL, NULL);
	...
	dev_info(dev, "%s(%pa) is mapped on %p with size of %d\n",
...
#ifdef ABOX_DUMMY_VSS_ALLOC
{
	void *vaddr = NULL;
//	paddr = virt_to_phys(kzalloc(SZ_512K, GFP_KERNEL));
	vaddr = dmam_alloc_coherent(dev, SZ_4M, &paddr, GFP_KERNEL);

	dev_info(dev, "%s(%pa) is mapped on %p\n",
			"vss firmware", &paddr, phys_to_virt(paddr));
	iommu_map(data->iommu_domain, IOVA_VSS_FIRMWARE, paddr,
			SZ_4M, 0);

	paddr = virt_to_phys(kzalloc(SZ_128K, GFP_KERNEL));
	dev_info(dev, "%s(%pa) is mapped on %p\n",
...
//	}
	...
	if (IS_ENABLED(CONFIG_SOC_EXYNOS8895)) {
		...
		abox_regmap_config.readable_reg = abox_readable_reg_9610;
		abox_regmap_config.writeable_reg = abox_writeable_reg_9610;
	} else if (IS_ENABLED(CONFIG_SOC_EXYNOS9110)) {
		abox_regmap_config.reg_defaults = abox_reg_defaults_9110;
		abox_regmap_config.num_reg_defaults =
				ARRAY_SIZE(abox_reg_defaults_9110);
		abox_regmap_config.readable_reg = abox_readable_reg_9110;
		abox_regmap_config.writeable_reg = abox_writeable_reg_9110;

		abox_timer_regmap_config.readable_reg = abox_timer_readable_reg_9110;
		abox_timer_regmap_config.writeable_reg = abox_timer_writeable_reg_9110;
	} else {
		BUG_ON(1);
	}
