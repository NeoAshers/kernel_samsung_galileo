...
static void *abox_dbg_alloc_mem_atomic(struct device *dev, struct abox_dbg_dump_min *p_dump)
{
	...
	alloc_size = sizeof(struct page *) * npages;
	p_dump->pages = kzalloc(alloc_size, alloc_gfp_flag);
	if (!p_dump->pages)
		return NULL;
...
#define __BIN_ATTR_RNO(_name, _size) {					\
	.attr	= { .name = __stringify(_name), .mode = 0440 },	\
	.read	= _name##_read,						\
	.size	= _size,						\
}

#define BIN_ATTR_RNO(_name, _size)					\
struct bin_attribute bin_attr_##_name = __BIN_ATTR_RNO(_name, _size)

/* size will be updated later */
static BIN_ATTR_RNO(calliope_sram, 0);
static BIN_ATTR_RNO(calliope_dram, DRAM_FIRMWARE_SIZE);
		...
		struct device_attribute *attr, char *buf)
{
	...
	buf_len += scnprintf(buf, TMP_BUF_LEN,
			"========================================\n");

	buf_len += scnprintf(&buf[buf_len], TMP_BUF_LEN,
			"A-Box CPU register dump (%c%c%c%c)\n",
			version[3], version[2], version[1], version[0]);
	buf_len += scnprintf(&buf[buf_len], TMP_BUF_LEN,
			"----------------------------------------\n");
	for (i = 0; i <= 14; i++) {
...
static int samsung_abox_debug_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "abox_dbg_dump size (%ld) (%ld)\n",
			sizeof(struct abox_dbg_dump) * ABOX_DBG_DUMP_COUNT,
			sizeof(struct abox_dbg_dump_simple) * ABOX_DBG_DUMP_COUNT);
