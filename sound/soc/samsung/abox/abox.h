...
#define DEBUG_PRINT_FUNNAME_LINE
...
#define abox_log_err(fmt, ...)		printk(KERN_ERR fmt, ##__VA_ARGS__)
#define abox_log_warn(fmt, ...)		printk(KERN_WARNING fmt, ##__VA_ARGS__)
#define abox_log_info(fmt, ...)		printk(KERN_INFO fmt, ##__VA_ARGS__)
#define abox_log_dbg(fmt, ...)		printk(KERN_DEBUG fmt, ##__VA_ARGS__)
#else
#define abox_log_err(fmt, ...)		pr_err(fmt, ##__VA_ARGS__)
#define abox_log_warn(fmt, ...)		pr_warning(fmt, ##__VA_ARGS__)
#define abox_log_info(fmt, ...)		pr_info(fmt, ##__VA_ARGS__)
#define abox_log_dbg(fmt, ...)		pr_info(fmt, ##__VA_ARGS__)
#define abox_log_dbg(fmt, ...)		pr_info(fmt, ##__VA_ARGS__)
#endif
...
//If Offload is enabled we have to change CMA SIZE 1M to 4M
...
#define ABOX_DMA_TIMEOUT_NS		(400000000)
#define ABOX_SUSPEND_TIMEOUT_NS		(100000000)
#define ABOX_SUSPEND_MAGIC		(0x53555350)
...
#define ABOX_QUIRK_LSI_CODEC		(1 << 5)
...
struct abox_data {
	...
	struct abox_qos_request gear_lock_request;
...
/* SVACE */
	...
	bool sif_auto_config[SET_INMUX4_FORMAT];
#else
	snd_pcm_format_t sif_format_min[SET_INMUX4_FORMAT -
			SET_MIXER_FORMAT + 1];
	unsigned int sif_channels_min[SET_INMUX4_FORMAT -
