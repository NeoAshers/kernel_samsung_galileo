...
#define COD9005X_SPK_BST_HR_MIN 0x00
#define COD9005X_SPK_BST_HR_MAX 0xFF
#define COD9005X_SPK_BST_HR0_DEFAULT 0x10
#define COD9005X_SPK_BST_HR1_DEFAULT 0x00
...
static void cod9005x_codec_initialize(void *context)
{
...
#if defined(CONFIG_RENAISSANCE_SMALL)
	snd_soc_write(codec, COD9005X_4A_DMIC2, 0x18); // 1.5Mhz for small model
#else
	...
	snd_soc_write(codec, COD9005X_92_BOOST_HR0, cod9005x->boost_hr0);
	snd_soc_write(codec, COD9005X_93_BOOST_HR1, cod9005x->boost_hr1);
...
static void cod9005x_i2c_parse_dt(struct cod9005x_priv *cod9005x)
{
	...
	unsigned int slope_param, boost_param;
	int ret;
	...
	if (!ret)
		if (boost_param < COD9005X_SPK_BST_HR_MIN)
			cod9005x->boost_hr0 = COD9005X_SPK_BST_HR_MIN;
		else if (boost_param > COD9005X_SPK_BST_HR_MAX)
			cod9005x->boost_hr0 = COD9005X_SPK_BST_HR_MAX;
		else
			cod9005x->boost_hr0 = boost_param;
	else
		cod9005x->boost_hr0 = COD9005X_SPK_BST_HR0_DEFAULT;

	ret = of_property_read_u32(dev->of_node, "boost-hr1", &boost_param);
	if (!ret)
		if (boost_param < COD9005X_SPK_BST_HR_MIN)
			cod9005x->boost_hr1 = COD9005X_SPK_BST_HR_MIN;
		else if (boost_param > COD9005X_SPK_BST_HR_MAX)
			cod9005x->boost_hr1 = COD9005X_SPK_BST_HR_MAX;
		else
			cod9005x->boost_hr1 = boost_param;
	else
